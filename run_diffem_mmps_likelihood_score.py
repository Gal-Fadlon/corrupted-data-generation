"""
DiffEM with MMPS E-step — Likelihood Score Injection

DPS-style hybrid with MMPS. At each reverse step, after the MMPS posterior
denoiser, add an explicit likelihood gradient correction:

    delta = -lambda(sigma_t) * A^T(A*x_hat - y) / sigma_obs^2

where lambda(sigma_t) = sigma_t^2 / (sigma_t^2 + sigma_obs^2) vanishes when
the model is confident.

When sigma_obs=0: correction becomes hard data consistency (standard MMPS
behavior).
...
# =============================================================================
# Likelihood Score Decomposition-Projected MMPS
# =============================================================================

class LikelihoodScoreMMPSDiffusionProcess:
    """
    MMPS sampler with likelihood score injection and decomposition
    ...
    When sigma_obs>0, after the standard MMPS CG solve, an explicit DPS-style
    likelihood gradient correction is added:
        delta = -lambda(sigma_t) * A^T(A*x_hat - y) / sigma_obs^2
    where lambda(sigma_t) = sigma_t^2 / (sigma_t^2 + sigma_obs^2) vanishes
    ...
    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        ...
        # Likelihood score injection (DPS-style)
        if self.sigma_obs > 0:
            # Scale factor: strong when model uncertain, vanishes when confident
            lambda_t = sigma_sq / (sigma_sq + sigma_obs_sq)
            # Likelihood gradient: push toward observations
            likelihood_grad = -mask * (result - x_obs) / sigma_obs_sq
            result = result + lambda_t * likelihood_grad
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    print(f"\n=== E-Step (EM iter {em_iter}) — Likelihood Score MMPS ===")
    uncond_model.eval()
    ...
    with uncond_model.ema_scope():
        process = LikelihoodScoreMMPSDiffusionProcess(
        ...
        for start_idx in tqdm(range(0, N, batch_size), desc="E-step Likelihood Score"):
...
def main(args):
    ...
    name = f"likelihood_score_mmps_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        if sigma_obs > 0:
            print(f"  Likelihood score injection ACTIVE (sigma_obs={sigma_obs})")
        else:
        ...
        print(f"\n{'='*60}")
        print(f"Starting Likelihood-Score DiffEM-MMPS with {args.em_iters} EM iterations")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("Likelihood-Score DiffEM-MMPS Complete!")
        print("="*60)
        if best_metrics:
