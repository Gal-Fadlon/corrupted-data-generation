"""
DiffEM MMPS Gaussian Noise + Ambient Tweedie M-step (Approach E)

Based on run_diffem_mmps_gaussian_noise_v2.py. Adds Ambient Tweedie auxiliary
loss and consistency regularizer to the M-step, from the "Consistent Diffusion
Meets Tweedie" (ICML 2024) paper.

Combined M-step loss:
    loss = loss_standard + alpha * loss_ambient_dsm + beta * loss_consistency

where:
  - loss_standard: standard loss_fn_irregular (EDM denoising loss)
  - loss_ambient_dsm: Ambient VE DSM residual (learns below noise floor)
  - loss_consistency: lightweight consistency regularizer between two noise levels

Activated via --use_ambient_tweedie_mstep flag.
"""
...
from utils.ambient_tweedie_loss import ambient_ve_dsm_residual, consistency_denoiser_stub
...
def m_step(args, uncond_model, optimizer, reconstructions, em_iter, device,
    ...
    """M-step with optional Ambient Tweedie auxiliary losses."""
    epochs = num_epochs if num_epochs is not None else args.m_step_epochs

    use_tweedie = getattr(args, 'use_ambient_tweedie_mstep', False)
    alpha = getattr(args, 'ambient_tweedie_loss_weight', 0.1) if use_tweedie else 0.0
    beta = getattr(args, 'ambient_tweedie_consistency_weight', 0.0) if use_tweedie else 0.0
    sigma_n = getattr(args, 'corruption_noise_level', 0.0)
    ambient_concat = getattr(args, 'ambient_concat_further_mask', False)
    num_features = args.input_channels

    print(f"\n=== M-Step (EM iter {em_iter}) ===")
    print(f"Training for {epochs} epochs, tweedie={use_tweedie}, "
          f"alpha={alpha}, beta={beta}, sigma_n={sigma_n}")
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (x_clean,) in enumerate(recon_loader):
            ...
            loss_std, to_log = uncond_model.loss_fn_irregular(x_img)
            loss = loss_std

            if use_tweedie and alpha > 0 and sigma_n > 0:
                loss_dsm = ambient_ve_dsm_residual(
                    uncond_model.net, x_img, sigma_n,
                    ambient_concat_further_mask=ambient_concat,
                    num_features=num_features,
                )
                loss = loss + alpha * loss_dsm
                epoch_loss_dsm += loss_dsm.item()

            if use_tweedie and beta > 0:
                loss_con = consistency_denoiser_stub(
                    uncond_model.net, x_img,
                    sigma_a=sigma_n if sigma_n > 0 else 0.002,
                    sigma_b=1.0,
                    ambient_concat_further_mask=ambient_concat,
                    num_features=num_features,
                )
                loss = loss + beta * loss_con
        ...
        if (epoch + 1) % 10 == 0 or epoch == 0:
            msg = f"  M-step epoch {epoch+1}/{epochs}, loss: {avg_loss:.4f}"
            if use_tweedie:
                msg += (f"  (std={epoch_loss_std/num_batches:.4f}, "
                        f"dsm={epoch_loss_dsm/num_batches:.4f}, "
                        f"con={epoch_loss_con/num_batches:.4f})")
            print(msg)
        ...
        if logger is not None:
            ...
            if use_tweedie:
                logger.log('em/m_step_loss_std', epoch_loss_std / num_batches, global_step)
                logger.log('em/m_step_loss_dsm', epoch_loss_dsm / num_batches, global_step)
                logger.log('em/m_step_loss_con', epoch_loss_con / num_batches, global_step)
...
def main(args):
    ...
    name = f"diffem_mmps_gaussian_tweedie_{name}"
