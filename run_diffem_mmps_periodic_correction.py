...
After computing the MMPS correction via the Jacobian, average corrections at
points that are one period apart. This enforces the prior that seasonal patterns
repeat — if you observe a value at time t, the correction should be similar at
t+T, t+2T, etc.
"""
...
# =============================================================================
# MMPSDiffusionProcess — Periodic Correction
# =============================================================================

class MMPSDiffusionProcess:
    """
    MMPS-style diffusion sampler with periodic correction.

    After computing the standard MMPS score (Jacobian-based correction), the
    score is averaged across periodic intervals along the W (time) axis. This
    enforces that seasonal patterns repeat: the correction at time t should be
    similar to t+T, t+2T, etc.

    At high sigma the periodic-averaged score dominates (enforce structure);
    at low sigma the original score is used (allow fine details).
    """

    def __init__(self, args, diffusion_fn, shape, sigma_y=0.01, cg_iters=1,
                 period=None, period_blend_sigma_ref=1.0):
        self.args = args
        ...
        self.period = period  # None = auto-detect from image width
        self.period_blend_sigma_ref = period_blend_sigma_ref

    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        """
        Posterior denoiser with periodic correction.

        Standard MMPS posterior correction is computed, then the score is
        averaged at periodic intervals along the W axis. The blend between
        periodic-averaged and original score is controlled by sigma.
        ...
        # --- Periodic correction ---
        B, C, H, W = score.shape
        period = self.period if self.period is not None else max(3, W // 4)

        # Reshape to (B, C, H, n_periods, period) and average across periods
        n_full_periods = W // period
        W_trim = n_full_periods * period

        if n_full_periods >= 2:
            score_periodic = score[..., :W_trim].reshape(B, C, H, n_full_periods, period)
            score_period_avg = score_periodic.mean(dim=3, keepdim=True).expand_as(score_periodic)
            score_period_avg = score_period_avg.reshape(B, C, H, W_trim)

            # Handle remainder
            if W_trim < W:
                remainder = score[..., W_trim:]
                score_period_avg = torch.cat([score_period_avg, remainder], dim=-1)

            # Blend original and periodic-averaged score based on sigma
            # At high sigma: use periodic-averaged (enforce structure) -> w near 0
            # At low sigma: use original (allow fine details) -> w near 1
            w = torch.sigmoid((self.period_blend_sigma_ref - sigma) / 0.5)
            score_final = w * score + (1 - w) * score_period_avg
        else:
            # Not enough periods to average — use original score
            score_final = score

        # Detach from computation graph
        result = denoised.detach() + sigma_sq * score_final.detach()
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    ...
    period_blend_sigma_ref = getattr(args, 'period_blend_sigma_ref', 1.0)
    ...
    with uncond_model.ema_scope():
        process = MMPSDiffusionProcess(
            ...
            period=period, period_blend_sigma_ref=period_blend_sigma_ref
        )
...
def main(args):
    ...
    name = f"diffem_mmps_periodic_correction_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print(f"\n{'='*60}")
        print(f"Starting DiffEM-MMPS + Periodic Correction + Curriculum with {args.em_iters} EM iterations")
        print(f"  MMPS sigma_y: {args.mmps_sigma_y}")
        print(f"  MMPS cg_iters: {args.mmps_cg_iters}")
        print(f"  Period: {getattr(args, 'period', None) or 'auto'}")
        print(f"  Period blend sigma ref: {getattr(args, 'period_blend_sigma_ref', 1.0)}")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("DiffEM-MMPS + Periodic Correction + Curriculum Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS + Periodic Correction + Curriculum training is complete")
...
if __name__ == '__main__':
    ...
    if not hasattr(args, 'period'):
        args.period = None  # Auto-detect from image width
    if not hasattr(args, 'period_blend_sigma_ref'):
        args.period_blend_sigma_ref = 1.0
