"""
DiffEM with MMPS E-step — Spectral Jacobian Filtering

Projects the Jacobian-based covariance into the frequency domain.
Time-series covariance is approximately diagonal in Fourier space
(Wiener-Khinchin theorem). Applies VJP in frequency domain and
attenuates high-frequency Jacobian responses via a smooth low-pass filter:
    filter[k] = 1 / (1 + (k/k_cutoff)^2)
"""
...
# =============================================================================
# MMPSDiffusionProcess — Spectral Jacobian Filtering
# =============================================================================

class MMPSDiffusionProcess:
    """
    MMPS-style diffusion sampler with spectral Jacobian filtering.

    Projects the Jacobian correction into the frequency domain and attenuates
    high-frequency components using a smooth low-pass filter based on the
    Wiener-Khinchin theorem (time-series covariance is approx. diagonal in
    Fourier space).

    The spectral filter applied to Jacobian output:
        filter[k] = 1 / (1 + (k / k_cutoff)^2)
    where k_cutoff = spectral_cutoff_ratio * (W // 2).
    """

    def __init__(self, args, diffusion_fn, shape, sigma_y=0.01, cg_iters=1,
                 spectral_cutoff_ratio=0.5):
        self.args = args
        ...
        self.spectral_cutoff_ratio = spectral_cutoff_ratio

    def _build_spectral_filter(self, W, device, dtype):
        """
        Build a smooth low-pass filter for the rfft output along W axis.

        filter[k] = 1 / (1 + (k / k_cutoff)^2)

        Args:
            W: size of the time (W) dimension
            device: torch device
            dtype: torch dtype

        Returns:
            spectral_filter: tensor of shape (1, 1, 1, W//2+1) for broadcasting
        """
        num_freq = W // 2 + 1
        k = torch.arange(num_freq, device=device, dtype=dtype)
        k_cutoff = self.spectral_cutoff_ratio * (W / 2.0)
        spectral_filter = 1.0 / (1.0 + (k / max(k_cutoff, 1e-6)) ** 2)
        return spectral_filter.reshape(1, 1, 1, num_freq)

    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        """
        Posterior denoiser with spectral Jacobian filtering.

        Same as standard MMPS posterior denoiser, but the CG operator applies
        a spectral low-pass filter to the Jacobian response before combining
        ...
        # Build spectral filter for this input's W dimension
        W = x_t.shape[-1]
        spectral_filter = self._build_spectral_filter(W, x_t.device, x_t.dtype)
        ...
        # CG operator with spectral filtering of Jacobian response
        def cg_operator(v):
            jac_response = vjp_fn(mask * v)
            # Apply spectral filter to Jacobian response along W axis
            jac_freq = torch.fft.rfft(jac_response, dim=-1)
            jac_freq = jac_freq * spectral_filter
            jac_filtered = torch.fft.irfft(jac_freq, n=jac_response.shape[-1], dim=-1)
            return sigma_y_sq * v + sigma_sq * mask * jac_filtered

        # Solve (sigma_y^2 I + sigma^2 * mask * spectral_filtered_J * mask) v = r
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    ...
    Uses MMPSDiffusionProcess with spectral Jacobian filtering.
    ...
    """
    print(f"\n=== E-Step (EM iter {em_iter}) — MMPS Imputation (Spectral Jacobian) ===")
    print("Imputing missing values via MMPS with spectral Jacobian filtering...")
    ...
    spectral_cutoff_ratio = getattr(args, 'spectral_cutoff_ratio', 0.5)
    ...
    with uncond_model.ema_scope():
        process = MMPSDiffusionProcess(
            ...
            spectral_cutoff_ratio=spectral_cutoff_ratio
        )
...
def main(args):
    """Main DiffEM-Uncond training loop with MMPS E-step (Spectral Jacobian)."""

    # Model name and directory
    name = create_model_name_and_dir(args)
    name = f"diffem_mmps_spectral_jacobian_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print(f"\n{'='*60}")
        print(f"Starting DiffEM-MMPS + Spectral Jacobian with {args.em_iters} EM iterations")
        print(f"  MMPS sigma_y: {args.mmps_sigma_y}")
        print(f"  MMPS cg_iters: {args.mmps_cg_iters}")
        print(f"  Spectral cutoff ratio: {args.spectral_cutoff_ratio}")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("DiffEM-MMPS + Spectral Jacobian Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS + Spectral Jacobian training is complete")
...
if __name__ == '__main__':
    ...
    # Spectral Jacobian parameters
    if not hasattr(args, 'spectral_cutoff_ratio'):
        args.spectral_cutoff_ratio = 0.5
