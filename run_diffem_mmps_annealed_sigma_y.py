"""
DiffEM with MMPS E-step — Annealed σ_y

Makes σ_y depend on the current diffusion noise level σ:
    σ_y_effective = σ_y_base · (1 + anneal_strength · σ / σ_max)

At high σ (early sampling, model uncertain) σ_y is inflated so observations
are trusted less.  At low σ (late sampling) σ_y shrinks towards σ_y_base,
enforcing tight observation consistency.
"""
...
# =============================================================================
# MMPSDiffusionProcess — Annealed σ_y
# =============================================================================

class MMPSDiffusionProcess:
    """
    MMPS-style diffusion sampler with noise-level-annealed observation noise.

    σ_y is dynamically computed at each reverse step as:
        σ_y_effective = σ_y_base · (1 + anneal_strength · σ / σ_max)
    This inflates σ_y at high noise levels (early steps) and shrinks it at
    low noise levels (late steps), providing adaptive observation trust.
    """

    def __init__(self, args, diffusion_fn, shape, sigma_y=0.01, cg_iters=1,
                 sigma_y_base=0.01, anneal_strength=2.0):
        self.args = args
        ...
        # Annealed σ_y parameters
        self.sigma_y_base = sigma_y_base
        self.anneal_strength = anneal_strength

    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        """
        Posterior denoiser with noise-level-annealed σ_y.

        σ_y_effective = σ_y_base · (1 + anneal_strength · σ / σ_max)

        At high σ (early sampling):  σ_y is large  -> trust observations less
        At low σ  (late sampling):   σ_y ~ σ_y_base -> enforce observations tightly
        ...
        # Dynamic σ_y: inflate at high noise, shrink at low noise
        sigma_y_effective = self.sigma_y_base * (1 + self.anneal_strength * (sigma / self.sigma_max))
        sigma_y_sq = sigma_y_effective ** 2
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    ...
    sigma_y_base = getattr(args, 'sigma_y_base', 0.01)
    anneal_strength = getattr(args, 'anneal_strength', 2.0)
    ...
    with uncond_model.ema_scope():
        process = MMPSDiffusionProcess(
            ...
            sigma_y_base=sigma_y_base, anneal_strength=anneal_strength
        )
...
def main(args):
    ...
    name = f"diffem_mmps_annealed_sigma_y_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print(f"  sigma_y_base: {args.sigma_y_base}")
        print(f"  anneal_strength: {args.anneal_strength}")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("DiffEM-MMPS + Annealed σ_y Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS + Annealed σ_y training is complete")
...
if __name__ == '__main__':
    ...
    # Annealed σ_y parameters (defaults)
    if not hasattr(args, 'sigma_y_base'):
        args.sigma_y_base = 0.01
    if not hasattr(args, 'anneal_strength'):
        args.anneal_strength = 2.0
