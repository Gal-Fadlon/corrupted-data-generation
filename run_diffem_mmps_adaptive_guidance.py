"""
DiffEM with MMPS E-step — Adaptive Guidance Scale
...
Unified model that handles all corruption regimes:
  ...
  - Observation noise only (missing_rate=0): E-step adaptively scales guidance
  - Missing + noise: imputes and denoises in one pass

Key idea: Instead of fixed sigma_obs, adaptively scale the observation guidance
at each denoising step by comparing the MMPS-corrected result with the
uncorrected denoiser output.

When the MMPS correction and the raw denoiser agree at observed positions, the
correction is helping — trust it (alpha ~ 1).  When they disagree strongly, the
correction may be fitting observation noise — reduce it (alpha ~ 0).

    alpha = 1 / (1 + disagreement / sigma_obs^2)

When sigma_obs=0: the adaptive branch is skipped and the result is standard
MMPS (alpha is always effectively 1, no change).

KEY ANISOTROPIC NOISE FIX: Uses per-feature noise std so that the observation
noise covariance correctly reflects the feature-dependent noise that
add_gaussian_noise actually applies.
...
# =============================================================================
# Adaptive Guidance Scale Decomposition-Projected MMPS
# =============================================================================

class AdaptiveGuidanceMMPSDiffusionProcess:
    """
    MMPS sampler with adaptive guidance scaling and decomposition
    ...
    When sigma_obs>0, the observation guidance is adaptively scaled at each
    reverse diffusion step.  The scale alpha is computed by comparing the
    MMPS-corrected result with the uncorrected denoiser output at observed
    positions:

        alpha = 1 / (1 + disagreement / sigma_obs^2)

    This means:
      - When MMPS correction agrees with denoiser: alpha ~ 1 (trust correction)
      - When MMPS correction disagrees strongly: alpha ~ 0 (correction may be
        fitting noise, reduce it)

    KEY ANISOTROPIC NOISE FIX: accepts per-feature noise std via
    sigma_obs_per_channel so the CG operator uses the correct observation
    noise covariance.
    """
    ...
    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        """
        Adaptive guidance posterior denoiser.

        When sigma_obs=0: standard MMPS (residual = y_obs - mask * denoised).
        When sigma_obs>0: compute full MMPS correction, then adaptively scale
        it based on agreement with the uncorrected denoiser at observed
        positions.
        """
        sigma_sq = sigma ** 2
        ...
        # Adaptive guidance: compare MMPS correction with uncorrected denoiser
        if self.sigma_obs > 0:
            uncond_result = denoised.detach()
            # Disagreement at observed positions
            correction = mmps_result - uncond_result
            correction_at_obs = mask * correction
            # Per-channel mean sigma_obs for normalization
            if self.sigma_obs_per_channel is not None:
                mean_sigma_obs_sq = (self.sigma_obs_per_channel.to(x_t.device).to(torch.float64) ** 2).mean().item()
            else:
                mean_sigma_obs_sq = self.sigma_obs ** 2
            # Scale factor: reduce correction when disagreement is large relative to noise
            disagreement_sq = (correction_at_obs ** 2).mean()
            # alpha = 1 / (1 + disagreement / sigma_obs^2)
            # When disagreement is small: alpha ~ 1 (trust correction)
            # When disagreement is large: alpha ~ 0 (correction may be fitting noise)
            alpha = 1.0 / (1.0 + float(disagreement_sq) / (mean_sigma_obs_sq + 1e-10))
            result = uncond_result + alpha * correction
        else:
            result = mmps_result
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    print(f"\n=== E-Step (EM iter {em_iter}) — Adaptive-Guidance MMPS ===")
    uncond_model.eval()
    ...
    if sigma_obs > 0:
        per_feature_std = compute_per_feature_noise_std(corrupted_data, sigma_obs)
        sigma_obs_per_channel = torch.tensor(per_feature_std, dtype=torch.float32)
        print(f"  Per-feature noise std: min={per_feature_std.min():.4f}, "
              f"max={per_feature_std.max():.4f}, mean={per_feature_std.mean():.4f}")
    else:
        sigma_obs_per_channel = None

    with uncond_model.ema_scope():
        process = AdaptiveGuidanceMMPSDiffusionProcess(
        ...
        for start_idx in tqdm(range(0, N, batch_size), desc="E-step Adaptive-Guidance"):
...
def main(args):
    ...
    name = f"adaptive_guidance_mmps_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        if sigma_obs > 0:
            print(f"  Adaptive guidance scaling ACTIVE (sigma_obs={sigma_obs})")
        else:
        ...
        print(f"\n{'='*60}")
        print(f"Starting Adaptive-Guidance DiffEM-MMPS with {args.em_iters} EM iterations")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("Adaptive-Guidance DiffEM-MMPS Complete!")
        print("="*60)
        if best_metrics:
