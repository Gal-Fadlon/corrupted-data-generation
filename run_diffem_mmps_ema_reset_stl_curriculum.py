"""
DiffEM with Unconditional Model + MMPS E-step — EMA Reset + STL Curriculum

Same as run_diffem_mmps_ema_reset_stl_init.py with ONE enhancement:
  After STL initialization, each sample is scored by "difficulty" — the
  ratio of residual variance to total variance.  During early EM iterations,
  only easy + medium samples (where STL init is most reliable) are used in
  the M-step.  Later iterations use all samples.

  This is curriculum learning applied to the EM loop: the model first learns
  clean structural patterns from reliable samples, then incorporates harder
  ones once the prior is strong enough to produce good imputations for them.

  New parameters (via getattr with defaults):
    curriculum_warmup_frac  (default 0.5): fraction of em_iters with curriculum
    curriculum_easy_frac    (default 0.7): fraction of samples used during warmup
"""
...
# =============================================================================
# Difficulty Scoring (NEW)
# =============================================================================

def compute_difficulty_scores(initial_reconstructions, obs_masks, period=None):
    """
    Score each sample's difficulty based on how much of its variance STL
    cannot explain (residual-to-total ratio).

    Low difficulty  = STL captures most of the signal (strong structure)
    High difficulty = large unpredictable residual (weak structure, unreliable init)

    Args:
        initial_reconstructions: (N, T, C) fully filled (from STL init)
        obs_masks: (N, T) boolean, True = observed
        period: STL period (auto-detected if None)

    Returns:
        sorted_indices: array of sample indices sorted by difficulty (ascending)
        difficulty_scores: array of difficulty scores per sample
    """
...
def main(args):
    """Main DiffEM-Uncond training loop with MMPS E-step + STL curriculum."""
    ...
    name = f"diffem_mmps_ema_reset_stl_curriculum_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        # During initial training, use only the easy samples (curriculum from the start)
        n_easy_init = int(len(sorted_indices) * curriculum_easy_frac)
        easy_idx_init = sorted_indices[:n_easy_init]

        print(f"\n{'='*60}")
        print(f"Phase 1.5: Initial model training on {n_easy_init}/{len(sorted_indices)} "
              f"easiest STL-initialized samples")
        print(f"{'='*60}")
        ...
        print(f"  Curriculum: warmup={warmup_end} iters, easy_frac={curriculum_easy_frac}")
        print(f"{'='*60}")
        ...
        for em_iter in range(args.em_iters):
            ...
            if em_iter < warmup_end:
                ...
                phase_label = f"CURRICULUM (using {n_use}/{len(sorted_indices)} easiest samples)"
            else:
            ...
            # E-step: impute ALL samples (hard samples also need good imputations)
            ...
            if em_iter < warmup_end:
                ...
                uncond_model = m_step(
                    args, uncond_model, optimizer,
                    reconstructions[train_idx], em_iter, args.device, logger
                )
            else:
                uncond_model = m_step(
                    args, uncond_model, optimizer,
        ...
        if final_metrics:
            ...
            for k, v in final_metrics.items():
                print(f"  {k}: {v:.4f}")
        print("\n" + "="*60)
        print("DiffEM-MMPS + STL Curriculum Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS + STL Curriculum training is complete")
...
if __name__ == '__main__':
    ...
    # Curriculum-specific defaults (can be overridden via CLI if added to args parser)
