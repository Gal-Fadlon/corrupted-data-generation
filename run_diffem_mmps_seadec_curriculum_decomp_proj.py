"""
DiffEM with MMPS E-step — Seasonal-Aware (SeaDec) Init + Curriculum + Decomposition Projection

Stability-focused combination of three orthogonal improvements:
  1. Seasonal-aware initialization (SeaDec) — deseasonalizes with STL, imputes
     the detrended series with PCHIP interpolation, then reseasonalizes. Smooth
     and stable, avoids overfitting artifacts.
  2. Decomposition-projected MMPS E-step — after each posterior denoising step,
     the estimate is projected through a decomposition-recomposition that
...
Based on run_diffem_mmps_iter_stl_decomp_projection.py with init swapped to
initialize_with_seasonal_interp.
"""
...
from utils.utils_stl import initialize_with_seasonal_interp, auto_detect_period
...
def main(args):
    ...
    name = f"diffem_mmps_seadec_curriculum_decomp_proj_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print(f"\n{'='*60}")
        print("Phase 1: Seasonal-Aware (SeaDec) Warm-Start Initialization")
        print(f"{'='*60}")
        ...
        print(f"\n{'='*60}")
        print(f"Starting SeaDec + Curriculum + Decomp-Projected DiffEM-MMPS with {args.em_iters} EM iterations")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("SeaDec + Curriculum + Decomp-Projected DiffEM-MMPS Complete!")
        print("="*60)
        if best_metrics:
