...
+ Annealed Observation Mask

At high sigma (early sampling), only use the most "reliable" observed points —
those most consistent with the current denoised estimate. Gradually include all
observations as sigma decreases. This prevents outlier observations (seasonal peaks
the model hasn't learned yet) from corrupting corrections at high noise levels.
"""
...
# =============================================================================
# MMPSDiffusionProcess — Annealed Observation Mask
# =============================================================================

class MMPSDiffusionProcess:
    """
    MMPS-style diffusion sampler with annealed observation masking.

    At high noise levels, only the most reliable (low-error) observed points
    are used. As sigma decreases toward zero, all observations are gradually
    included. This prevents outlier observations from corrupting the posterior
    correction when the denoiser is still very uncertain.
    """

    def __init__(self, args, diffusion_fn, shape, sigma_y=0.01, cg_iters=1,
                 reliability_temp=2.0, min_mask_ratio=0.3):
        self.args = args
        ...
        # Annealed observation mask parameters
        self.reliability_temp = reliability_temp
        self.min_mask_ratio = min_mask_ratio

    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        """
        Posterior denoiser with annealed observation masking.

        At high sigma, only the most reliable observed points (those consistent
        with the current denoised estimate) are used. As sigma -> 0, all
        ...
        # --- Annealed observation mask ---
        # Per-pixel squared error between observation and denoised (at observed positions)
        error_sq = ((x_obs - denoised.detach()) * mask) ** 2
        # Average over channels
        error_per_point = error_sq.mean(dim=1, keepdim=True)  # (B, 1, H, W)

        # Normalize errors to [0, 1] range per sample
        max_err = error_per_point.amax(dim=(-2, -1), keepdim=True).clamp(min=1e-10)
        normalized_err = error_per_point / max_err
        reliability = torch.exp(-normalized_err * self.reliability_temp * (sigma / self.sigma_max))

        # Threshold: keep at least min_mask_ratio of observed points
        # Compute threshold only over observed positions
        observed_reliability = reliability[mask.bool()]
        if observed_reliability.numel() > 0:
            threshold = torch.quantile(observed_reliability.flatten(),
                                       1 - self.min_mask_ratio)
        else:
            threshold = torch.tensor(0.0, device=mask.device, dtype=mask.dtype)

        effective_mask = mask * (reliability >= threshold.clamp(max=0.99)).float()

        # As sigma -> 0, use all observations; at high sigma, use effective_mask
        blend = (sigma / self.sigma_max).clamp(max=1.0)
        effective_mask = (1 - blend) * mask + blend * effective_mask

        # --- Standard MMPS with effective_mask ---
        ...
        # Compute residual using effective_mask
        r = x_obs - effective_mask * denoised

        # CG linear operator with effective_mask
        def cg_operator(v):
            return sigma_y_sq * v + sigma_sq * effective_mask * vjp_fn(effective_mask * v)

        # Solve linear system
        v = conjugate_gradient(cg_operator, r, maxiter=self.cg_iters)

        # Compute posterior correction
        score = vjp_fn(effective_mask * v)
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    ...
    reliability_temp = getattr(args, 'reliability_temp', 2.0)
    min_mask_ratio = getattr(args, 'min_mask_ratio', 0.3)
    ...
    with uncond_model.ema_scope():
        process = MMPSDiffusionProcess(
            ...
            reliability_temp=reliability_temp, min_mask_ratio=min_mask_ratio
        )
...
def main(args):
    ...
    name = f"diffem_mmps_annealed_obs_mask_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print(f"\n{'='*60}")
        print(f"Starting DiffEM-MMPS + Annealed Obs Mask + Curriculum with {args.em_iters} EM iterations")
        print(f"  MMPS sigma_y: {args.mmps_sigma_y}")
        print(f"  MMPS cg_iters: {args.mmps_cg_iters}")
        print(f"  Reliability temp: {getattr(args, 'reliability_temp', 2.0)}")
        print(f"  Min mask ratio: {getattr(args, 'min_mask_ratio', 0.3)}")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("DiffEM-MMPS + Annealed Obs Mask + Curriculum Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS + Annealed Obs Mask + Curriculum training is complete")
...
if __name__ == '__main__':
    ...
    # Annealed observation mask parameters
    if not hasattr(args, 'reliability_temp'):
        args.reliability_temp = 2.0
    if not hasattr(args, 'min_mask_ratio'):
        args.min_mask_ratio = 0.3
