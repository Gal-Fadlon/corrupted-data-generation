"""
DiffEM with MMPS E-step — Decomposition-Aware Pre-Denoising

Before passing observations to MMPS, decompose via STL into trend + seasonal +
residual. Attenuate the residual proportional to sigma_obs (noise lives in the
residual). MMPS sees pre-denoised observations. Across EM iterations the model
improves -> decomposition improves -> pre-denoising improves.
When sigma_obs=0: no attenuation (unchanged).
...
# =============================================================================
# Decomp Pre-Denoise Decomposition-Projected MMPS
# =============================================================================

class DecompPredenoiseMMPSDiffusionProcess:
    """
    MMPS sampler with decomposition-aware pre-denoising of observations and
    ...
    When sigma_obs>0, observations are pre-denoised via STL decomposition before
    being passed to MMPS. The residual component (where noise lives) is attenuated
    proportional to sigma_obs. Across EM iterations, as the model improves, the
    decomposition improves, and the pre-denoising improves.
    """
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    print(f"\n=== E-Step (EM iter {em_iter}) — Decomp Pre-Denoise MMPS ===")
    uncond_model.eval()
    ...
    # Pre-denoise observations via STL decomposition
    if sigma_obs > 0:
        predenoised_data = corrupted_data.copy()
        # Only process observed positions
        for i in range(len(predenoised_data)):
            seq = predenoised_data[i]  # (T, C)
            ...
            seq_3d = seq_filled[np.newaxis, :, :]  # (1, T, C)
            projected = decompose_and_smooth_ts(seq_3d, residual_keep_fraction=0.0)  # remove all residual
            denoised_seq = projected[0]  # (T, C)
            # Estimate residual std from observed positions
            obs_idx = mask_i.astype(bool)
            residual = seq_filled[obs_idx] - denoised_seq[obs_idx]
            residual_std = max(np.std(residual), 1e-6)
            # Attenuation: keep fraction of residual based on noise vs signal
            keep_fraction = max(0.0, 1.0 - sigma_obs / residual_std)
            # Blend: denoised + keep_fraction * residual at observed positions
            predenoised = denoised_seq + keep_fraction * (seq_filled - denoised_seq)
            predenoised_data[i] = np.where(
                obs_idx[:, np.newaxis],
                predenoised,
                seq  # keep NaN at missing positions
            )
        print(f"  Pre-denoised observations (σ_obs={sigma_obs:.3f})")
    else:
        predenoised_data = corrupted_data

    with uncond_model.ema_scope():
        process = DecompPredenoiseMMPSDiffusionProcess(
        ...
        for start_idx in tqdm(range(0, N, batch_size), desc="E-step Decomp Pre-Denoise"):
            end_idx = min(start_idx + batch_size, N)

            corrupted_batch = predenoised_data[start_idx:end_idx]
...
def main(args):
    ...
    name = f"decomp_predenoise_mmps_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        if sigma_obs > 0:
            print(f"  Decomp pre-denoising ACTIVE (sigma_obs={sigma_obs})")
        else:
        ...
        print(f"\n{'='*60}")
        print(f"Starting Decomp Pre-Denoise DiffEM-MMPS with {args.em_iters} EM iterations")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("Decomp Pre-Denoise DiffEM-MMPS Complete!")
        print("="*60)
        if best_metrics:
