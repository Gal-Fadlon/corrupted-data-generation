"""
DiffEM with MMPS E-step — Triple Combo: Iter-STL Init + Decomp-Projected E-step + Decomp-Weighted M-step

Combines all three strongest orthogonal improvements:
  1. Iterative STL initialization (best init) — high-quality warm start via
     iterative STL decomposition → impute → re-decompose cycles.
  2. Decomposition-projected MMPS E-step (best E-step) — after each posterior
     denoising step, the estimate is projected through a decomposition that
     enforces structural constraints. Projection strength is sigma-dependent.
  3. Decomposition-weighted loss M-step (best M-step) — per-pixel importance
     weighting where high-frequency residual regions get higher loss weight,
     focusing the model on hard-to-reconstruct details.

Also retains curriculum reveal from decomp_projection.
"""
...
# =============================================================================
# Fast decomposition-projection in TS space (for E-step)
# =============================================================================
...
def decompose_and_smooth_ts(ts_batch, period=None, trend_smooth_window=5,
    ...
    Used inside the E-step sampler for structural projection.
    """
...
# =============================================================================
# Decomposition-Weighted Loss Helpers (for M-step)
# =============================================================================
...
def compute_importance_weights(reconstructions, window=5):
    """
    Per-sample importance weights from MA decomposition.
    High-frequency residual regions get higher weight.
    """
...
# =============================================================================
# Decomposition-Projected MMPSDiffusionProcess (E-step sampler)
# =============================================================================
...
class DecompProjectedMMPSDiffusionProcess:
    ...
    After each MMPS posterior correction, the denoised estimate is
    projected through decomposition-recomposition. The blending weight
    alpha = sigma / sigma_max ensures projection is strongest at high noise
    and vanishes at low noise (preserving fine details).
    """
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    """E-step: Decomposition-projected MMPS imputation."""
...
def m_step(args, uncond_model, optimizer, reconstructions, em_iter, device, logger=None):
    """M-step with decomposition-weighted loss."""
...
def main(args):
    ...
    name = f"diffem_mmps_triple_combo_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print(f"\n{'='*60}")
        print(f"Starting Triple-Combo DiffEM-MMPS with {args.em_iters} EM iterations")
        print(f"  Init: Iterative STL")
        print(f"  E-step: Decomposition-projected MMPS")
        print(f"  M-step: Decomposition-weighted loss")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("Triple-Combo DiffEM-MMPS Complete!")
        print("  (Iter-STL Init + Decomp-Projected E-step + Decomp-Weighted M-step)")
        print("="*60)
        if best_metrics:
