"""
DiffEM with MMPS E-step — Frequency-Dependent σ_y

Different frequency bands of observations have different noise levels.
Low frequencies (trend) are more reliable, high frequencies are noisier.
The observation noise covariance is applied per-frequency via FFT along
the W (time) axis:
    σ_y(f) = σ_y_low   if f < cutoff
    σ_y(f) = σ_y_high  if f >= cutoff
"""
...
# =============================================================================
# MMPSDiffusionProcess — Frequency-Dependent σ_y
# =============================================================================

class MMPSDiffusionProcess:
    """
    MMPS-style diffusion sampler with frequency-dependent observation noise.

    Low-frequency components of the observation are trusted more (small σ_y),
    high-frequency components are trusted less (large σ_y).  The weighting
    is applied in Fourier space along the W (time) axis.
    """

    def __init__(self, args, diffusion_fn, shape, sigma_y=0.01, cg_iters=1,
                 sigma_y_low_freq=0.001, sigma_y_high_freq=0.05,
                 freq_cutoff_ratio=0.2):
        self.args = args
        ...
        # Frequency-dependent σ_y parameters
        self.sigma_y_low_freq = sigma_y_low_freq
        self.sigma_y_high_freq = sigma_y_high_freq
        self.freq_cutoff_ratio = freq_cutoff_ratio

    def _apply_freq_sigma_y(self, v):
        """
        Apply frequency-dependent σ_y² weighting to v via FFT along the W axis.

        Returns a tensor of the same shape as v, where each frequency component
        has been scaled by the appropriate σ_y².
        """
        W = v.shape[-1]

        # FFT along the last (W / time) dimension
        v_freq = torch.fft.rfft(v, dim=-1)

        # Number of frequency bins
        n_freqs = v_freq.shape[-1]
        cutoff_bin = int(self.freq_cutoff_ratio * n_freqs)
        cutoff_bin = max(cutoff_bin, 1)

        # Build per-frequency σ_y² array
        sigma_y_sq = torch.full((n_freqs,), self.sigma_y_high_freq ** 2,
                                dtype=v.dtype, device=v.device)
        sigma_y_sq[:cutoff_bin] = self.sigma_y_low_freq ** 2

        # Reshape for broadcasting: (1, 1, 1, n_freqs)
        sigma_y_sq = sigma_y_sq.reshape(1, 1, 1, -1)

        # Multiply in frequency domain
        v_freq_weighted = v_freq * sigma_y_sq

        # IFFT back to spatial domain
        result = torch.fft.irfft(v_freq_weighted, n=W, dim=-1)
        return result

    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        """
        Posterior denoiser with frequency-dependent observation noise.

        The CG operator becomes:
            M(v) = Σ_y(f) · v  +  σ² · mask · Jᵀ(mask · v)
        where Σ_y(f) applies different σ_y² per frequency band via FFT.
        """
        sigma_sq = sigma ** 2
        ...
        # CG linear operator: M(v) = Σ_y(f) · v + σ² · mask · Jᵀ(mask · v)
        def cg_operator(v):
            return self._apply_freq_sigma_y(v) + sigma_sq * mask * vjp_fn(mask * v)

        # Solve (Σ_y(f) + σ² · mask · Jᵀ · mask) · v = r
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    ...
    sigma_y_low_freq = getattr(args, 'sigma_y_low_freq', 0.001)
    sigma_y_high_freq = getattr(args, 'sigma_y_high_freq', 0.05)
    freq_cutoff_ratio = getattr(args, 'freq_cutoff_ratio', 0.2)
    ...
    with uncond_model.ema_scope():
        process = MMPSDiffusionProcess(
            ...
            sigma_y_low_freq=sigma_y_low_freq,
            sigma_y_high_freq=sigma_y_high_freq,
...
def main(args):
    ...
    name = f"diffem_mmps_freq_dependent_sigma_y_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print(f"  sigma_y_low_freq: {args.sigma_y_low_freq}")
        print(f"  sigma_y_high_freq: {args.sigma_y_high_freq}")
        print(f"  freq_cutoff_ratio: {args.freq_cutoff_ratio}")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("DiffEM-MMPS + Freq-Dependent σ_y Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS + Freq-Dependent σ_y training is complete")
...
if __name__ == '__main__':
    ...
    # Frequency-dependent σ_y parameters (defaults)
    if not hasattr(args, 'sigma_y_low_freq'):
        args.sigma_y_low_freq = 0.001
    if not hasattr(args, 'sigma_y_high_freq'):
        args.sigma_y_high_freq = 0.05
    if not hasattr(args, 'freq_cutoff_ratio'):
        args.freq_cutoff_ratio = 0.2
