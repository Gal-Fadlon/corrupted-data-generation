"""
DiffEM with MMPS E-step — EMA Reset + STL Init + Multiple Posterior Samples

Fork of run_diffem_mmps_ema_reset_stl_init.py with ONE change:
  In the E-step, K posterior samples are generated per batch and averaged
  for more robust imputation.  At high missing rates the posterior is broad;
  averaging reduces variance and yields stabler training data.
  Use --num_posterior_samples 3 (default) or higher.
"""
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    ...
    print(f"\n=== E-Step (EM iter {em_iter}) — MMPS Imputation (K={K} samples) ===")
    ...
    with uncond_model.ema_scope():
        ...
        for start_idx in tqdm(range(0, N, batch_size), desc="E-step MMPS"):
            ...
            # MMPS sampling: average K posterior samples for robustness
            K = getattr(args, 'num_posterior_samples', 3)
            all_samples = []
            for _ in range(K):
    ...
    print(f"E-step complete. Imputed {len(reconstructions)} sequences (avg of {K} samples).")
...
def main(args):
    ...
    name = f"diffem_mmps_stl_multi_sample_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        # === Phase 2: EM Loop ===
        print(f"\n{'='*60}")
        print(f"Starting DiffEM-MMPS + Multi-Sample with {args.em_iters} EM iterations")
        print(f"  MMPS sigma_y: {args.mmps_sigma_y}")
        print(f"  MMPS cg_iters: {args.mmps_cg_iters}")
        print(f"  num_posterior_samples: {getattr(args, 'num_posterior_samples', 3)}")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("DiffEM-MMPS + Multi-Sample Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS + Multi-Sample training is complete")
