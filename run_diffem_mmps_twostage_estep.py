"""
DiffEM with MMPS E-step — Two-Stage E-step (MMPS then Denoise)
...
Key idea: Two-stage approach to handle noisy, missing data.
  Stage 1: Standard MMPS (imputes missing, roughly handles noise).
  Stage 2: Take MMPS output, add noise at sigma_obs level, run unconditional
           reverse diffusion from sigma_obs -> 0 (pure denoising via learned prior).
  When sigma_obs=0: skip stage 2 entirely.
...
# =============================================================================
# Two-Stage MMPS with Decomposition Projection
# =============================================================================

class TwoStageMMPSDiffusionProcess:
    """
    Two-stage MMPS sampler with decomposition consistency projection.
    ...
    When sigma_obs>0, stage 1 handles imputation and rough denoising, then
    stage 2 leverages the learned unconditional prior to clean up remaining
    observation noise via pure reverse diffusion.
    """
    ...
    def denoise_pass(self, x_img):
        """Stage 2: unconditional denoising from sigma_obs to 0."""
        if self.sigma_obs <= 0:
            return x_img

        sigma_min = max(self.sigma_min, self.net.sigma_min)
        sigma_start = max(self.sigma_obs, sigma_min)

        # Add noise at sigma_obs level
        x_noisy = x_img.to(torch.float64) + torch.randn_like(x_img).to(torch.float64) * sigma_start

        # Build schedule from sigma_start to 0
        num_denoise_steps = max(10, self.num_steps // 4)
        step_indices = torch.arange(num_denoise_steps, dtype=torch.float64, device=x_img.device)
        t_steps = (sigma_start ** (1 / self.rho) + step_indices / (num_denoise_steps - 1) * (
                    sigma_min ** (1 / self.rho) - sigma_start ** (1 / self.rho))) ** self.rho
        t_steps = torch.cat([self.net.round_sigma(t_steps), torch.zeros_like(t_steps[:1])])

        x_next = x_noisy
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):
            x_cur = x_next
            gamma = min(self.S_churn / num_denoise_steps, np.sqrt(2) - 1) if self.S_min <= t_cur <= self.S_max else 0
    ...
    def sampling_mmps(self, x_obs_img, mask_img):
        ...
        # Stage 1: MMPS
        x_mmps = self.mmps_sample(latents, x_obs_img, mask_img)
        # Stage 2: Denoise (no-op when sigma_obs=0)
        x_denoised = self.denoise_pass(x_mmps)
        return x_denoised
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    print(f"\n=== E-Step (EM iter {em_iter}) — Two-Stage MMPS ===")
    uncond_model.eval()
    ...
    with uncond_model.ema_scope():
        process = TwoStageMMPSDiffusionProcess(
        ...
        for start_idx in tqdm(range(0, N, batch_size), desc="E-step Two-Stage"):
...
def main(args):
    ...
    name = f"twostage_estep_mmps_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        if sigma_obs > 0:
            print(f"  Two-stage E-step ACTIVE (sigma_obs={sigma_obs})")
        else:
        ...
        print(f"\n{'='*60}")
        print(f"Starting Two-Stage DiffEM-MMPS with {args.em_iters} EM iterations")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("Two-Stage DiffEM-MMPS Complete!")
        print("="*60)
        if best_metrics:
