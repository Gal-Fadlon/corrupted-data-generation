"""
DiffEM with MMPS E-step + Ambient Diffusion M-step (Approach D)

Same MMPS E-step as run_diffem_mmps.py, but the M-step uses Ambient Diffusion
loss instead of standard loss_fn_irregular on full reconstructions.

Key idea: prevent error amplification by not training on imputed (potentially
wrong) values at full weight. The Ambient M-step:
  1. Applies further corruption (randomly masks delta fraction of observed positions)
  2. Computes loss only on surviving observed positions (loss_fn_ambient_masked)
  3. Optionally concatenates the further-corruption mask as an extra UNet channel

A blending schedule interpolates from lambda_obs=1.0 (pure ambient) at early EM
iters to lambda_obs_end (mostly standard loss) at later iters.
"""
...
class MMPSDiffusionProcess:
    ...
    def _net_forward(self, x, sigma, labels=None):
        """Call self.net with ambient mask concatenation if needed."""
        x_in = concat_ones_mask(x, self._ambient_concat)
        out = self.net(x_in, sigma, labels).to(torch.float64)
        if self._ambient_concat:
            out = out[:, :self._data_c, :, :]
        return out
    ...
    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        ...
        x_in = concat_ones_mask(x_t_input, self._ambient_concat)
        denoised_raw = self.net(x_in, sigma, None).to(torch.float64)
        denoised = denoised_raw[:, :self._data_c, :, :] if self._ambient_concat else denoised_raw
    ...
    def sampling(self, sampling_number=16):
        ...
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):
            ...
            denoised = self._net_forward(x_hat, t_hat)
            d_cur = (x_hat - denoised) / t_hat
            x_next = x_hat + (t_next - t_hat) * d_cur

            if i < self.num_steps - 1:
                denoised = self._net_forward(x_next, t_next)
...
def m_step_ambient(args, uncond_model, optimizer, reconstructions, obs_masks,
                   em_iter, total_em_iters, device, logger=None):
    """
    Ambient M-step: blend ambient loss (on observed positions only, with further
    corruption) and standard loss (on full reconstructions) with a schedule.
    """
    ...
    progress = max(0.0, em_iter) / max(total_em_iters - 1, 1) if total_em_iters > 1 else 1.0
    progress = min(progress, 1.0)
    lambda_obs = lambda_obs_start + (lambda_obs_end - lambda_obs_start) * progress

    print(f"\n=== Ambient M-Step (EM iter {em_iter}) ===")
    print(f"  lambda_obs={lambda_obs:.2f}, delta={delta}, epochs={args.m_step_epochs}")

    recon_tensor = torch.tensor(reconstructions, dtype=torch.float32)
    mask_tensor = torch.tensor(
        np.repeat(obs_masks[:, :, np.newaxis], reconstructions.shape[-1], axis=2),
        dtype=torch.float32,
    )
    ...
    for epoch in range(args.m_step_epochs):
        ...
        for x_clean, mask_ts in recon_loader:
            x_clean = x_clean.to(device)
            mask_ts = mask_ts.to(device)

            x_img = uncond_model.ts_to_img(x_clean)
            mask_img = uncond_model.ts_to_img(mask_ts)
            mask_img = mask_img[:, :1, :, :]

            loss_ambient, _ = uncond_model.loss_fn_ambient_masked(
                x_img, mask_img, delta=delta)

            loss_standard, _ = uncond_model.loss_fn_irregular(x_img)

            loss = lambda_obs * loss_ambient + (1.0 - lambda_obs) * loss_standard
        ...
        if (epoch + 1) % 10 == 0 or epoch == 0:
            print(f"  Ambient M-step epoch {epoch+1}/{args.m_step_epochs}, loss: {avg_loss:.4f}")

        if logger is not None:
            global_step = em_iter * args.m_step_epochs + epoch
            logger.log('em/m_step_loss', avg_loss, global_step)
            logger.log('em/lambda_obs', lambda_obs, global_step)

    print(f"Ambient M-step complete for EM iteration {em_iter}.")
    return uncond_model
...
def main(args):
    """Main DiffEM-MMPS + Ambient M-step training loop."""
    ...
    name = f"diffem_mmps_ambient_mstep_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        # === Phase 1.5: Warmstart with ambient M-step ===
        print(f"\n{'='*60}")
        print("Phase 1.5: Initial training with ambient M-step")
        print(f"{'='*60}")

        uncond_model = m_step_ambient(
            args, uncond_model, optimizer,
            initial_reconstructions, obs_masks,
            em_iter=-1, total_em_iters=args.em_iters,
            device=args.device, logger=logger,
        )

        # === Phase 2: EM Loop with ambient M-step ===
        delta = getattr(args, 'further_corrupt_delta', 0.1)

        print(f"\n{'='*60}")
        print(f"Starting DiffEM-MMPS + Ambient M-step with {args.em_iters} EM iterations")
        print(f"  MMPS sigma_y: {args.mmps_sigma_y}")
        print(f"  MMPS cg_iters: {args.mmps_cg_iters}")
        print(f"  Further corruption delta: {delta}")
        print(f"  lambda_obs: {getattr(args, 'lambda_obs_start', 1.0)} -> "
              f"{getattr(args, 'lambda_obs_end', 0.3)}")
        print(f"{'='*60}")
        ...
        for em_iter in range(args.em_iters):
            ...
            uncond_model = m_step_ambient(
                ...
                em_iter=em_iter, total_em_iters=args.em_iters,
                device=args.device, logger=logger,
            )
        ...
        print("\n" + "="*60)
        print("DiffEM-MMPS + Ambient M-step Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS Ambient M-step training is complete")
