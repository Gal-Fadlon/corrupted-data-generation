"""
DiffEM with MMPS Bootstrap + Conditional EM — No Phase 3 (Approach A)

Eliminates the Phase 3 information bottleneck from run_diffem_cond_mmps_init.py.
Instead of training a brand-new unconditional model on final reconstructions,
this script continues refining the Phase 0 MMPS unconditional model on the
conditional EM reconstructions and evaluates that model directly.

Pipeline:
  Phase 0 — MMPS Bootstrap: identical to run_diffem_cond_mmps_init.py
  Phase 1 — Initial Conditional Training: identical
  Phase 2 — Conditional EM with Repaint Guidance: identical
  Phase 3 — REPLACED: continue training the Phase 0 uncond_model on final
            reconstructions via mmps_m_step + evaluate_uncond (no model reset)
"""
...
def main(args):
    ...
    name = f"diffem_cond_mmps_no_phase3_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        # ==================================================================
        # Phase 3 — Continue MMPS model on final reconstructions (NO reset)
        # ==================================================================
        phase3_epochs = getattr(args, 'uncond_epochs', 100)

        print(f"\n{'=' * 70}")
        print(f"PHASE 3: Continue MMPS unconditional model on final reconstructions "
              f"({phase3_epochs} epochs, no model reset)")
        print(f"{'=' * 70}")

        uncond_model = mmps_m_step(
            args, uncond_model, uncond_optimizer, reconstructions,
            em_iter=mmps_bootstrap_iters + cond_em_iters,
            device=args.device,
            m_step_epochs=phase3_epochs, logger=logger,
        )

        final_metrics = evaluate_uncond(
            args, uncond_model, test_loader,
            step=mmps_bootstrap_iters + cond_em_iters + 1,
        ...
        # ==================================================================
        print(f"\n{'=' * 70}")
        print("DiffEM MMPS Bootstrap + Cond EM — No Phase 3 — Complete")
        print(f"{'=' * 70}")
        if best_mmps_metrics:
            ...
            for k, v in best_mmps_metrics.items():
                print(f"  {k}: {v:.4f}")
        if final_metrics:
            print("Phase 3 (continued MMPS model) final metrics:")
