...
+ Curriculum + Ambient M-step with Annealing Delta

Same as kalman_freqgate_decomp_proj_ambient but the further-corruption
probability delta is annealed over EM iterations:

    Early EM (rough reconstructions):  delta = 0.3  (heavy regularisation)
    Late  EM (refined reconstructions): delta = 0.05 (preserves detail)

    delta(em_iter) = delta_start * (1 - progress) + delta_end * progress

This mirrors curriculum learning applied to the M-step: early iterations
need strong corruption to avoid memorising bad imputations; later iterations
can afford lighter corruption to preserve fine-grained structure.
"""
...
def m_step_ambient(args, uncond_model, optimizer, reconstructions, obs_masks,
    ...
    print(f"\n=== Ambient M-Step (EM iter {em_iter}, delta={delta:.3f}) ===")
    ...
    for epoch in range(args.m_step_epochs):
        ...
        if logger is not None:
            ...
            logger.log('em/ambient_delta', delta, em_iter)
...
def main(args):
    ...
    name = f"diffem_mmps_kalman_freqgate_decomp_proj_ambient_anneal_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print(f"\n{'='*60}")
        print(f"Starting Kalman + FreqGate + DecompProj + Ambient-Anneal DiffEM-MMPS")
        print(f"  EM iterations: {args.em_iters}")
        print(f"  Delta annealing: {DELTA_START} -> {DELTA_END}")
        print(f"{'='*60}")
        ...
        for em_iter in range(args.em_iters):
            ...
            # Anneal delta: high early (strong regularisation), low late (preserve detail)
            em_progress = min(1.0, (em_iter + 1) / args.em_iters)
            current_delta = DELTA_START * (1.0 - em_progress) + DELTA_END * em_progress
            print(f"  Ambient delta: {current_delta:.3f} (progress={em_progress:.2f})")

            uncond_model = m_step_ambient(
                args, uncond_model, optimizer,
                reconstructions, obs_masks,
                em_iter, args.device, delta=current_delta, logger=logger
            )
        ...
        print("\n" + "="*60)
        print("Kalman + FreqGate + DecompProj + Ambient-Anneal DiffEM-MMPS Complete!")
        print("="*60)
        if best_metrics:
