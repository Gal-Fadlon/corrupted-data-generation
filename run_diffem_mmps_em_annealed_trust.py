"""
DiffEM with MMPS E-step — EM-Annealed Observation Trust

sigma_obs_effective decreases across EM iterations.
  - Early EM: model doesn't trust noisy observations (large sigma_obs)
  - Late EM: trusts observations more (small sigma_obs)
  - When noise_level=0: sigma_obs is always 0 (unchanged)
...
# =============================================================================
# EM-Annealed Trust Decomposition-Projected MMPS
# =============================================================================

class EMAnnealedTrustMMPSDiffusionProcess:
    """
    MMPS sampler with EM-annealed observation trust and decomposition
    consistency projection.

    sigma_obs_effective decreases across EM iterations:
      - Early EM: large sigma_obs (don't trust noisy observations)
      - Late EM: small sigma_obs (trust observations more)
      - When noise_level=0: sigma_obs is always 0 (unchanged)
    """
    ...
    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        ...
        # Anneal: early EM -> large sigma_obs (don't trust noisy obs)
        # Late EM -> small sigma_obs (trust observations more)
        anneal_factor = max(0.01, 1.0 - self.em_progress)
        sigma_obs_effective = self.sigma_obs * anneal_factor
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    print(f"\n=== E-Step (EM iter {em_iter}) — EM-Annealed Trust MMPS ===")
    uncond_model.eval()
    ...
    print(f"  EM progress={em_progress:.2f}, anneal_factor={max(0.01, 1.0 - em_progress):.3f}")

    with uncond_model.ema_scope():
        process = EMAnnealedTrustMMPSDiffusionProcess(
            args, uncond_model.net, target_shape, uncond_model,
            sigma_y=sigma_y, cg_iters=cg_iters,
            projection_strength=0.5, sigma_threshold_frac=0.5,
            sigma_obs=sigma_obs, em_progress=em_progress
        )

        for start_idx in tqdm(range(0, N, batch_size), desc="E-step EM-Annealed Trust"):
...
def main(args):
    ...
    name = f"em_annealed_trust_mmps_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        if sigma_obs > 0:
            print(f"  EM-Annealed Trust ACTIVE (sigma_obs={sigma_obs})")
        else:
        ...
        print(f"\n{'='*60}")
        print(f"Starting EM-Annealed Trust DiffEM-MMPS with {args.em_iters} EM iterations")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("EM-Annealed Trust DiffEM-MMPS Complete!")
        print("="*60)
        if best_metrics:
