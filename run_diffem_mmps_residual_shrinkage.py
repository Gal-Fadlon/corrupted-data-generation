"""
DiffEM with MMPS E-step — Residual Shrinkage
...
Unified model that handles all corruption regimes:
  ...
  - Observation noise only (missing_rate=0): E-step actively denoises via residual shrinkage
  - Missing + noise: imputes and denoises in one pass

Key idea: After MMPS posterior denoiser, at observed positions compute residual
r = denoised - x_obs. Apply soft thresholding:
    r_shrunk = sign(r) * max(|r| - lambda*sigma_obs, 0)
Large corrections (structural) are preserved; small corrections (noise fitting)
are zeroed. When sigma_obs=0: threshold is 0, no shrinkage (unchanged).
...
# =============================================================================
# Residual Shrinkage Decomposition-Projected MMPS
# =============================================================================

class ResidualShrinkageMMPSDiffusionProcess:
    """
    MMPS sampler with residual shrinkage at observed positions and decomposition
    ...
    When sigma_obs>0, after the standard MMPS CG solve, residual shrinkage is
    applied at observed positions: r = denoised - x_obs is soft-thresholded so
    that small corrections (likely noise fitting) are zeroed out while large
    corrections (structural) are preserved.
    ...
    def __init__(self, args, diffusion_fn, shape, model,
                 ...
                 sigma_obs=0.0, shrinkage_lambda=1.5):
        self.args = args
    ...
    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        ...
        # Residual shrinkage at observed positions
        if self.sigma_obs > 0:
            threshold = self.shrinkage_lambda * self.sigma_obs
            residual = result - x_obs  # what MMPS changed from observation
            # Soft thresholding: zero out small changes (likely noise fitting)
            shrunk = torch.sign(residual) * torch.clamp(torch.abs(residual) - threshold, min=0)
            result = mask * (x_obs + shrunk) + (1 - mask) * result
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    print(f"\n=== E-Step (EM iter {em_iter}) — Residual Shrinkage MMPS ===")
    uncond_model.eval()
    ...
    with uncond_model.ema_scope():
        process = ResidualShrinkageMMPSDiffusionProcess(
        ...
        for start_idx in tqdm(range(0, N, batch_size), desc="E-step Residual Shrinkage"):
...
def main(args):
    ...
    name = f"residual_shrinkage_mmps_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        if sigma_obs > 0:
            print(f"  Residual shrinkage ACTIVE (sigma_obs={sigma_obs})")
        else:
        ...
        print(f"\n{'='*60}")
        print(f"Starting Residual Shrinkage DiffEM-MMPS with {args.em_iters} EM iterations")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("Residual Shrinkage DiffEM-MMPS Complete!")
        print("="*60)
        if best_metrics:
