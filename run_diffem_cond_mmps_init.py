"""
DiffEM with MMPS Bootstrap + Conditional EM + Repaint Guidance

Implements the reference DiffEM paper's pipeline adapted for time series:

  Phase 0 — MMPS Bootstrap (like Lap 0 in the paper):
    Train an unconditional model with Gaussian init, then run 3 MMPS EM
    iterations to produce high-quality initial reconstructions.

  Phase 1 — Initial Conditional Training:
    Build a conditional model and train it on the MMPS reconstructions
    using FreshCorruptionDataset for diverse corruption patterns.

  Phase 2 — Conditional EM with Repaint Guidance:
    Alternate conditional E-step (with repaint-style observation enforcement
    at every reverse diffusion step, 256 steps) and M-step (FreshCorruption,
    200 epochs, persistent EMA across laps).

  Phase 3 — Unconditional Evaluation:
    Train a fresh unconditional model on the final reconstructions using
    the same protocol as run_regular.py for fair comparison.

Key improvements over run_diffem_pure_combined.py:
  1. MMPS bootstrap for Lap 0 (solves cold-start problem)
  2. 256 E-step sampling steps (was 64)
  3. No EMA reset between conditional EM laps
  4. Repaint guidance: per-step observation enforcement during conditional sampling
  5. 200 M-step epochs per lap (was 100)
"""
...
def main(args):
    ...
    name = f"diffem_cond_mmps_init_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        # ==================================================================
        # Phase 3 — Unconditional Evaluation (same as run_regular.py)
        # ==================================================================
        print(f"\n{'=' * 70}")
        print("PHASE 3: Unconditional Model Training (run_regular.py protocol)")
        print(f"{'=' * 70}")
        ...
        # ==================================================================
        print(f"\n{'=' * 70}")
        print("DiffEM with MMPS Bootstrap + Repaint — Complete")
        print(f"{'=' * 70}")
        if best_mmps_metrics:
