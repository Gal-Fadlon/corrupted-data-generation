"""
DiffEM with Unconditional Model + RePaint E-step

This script implements EM with diffusion priors using a SINGLE unconditional model:
- E-step: RePaint-style imputation (replace observed pixels with forward-noised observations
  at each reverse step, let the model freely denoise missing pixels)
- M-step: Train the same unconditional model on fully imputed data

Key difference from run_diffem_pure.py:
- ONE model does both imputation and generation (no separate conditional model)
- E-step uses RePaint instead of conditional sampling

RePaint core idea: At each reverse diffusion step, replace observed pixels with
forward-noised versions of the actual observations, while letting the model freely
denoise missing pixels. Optional jump-back resampling improves boundary harmonization.
"""
...
# =============================================================================
# RepaintDiffusionProcess
# =============================================================================

class RepaintDiffusionProcess:
    """
    RePaint-style diffusion sampler for inpainting with an unconditional model.

    Uses the same EDM/Karras sigma schedule as DiffusionProcess. At each reverse
    step, observed pixels are replaced with forward-noised observations while
    missing pixels are freely denoised by the model.

    EDM forward noise: x_noisy = x_clean + sigma * epsilon
    Jump-back re-noise: x_back = x + sqrt(t_cur^2 - t_next^2) * epsilon
    """

    def __init__(self, args, diffusion_fn, shape, n_resample=1, jump_length=10):
        self.args = args
    ...
    def repaint_sample(self, latents, x_obs_img, mask_img):
        """
        RePaint sampling: inpaint missing regions using an unconditional model.

        At each denoising step (t_cur -> t_next):
        1. Run standard EDM Euler step + 2nd-order correction
        2. Replace observed pixels: x_next = mask * (x_obs + t_next * noise) + (1 - mask) * x_next
        3. At final step (t_next=0): use clean observed values (no noise)
        4. Optional jump-back resampling for better boundary harmonization
        ...
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):
            ...
            for r in range(self.n_resample):
                ...
                # Apply 2nd order correction (except at last step)
                if i < self.num_steps - 1:
                    denoised = self.net(x_next, t_next, None).to(torch.float64)
                    d_prime = (x_next - denoised) / t_next
                    x_next = x_hat + (t_next - t_hat) * (0.5 * d_cur + 0.5 * d_prime)

                # RePaint replacement: merge observed and denoised
                if t_next > 0:
                    # Forward-noise the observed data to noise level t_next
                    noise = torch.randn_like(x_obs)
                    x_obs_noisy = x_obs + t_next * noise
                    x_next = mask * x_obs_noisy + (1 - mask) * x_next
                else:
                    # Final step: use clean observed values
                    x_next = mask * x_obs + (1 - mask) * x_next

                # Jump-back resampling: re-noise and repeat (except on last resample iteration)
                if r < self.n_resample - 1 and t_next > 0:
                    # Re-noise x_next back to t_cur level
                    # x_back = x + sqrt(t_cur^2 - t_next^2) * noise
                    jump_noise = torch.randn_like(x_next)
                    x_next = x_next + (t_cur ** 2 - t_next ** 2).sqrt() * jump_noise

        return x_next

    @torch.no_grad()
    def sampling_repaint(self, x_obs_img, mask_img):
        """
        Convenience wrapper for RePaint sampling.
        ...
        return self.repaint_sample(latents, x_obs_img, mask_img)
...
# =============================================================================
# Helper functions (from run_diffem_pure.py)
# =============================================================================
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    """
    E-step: Impute missing values using RePaint with the unconditional model.

    Uses RepaintDiffusionProcess: at each reverse step, observed pixels are replaced
    with forward-noised observations while missing pixels are freely denoised.
    ...
    """
    print(f"\n=== E-Step (EM iter {em_iter}) — RePaint Imputation ===")
    print("Imputing missing values via RePaint with unconditional model...")
    ...
    n_resample = getattr(args, 'repaint_n_resample', 1)
    jump_length = getattr(args, 'repaint_jump_length', 10)

    with torch.no_grad():
        with uncond_model.ema_scope():
            process = RepaintDiffusionProcess(
                args, uncond_model.net, target_shape,
                n_resample=n_resample, jump_length=jump_length
            )

            for start_idx in tqdm(range(0, N, batch_size), desc="E-step RePaint"):
                ...
                # This broadcasts correctly over all channels
                mask_ts_expanded = mask_ts.unsqueeze(-1).expand(-1, -1, corrupted_ts.shape[-1])
                mask_img = uncond_model.ts_to_img(mask_ts_expanded)
                mask_img = mask_img[:, :1, :, :]  # Take first channel for mask (broadcasts over C)

                # RePaint sampling
                x_img_imputed = process.sampling_repaint(x_obs_img, mask_img)
...
def main(args):
    """Main DiffEM-Uncond training loop with RePaint E-step."""

    # Model name and directory
    name = create_model_name_and_dir(args)
    name = f"diffem_uncond_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        # === Phase 2: EM Loop ===
        print(f"\n{'='*60}")
        print(f"Starting DiffEM-Uncond with {args.em_iters} EM iterations")
        print(f"  RePaint n_resample: {args.repaint_n_resample}")
        print(f"  RePaint jump_length: {args.repaint_jump_length}")
        print(f"{'='*60}")
        ...
        for em_iter in range(args.em_iters):
            ...
            # E-step: RePaint imputation with current model
        ...
        if final_metrics:
            ...
            for k, v in final_metrics.items():
                print(f"  {k}: {v:.4f}")
        print("\n" + "="*60)
        print("DiffEM-Uncond Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-Uncond training is complete")
