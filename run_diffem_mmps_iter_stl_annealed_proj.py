"""
DiffEM with MMPS E-step — Iterative-STL Init + EM-Annealed Projection Strength

Combines iterative STL initialization with an adaptive projection schedule:
  The decomposition projection strength starts high (0.7) in early EM
  iterations — when the model is poor and structural priors are valuable —
  and decays to low (0.15) in later iterations, letting the model express
  fine details once it has learned the distribution.

  Within each diffusion step, the sigma-dependent scaling still applies
  (strong at high noise, vanishes at low noise), so the annealing is a
  second, outer schedule on top of the existing inner schedule.

Based on run_diffem_mmps_iter_stl_decomp_projection.py with EM-iteration-
dependent projection_strength.
"""
...
PROJ_STRENGTH_START = 0.7
PROJ_STRENGTH_END = 0.15
...
def compute_annealed_projection_strength(em_iter, total_em_iters):
    """Linearly anneal projection strength from PROJ_STRENGTH_START to PROJ_STRENGTH_END."""
    progress = em_iter / max(1, total_em_iters - 1)
    return PROJ_STRENGTH_START + progress * (PROJ_STRENGTH_END - PROJ_STRENGTH_START)
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device,
           projection_strength, logger=None):
    """E-step with EM-annealed decomposition projection."""
    print(f"\n=== E-Step (EM iter {em_iter}) — Annealed Decomp-Projected MMPS "
          f"(proj_strength={projection_strength:.3f}) ===")
    uncond_model.eval()
    ...
    with uncond_model.ema_scope():
        process = DecompProjectedMMPSDiffusionProcess(
            ...
            projection_strength=projection_strength, sigma_threshold_frac=0.5
        )

        for start_idx in tqdm(range(0, N, batch_size), desc="E-step Annealed-Proj"):
    ...
    if logger is not None:
        ...
        logger.log('em/projection_strength', projection_strength, em_iter)
...
def main(args):
    ...
    name = f"diffem_mmps_iter_stl_annealed_proj_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print(f"\n{'='*60}")
        print(f"Starting Annealed-Projection DiffEM-MMPS with {args.em_iters} EM iterations")
        print(f"  Projection strength: {PROJ_STRENGTH_START:.2f} → {PROJ_STRENGTH_END:.2f}")
        print(f"{'='*60}")
        ...
        for em_iter in range(args.em_iters):
            proj_strength = compute_annealed_projection_strength(em_iter, args.em_iters)

            print(f"\n{'='*60}")
            print(f"EM Iteration {em_iter + 1}/{args.em_iters}  "
                  f"(proj_strength={proj_strength:.3f})")
            print(f"{'='*60}")
            ...
            reconstructions = e_step(
                ...
                em_iter, args.device, proj_strength, logger
            )
        ...
        print("\n" + "="*60)
        print("Annealed-Projection Iter-STL DiffEM-MMPS Complete!")
        print(f"  Projection annealed: {PROJ_STRENGTH_START:.2f} → {PROJ_STRENGTH_END:.2f}")
        print("="*60)
        if best_metrics:
