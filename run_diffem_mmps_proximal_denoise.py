"""
DiffEM with MMPS E-step — Proximal Denoising Step (RED-diff inspired)
...
Unified model that handles all corruption regimes:
  ...
  - Observation noise only (missing_rate=0): E-step actively denoises via proximal step
  - Missing + noise: imputes and denoises in one pass

Key idea: After the MMPS correction at each reverse step, apply a proximal denoising
regularization: run the denoiser D_theta at the observation noise level sigma_obs on
the MMPS result, then blend.

    lambda(sigma_t) = sigma_obs^2 / sigma_t^2

so the regularization is strong when the model is confident (low sigma_t) and vanishes
at high sigma_t. When sigma_obs=0: lambda=0, no regularization (unchanged baseline).

KEY ANISOTROPIC NOISE FIX: Uses per-feature noise std (computed from data) instead of
scalar sigma_obs. The CG operator accounts for per-channel observation noise variance,
and the proximal step uses the mean per-channel sigma_obs for the denoiser call.
...
# =============================================================================
# Proximal Denoising Decomposition-Projected MMPS
# =============================================================================

class ProximalDenoiseMMPSDiffusionProcess:
    """
    MMPS sampler with proximal denoising regularization (RED-diff inspired)
    and decomposition consistency projection.

    When sigma_obs=0, this is identical to DecompProjectedMMPSDiffusionProcess
    (standard MMPS with hard observation matching).

    When sigma_obs>0, after the standard MMPS CG solve at each reverse diffusion
    step, a proximal denoising step is applied:
      1. Run the denoiser D_theta at sigma_obs on the MMPS result
      2. Blend with strength lambda_t = sigma_obs^2 / sigma_t^2

    This means:
      - At high sigma_t (early): lambda_t is small, proximal step barely changes result
      - At low sigma_t (late):   lambda_t is large, proximal step strongly regularizes

    Supports anisotropic (per-channel) observation noise via sigma_obs_per_channel.
    """

    def __init__(self, args, diffusion_fn, shape, model,
                 sigma_y=0.01, cg_iters=1,
                 projection_strength=0.5, sigma_threshold_frac=0.5,
                 sigma_obs=0.0, sigma_obs_per_channel=None):
        self.args = args
    ...
    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        """
        Proximal denoising posterior denoiser.

        When sigma_obs=0: standard MMPS (no proximal step).
        When sigma_obs>0: standard MMPS CG solve with per-channel noise,
        followed by a proximal denoising step that runs D_theta at sigma_obs
        on the MMPS result and blends with strength lambda_t = sigma_obs^2 / sigma_t^2.
        """
        sigma_sq = sigma ** 2
        ...
        # Proximal denoising step (RED-diff inspired)
        if self.sigma_obs > 0 and float(sigma) > self.sigma_obs:
            # Apply denoiser at observation noise level as regularizer
            # lambda_t: strong when model confident (low sigma_t), weak at high sigma_t
            mean_sigma_obs = self.sigma_obs_per_channel.mean().item() if self.sigma_obs_per_channel is not None else self.sigma_obs
            lambda_t = mean_sigma_obs ** 2 / (float(sigma) ** 2)
            # Run denoiser at sigma_obs level on the MMPS result
            sigma_obs_tensor = torch.tensor([mean_sigma_obs], device=x_t.device, dtype=torch.float64).reshape(1,1,1,1)
            with torch.no_grad():
                prox_denoised = self.net(result.float(), sigma_obs_tensor.float(), None).to(torch.float64)
            # Blend: result + lambda_t * (prox_denoised - result)
            result = result + lambda_t * (prox_denoised - result)
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    print(f"\n=== E-Step (EM iter {em_iter}) — Proximal-Denoise MMPS ===")
    uncond_model.eval()
    ...
    # Compute per-feature noise std (anisotropic noise)
    sigma_obs_per_channel = None
    if sigma_obs > 0:
        per_feat_std = compute_per_feature_noise_std(corrupted_data, sigma_obs)
        sigma_obs_per_channel = torch.tensor(per_feat_std, dtype=torch.float32)
        print(f"  Per-feature noise std: min={per_feat_std.min():.4f}, "
              f"max={per_feat_std.max():.4f}, mean={per_feat_std.mean():.4f}")
        print(f"  Scalar sigma_obs={sigma_obs}, using anisotropic per-channel noise")
    else:
        print(f"  sigma_obs=0, proximal denoising disabled (standard MMPS)")

    with uncond_model.ema_scope():
        process = ProximalDenoiseMMPSDiffusionProcess(
            args, uncond_model.net, target_shape, uncond_model,
            sigma_y=sigma_y, cg_iters=cg_iters,
            projection_strength=0.5, sigma_threshold_frac=0.5,
            sigma_obs=sigma_obs, sigma_obs_per_channel=sigma_obs_per_channel
        )

        for start_idx in tqdm(range(0, N, batch_size), desc="E-step Proximal-Denoise"):
...
def main(args):
    ...
    name = f"proximal_denoise_mmps_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        if sigma_obs > 0:
            per_feat_std = compute_per_feature_noise_std(corrupted_data, sigma_obs)
            print(f"  Proximal denoising ACTIVE (sigma_obs={sigma_obs})")
            print(f"  Per-feature noise std: min={per_feat_std.min():.4f}, "
                  f"max={per_feat_std.max():.4f}, mean={per_feat_std.mean():.4f}")
        else:
        ...
        print(f"\n{'='*60}")
        print(f"Starting Proximal-Denoise DiffEM-MMPS with {args.em_iters} EM iterations")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("Proximal-Denoise DiffEM-MMPS Complete!")
        print("="*60)
        if best_metrics:
