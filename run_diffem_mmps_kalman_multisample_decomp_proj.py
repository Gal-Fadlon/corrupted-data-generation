"""
DiffEM with MMPS E-step — Kalman Init + Multi-Sample Averaging + Decomposition Projection

Combines Kalman smoother initialization with a variance-reduced, structurally
projected E-step:
  1. Kalman smoother initialization — optimal linear state-space imputation
     via UnobservedComponents that handles NaN natively.
  2. Multi-sample E-step (K=3) — draws K independent MMPS samples and averages
     them, better approximating the true E-step expectation E[x_0 | x_obs, theta].
  3. Decomposition-projected MMPS — each sample is projected through
     decomposition-recomposition for structural consistency.
  4. Curriculum missing rate — early EM iterations reveal extra observations.

Based on run_diffem_mmps_iter_stl_multisample.py with init swapped to
...
def main(args):
    ...
    name = f"diffem_mmps_kalman_multisample_decomp_proj_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print(f"\n{'='*60}")
        print(f"Starting Kalman + {n_samples}-Sample Averaged Decomp-Projected DiffEM-MMPS with {args.em_iters} EM iterations")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print(f"Kalman + {n_samples}-Sample Averaged Decomp-Projected DiffEM-MMPS Complete!")
        print("="*60)
        if best_metrics:
