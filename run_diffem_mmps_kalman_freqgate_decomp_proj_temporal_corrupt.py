...
+ Curriculum + Temporal Block Ambient M-step

Instead of the i.i.d. pixel-wise further corruption of standard Ambient Diffusion,
this variant drops *contiguous temporal blocks* during M-step training.  Because
time series have strong temporal autocorrelation, i.i.d. pixel dropout is easy to
interpolate and provides weak regularisation.  Block dropout forces the model to
learn longer-range temporal dependencies and produce coherent structure.

Analogous to the "block inpainting" corruption in Ambient Diffusion (Section 3.1).
"""
...
# =============================================================================
# Temporal block mask generation
# =============================================================================

def generate_temporal_block_mask(batch_size, seq_len, obs_mask_ts,
                                 block_prob=0.1, block_width=4, device='cpu'):
    """Generate temporal block further-corruption masks.

    For each sample, randomly select ~block_prob fraction of *observed*
    timesteps as block centers, then mask out windows of block_width around
    each center.  The mask B is 1 = keep, 0 = further-corrupted.

    Args:
        batch_size: number of samples
        seq_len: sequence length
        obs_mask_ts: (B, T) float tensor, 1 = observed, 0 = missing
        block_prob: probability that an observed timestep becomes a block center
        block_width: width of each masked block
        device: torch device

    Returns:
        B_mask_ts: (B, T) float tensor, 1 = keep, 0 = further-corrupted
    """
    half_w = block_width // 2
    centers = (torch.rand(batch_size, seq_len, device=device) < block_prob).float()
    centers = centers * obs_mask_ts

    block_drop = torch.zeros(batch_size, seq_len, device=device)
    for offset in range(-half_w, half_w + 1):
        shifted = torch.roll(centers, shifts=offset, dims=1)
        if offset < 0:
            shifted[:, offset:] = 0
        elif offset > 0:
            shifted[:, :offset] = 0
        block_drop = torch.clamp(block_drop + shifted, 0, 1)

    B_mask_ts = 1.0 - block_drop * obs_mask_ts
    return B_mask_ts
...
# =============================================================================
# Freq-Gated + Decomp-Projected MMPSDiffusionProcess
# =============================================================================

class FreqGatedDecompProjectedMMPSDiffusionProcess:
    ...
    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        ...
        mask_m = mask[:, :1].to(dtype=x_data.dtype)
        x_t_input = concat_mask_channel(x_data, mask_m, self._ambient_concat)
        ...
        def vjp_fn(cotangent):
            ...
            if self._ambient_concat:
                grad = grad[:, : self._data_c, :, :]
            return grad
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    ...
    with uncond_model.ema_scope():
        ...
        for start_idx in tqdm(range(0, N, batch_size), desc="E-step FreqGate+DecompProj"):
...
def m_step_temporal_block_ambient(args, uncond_model, optimizer, reconstructions,
                                  obs_masks, em_iter, device,
                                  block_prob=0.1, block_width=4, logger=None):
    """Ambient M-step with temporal block further corruption.

    Instead of i.i.d. pixel dropout, we drop contiguous blocks of timesteps
    from observed positions.  The model sees the further-corrupted data but
    is evaluated on all originally-observed positions.
    """
    print(f"\n=== Temporal-Block Ambient M-Step (EM iter {em_iter}) ===")
    print(f"  block_prob={block_prob}, block_width={block_width}")
    ...
    for epoch in range(args.m_step_epochs):
        ...
        for x_clean, mask_batch in recon_loader:
            ...
            B_cur = x_clean.shape[0]
            seq_len = x_clean.shape[1]
            n_features = x_clean.shape[2]

            x_img = uncond_model.ts_to_img(x_clean)

            mask_expanded = mask_batch.unsqueeze(-1).expand(-1, -1, n_features)
            mask_img = uncond_model.ts_to_img(mask_expanded)[:, :1, :, :]

            if mask_img.shape[1] == 1 and x_img.shape[1] > 1:
                mask_img_full = mask_img.expand_as(x_img)
            else:
                mask_img_full = mask_img

            B_ts = generate_temporal_block_mask(
                B_cur, seq_len, mask_batch,
                block_prob=block_prob, block_width=block_width,
                device=device
            )
            B_ts_expanded = B_ts.unsqueeze(-1).expand(-1, -1, n_features)
            B_img = uncond_model.ts_to_img(B_ts_expanded)[:, :1, :, :]
            if B_img.shape[1] == 1 and x_img.shape[1] > 1:
                B_img = B_img.expand_as(x_img)

            further_mask = mask_img_full * B_img

            rnd_normal = torch.randn([B_cur, 1, 1, 1], device=device)
            sigma = (rnd_normal * uncond_model.P_std + uncond_model.P_mean).exp()
            edm_weight = (sigma ** 2 + uncond_model.sigma_data ** 2) / (sigma * uncond_model.sigma_data) ** 2

            noise = torch.randn_like(x_img) * sigma
            x_noisy = x_img + noise
            x_data = x_noisy * further_mask
            x_input = concat_mask_channel(
                x_data, further_mask[:, :1, :, :],
                uncond_model.ambient_concat_further_mask,
            )

            D_x = uncond_model.net(x_input, sigma, None)

            x_unpad = uncond_model.unpad(x_img * mask_img_full, x_img.shape)
            D_unpad = uncond_model.unpad(D_x * mask_img_full, x_img.shape)

            n_observed = mask_img_full.sum() + 1e-8
    ...
    print(f"Temporal-block ambient M-step complete for EM iteration {em_iter}.")
    return uncond_model


def m_step_vanilla(args, uncond_model, optimizer, reconstructions, em_iter, device, logger=None):
    """Standard M-step for Phase 1.5."""
...
def main(args):
    ...
    name = f"diffem_mmps_kalman_freqgate_decomp_proj_temporal_corrupt_{name}"
    logging.info(args)
    # Align M-step (block ambient), E-step MMPS, and eval sampling (C+1 UNet).
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print(f"\n{'='*60}")
        print(f"Starting Kalman + FreqGate + DecompProj + Temporal-Block-Ambient DiffEM-MMPS")
        print(f"  EM iterations: {args.em_iters}")
        print(f"  Block prob: 0.1, Block width: 4")
        print(f"{'='*60}")
        ...
        for em_iter in range(args.em_iters):
            ...
            uncond_model = m_step_temporal_block_ambient(
                args, uncond_model, optimizer,
                reconstructions, obs_masks,
                em_iter, args.device,
                block_prob=0.1, block_width=4, logger=logger
            )
        ...
        if final_metrics:
            ...
            for k, v in final_metrics.items():
                print(f"  {k}: {v:.4f}")
        print("\n" + "="*60)
        print("Kalman + FreqGate + DecompProj + Temporal-Block-Ambient DiffEM-MMPS Complete!")
        print("="*60)
        if best_metrics:
