"""
DiffEM with Unconditional Model + MMPS E-step — Fourier Loss variant

Same as run_diffem_mmps.py with ONE change:
  An additional Fourier magnitude loss term is added to the M-step, encouraging
  the model to match frequency-domain characteristics of the training data
  (inspired by Diffusion-TS).
"""
...
def m_step(args, uncond_model, optimizer, reconstructions, em_iter, device, logger=None):
    ...
    for epoch in range(args.m_step_epochs):
        ...
        for batch_idx, (x_clean,) in enumerate(recon_loader):
            ...
            # Compute mask (all ones since reconstructions have no NaN)
            mask = torch.isnan(x_img).float() * -1 + 1
            x_img_clean = torch.nan_to_num(x_img, nan=0.0)

            # Get denoised output from the network
            denoised_img, weight = uncond_model.forward_irregular(x_img_clean, mask)

            # Standard Karras diffusion loss (same as loss_fn_irregular)
            x_unpad = uncond_model.unpad(x_img_clean * mask, x_img_clean.shape)
            d_unpad = uncond_model.unpad(denoised_img * mask, x_unpad.shape)
            loss = (weight * (d_unpad - x_unpad).square()).mean()
            to_log = {'karras loss': loss.detach().item()}

            # Fourier magnitude loss on denoised output in time-series domain
            lambda_fft = 0.1
            fft_target = torch.fft.rfft(x_clean, dim=1)
            denoised_ts = uncond_model.img_to_ts(denoised_img)
            fft_denoised = torch.fft.rfft(denoised_ts, dim=1)
            loss_fft = (fft_denoised.abs() - fft_target.abs()).square().mean()
            loss = loss + lambda_fft * loss_fft
            to_log['fft_loss'] = loss_fft.detach().item()
...
def main(args):
    ...
    name = f"diffem_mmps_fft_loss_{name}"
