"""
DiffEM with MMPS E-step — Kalman Init + Frequency-Gated + Decomposition Projection + Curriculum

Combines four orthogonal improvements:
  ...
  2. Frequency-gated MMPS E-step — observation guidance residual is low-pass
     filtered based on sigma. High-frequency observation details are only
     injected at low noise levels where coarse structure has settled.
  3. Decomposition projection — after each posterior correction, the result
     is projected through decomposition-recomposition to enforce structural
     constraints. Projection strength is sigma-dependent.
  4. Curriculum missing rate — early EM iterations reveal extra observations
     to make the problem easier, gradually increasing to full difficulty.

Combines run_diffem_mmps_freq_gated.py and
run_diffem_mmps_kalman_curriculum_decomp_proj.py.
"""
...
class FreqGatedDecompProjectedMMPSDiffusionProcess:
    """
    MMPS sampler combining frequency-gated guidance with decomposition projection.

    Two complementary modifications to the posterior denoiser:
    1. The residual r = x_obs - mask * denoised is low-pass filtered based on
       sigma before the CG solve, ensuring high-frequency observation details
       are only injected when the model has settled coarse structure.
    2. After the posterior correction, the result is projected through
       decomposition-recomposition to enforce structural consistency.
    """
...
def main(args):
    ...
    name = f"diffem_mmps_kalman_freqgate_decomp_proj_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print(f"\n{'='*60}")
        print(f"Starting Kalman + FreqGate + DecompProj DiffEM-MMPS with {args.em_iters} EM iterations")
        print(f"{'='*60}")
        ...
        if final_metrics:
            ...
            for k, v in final_metrics.items():
                print(f"  {k}: {v:.4f}")
        print("\n" + "="*60)
        print("Kalman + FreqGate + DecompProj DiffEM-MMPS Complete!")
        print("="*60)
        if best_metrics:
