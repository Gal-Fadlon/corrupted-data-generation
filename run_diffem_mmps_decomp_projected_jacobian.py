"""
DiffEM with MMPS E-step — Decomposition-Projected Jacobian

Projects the Jacobian correction onto a decomposition basis so that
the VJP output can adjust trend and seasonality but not inject
arbitrary high-frequency artifacts.

Decomposition weights control how much of each component is retained:
  - trend_weight (default 1.0): full trend correction
  - seasonal_weight (default 0.8): mostly retain seasonal correction
  - residual_weight (default 0.1): heavily attenuate residual/noise correction
"""
...
# =============================================================================
# MMPSDiffusionProcess — Decomposition-Projected Jacobian
# =============================================================================

class MMPSDiffusionProcess:
    """
    MMPS-style diffusion sampler with decomposition-projected Jacobian.

    The Jacobian correction (VJP output) is decomposed into trend, seasonal,
    and residual components. Each component is reweighted before being used
    as the posterior correction, ensuring the correction respects valid
    time-series structure.

    Trend is extracted via moving average along W axis (box kernel).
    Seasonal = signal - trend.  Residual is implicit (seasonal includes it
    unless further period-averaging is done; here we use a simple 2-level split).
    """

    def __init__(self, args, diffusion_fn, shape, sigma_y=0.01, cg_iters=1,
                 trend_weight=1.0, seasonal_weight=0.8, residual_weight=0.1,
                 decomp_period=None):
        self.args = args
        ...
        # Decomposition projection parameters
        self.trend_weight = trend_weight
        self.seasonal_weight = seasonal_weight
        self.residual_weight = residual_weight
        self.decomp_period = decomp_period

    def _extract_trend(self, x, period):
        """
        Extract trend via moving average along W axis.
        ...
        Returns:
            trend: tensor (B, C, H, W) — smoothed trend component
        """
        kernel = torch.ones(1, 1, 1, period, device=x.device, dtype=x.dtype) / period
        pad = period // 2
        B, C, H, W = x.shape
        x_reshaped = x.reshape(B * C, 1, H, W)
        trend = F.pad(x_reshaped, (pad, pad, 0, 0), mode='reflect')
        trend = F.conv2d(trend, kernel, groups=1)
        # Handle potential size mismatch from even period
        if trend.shape[-1] != W:
            trend = trend[..., :W]
        return trend.reshape(B, C, H, W)

    def _decompose_and_reweight(self, score, period):
        """
        Decompose score into trend/seasonal/residual and reweight.

        Args:
            score: tensor (B, C, H, W) — the Jacobian correction
            period: decomposition period

        Returns:
            score_projected: reweighted score (B, C, H, W)
        """
        # Extract trend via moving average
        trend = self._extract_trend(score, period)

        # Seasonal + residual = score - trend
        seasonal_plus_residual = score - trend

        # Extract seasonal by applying a second, smaller moving average to get
        # the slowly-varying part of seasonal_plus_residual (which is residual-like)
        # Then seasonal = seasonal_plus_residual - residual
        residual_window = max(3, period // 4)
        residual_smooth = self._extract_trend(seasonal_plus_residual, residual_window)
        seasonal = seasonal_plus_residual - residual_smooth
        residual = residual_smooth

        # Reweight components
        score_projected = (self.trend_weight * trend +
    ...
    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        """
        Posterior denoiser with decomposition-projected Jacobian correction.

        Same as standard MMPS, but the final score (VJP output) is decomposed
        into trend/seasonal/residual and reweighted before applying the
        ...
        W = x_t.shape[-1]
        period = self.decomp_period if self.decomp_period is not None else max(3, W // 4)
        # Ensure period is odd for symmetric padding
        if period % 2 == 0:
            period += 1
        ...
        # CG linear operator: M(v) = sigma_y^2 * v + sigma^2 * mask * J^T(mask * v)
        def cg_operator(v):
            return sigma_y_sq * v + sigma_sq * mask * vjp_fn(mask * v)

        # Solve (sigma_y^2 I + sigma^2 * mask * J^T * mask) * v = r
        v = conjugate_gradient(cg_operator, r, maxiter=self.cg_iters)

        # Compute posterior correction: score = J^T(mask * v)
        score = vjp_fn(mask * v)

        # Project score onto decomposition basis with reweighting
        score_projected = self._decompose_and_reweight(score, period)

        # Detach from computation graph
        result = denoised.detach() + sigma_sq * score_projected.detach()
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    ...
    Uses MMPSDiffusionProcess with decomposition-projected Jacobian.
    ...
    """
    print(f"\n=== E-Step (EM iter {em_iter}) — MMPS Imputation (Decomp-Projected Jacobian) ===")
    print("Imputing missing values via MMPS with decomposition-projected Jacobian...")
    ...
    trend_weight = getattr(args, 'trend_weight', 1.0)
    seasonal_weight = getattr(args, 'seasonal_weight', 0.8)
    residual_weight = getattr(args, 'residual_weight', 0.1)
    decomp_period = getattr(args, 'decomp_period', None)
    ...
    with uncond_model.ema_scope():
        process = MMPSDiffusionProcess(
            ...
            trend_weight=trend_weight, seasonal_weight=seasonal_weight,
            residual_weight=residual_weight, decomp_period=decomp_period
        )
...
def main(args):
    """Main DiffEM-Uncond training loop with MMPS E-step (Decomp-Projected Jacobian)."""

    # Model name and directory
    name = create_model_name_and_dir(args)
    name = f"diffem_mmps_decomp_projected_jacobian_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print(f"\n{'='*60}")
        print(f"Starting DiffEM-MMPS + Decomp-Projected Jacobian with {args.em_iters} EM iterations")
        print(f"  MMPS sigma_y: {args.mmps_sigma_y}")
        print(f"  MMPS cg_iters: {args.mmps_cg_iters}")
        print(f"  Trend weight: {args.trend_weight}, Seasonal weight: {args.seasonal_weight}, Residual weight: {args.residual_weight}")
        print(f"  Decomp period: {args.decomp_period}")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("DiffEM-MMPS + Decomp-Projected Jacobian Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS + Decomp-Projected Jacobian training is complete")
...
if __name__ == '__main__':
    ...
    # Decomposition-projected Jacobian parameters
    if not hasattr(args, 'trend_weight'):
        args.trend_weight = 1.0
    if not hasattr(args, 'seasonal_weight'):
        args.seasonal_weight = 0.8
    if not hasattr(args, 'residual_weight'):
        args.residual_weight = 0.1
    if not hasattr(args, 'decomp_period'):
        args.decomp_period = None
