"""
DiffEM with MMPS E-step — Iterative-STL Init + Decomposition Projection + Masked Loss

Same as stl_decomp_proj_curriculum but with masked M-step loss (like run_irregular.py):
  - The UNet sees the full E-step reconstruction (imputed + observed)
  - But noise is only added on observed positions
  - And loss is only computed on observed positions
  - Imputed values act as free conditioning context, not training signal

This avoids training on potentially wrong imputation artifacts while still
letting the UNet use them as structural context.

Also combines:
  1. Iterative STL initialization (best init method) — gives the model a
     high-quality warm start by iteratively refining STL decomposition.
  2. Decomposition-projected MMPS E-step (best E-step variant) — after each
     posterior denoising step, the estimate is projected through a
     decomposition-recomposition that enforces structural constraints.
     Projection strength is sigma-dependent (strong at high noise, vanishes
     at low noise).

Based on run_diffem_mmps_decomp_projection.py with init swapped to
...
def m_step(args, uncond_model, optimizer, reconstructions, corrupted_data, em_iter, device, logger=None):
    """
    M-step with masked loss (like run_irregular.py).

    The UNet sees the full E-step reconstruction but:
      - Noise is only added on originally observed positions
      - Loss is only computed on originally observed positions
      - Imputed values act as conditioning context only

    The mask is derived from NaN in the original corrupted data converted to
    image space — same approach as run_irregular.py (line 216-217), ensuring
    pixel-accurate masking through the delay embedding.
    """
    print(f"\n=== M-Step (EM iter {em_iter}) — Masked Loss ===")
    print(f"Training unconditional model for {args.m_step_epochs} epochs...")

    # Create dataset with both reconstructions and original corrupted data (with NaN)
    recon_tensor = torch.tensor(reconstructions, dtype=torch.float32)
    corrupted_tensor = torch.tensor(corrupted_data, dtype=torch.float32)
    recon_dataset = Data.TensorDataset(recon_tensor, corrupted_tensor)
    ...
    for epoch in range(args.m_step_epochs):
        ...
        for x_recon, x_corrupted in recon_loader:
            x_recon = x_recon.to(device)
            x_corrupted = x_corrupted.to(device)

            # Convert corrupted data (with NaN) to image space to get pixel-accurate mask
            x_corrupted_img = uncond_model.ts_to_img(x_corrupted)
            mask_img = torch.isnan(x_corrupted_img).float() * -1 + 1  # 1=observed, 0=missing

            # Convert E-step reconstruction to image space (no NaN, fully imputed)
            x_img = uncond_model.ts_to_img(x_recon)

            # Masked loss: noise only on observed, loss only on observed
            # UNet sees full reconstruction but is only trained on observed pixels
            loss, to_log = uncond_model.loss_fn_irregular(x_img, mask_img)
...
def main(args):
    ...
    name = f"diffem_mmps_iter_stl_decomp_proj_curr_masked_loss_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        # Force observed positions to exact original values
        obs_expanded = np.expand_dims(obs_masks, -1)
        nan_filled = np.nan_to_num(corrupted_data, nan=0.0)
        initial_reconstructions = np.where(obs_expanded, nan_filled, initial_reconstructions)

        uncond_model = m_step(
            args, uncond_model, optimizer,
            initial_reconstructions, corrupted_data, em_iter=-1, device=args.device, logger=logger
        )
        ...
        for em_iter in range(args.em_iters):
            ...
            # Force observed positions to exact original values
            reconstructions = np.where(obs_expanded, nan_filled, reconstructions)
            ...
            uncond_model = m_step(
                args, uncond_model, optimizer,
                reconstructions, corrupted_data, em_iter, args.device, logger
            )
