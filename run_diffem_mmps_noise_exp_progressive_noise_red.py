"""
Experiment 5: Progressive Noise Reduction.

In the M-step, the training data is a blend:
  x_train = (1-alpha)*reconstruction + alpha*noisy_obs,
where alpha decreases from 1.0 to 0.0 across EM iterations.

Early: train mostly on noisy data (learn rough structure).
Late: train on clean MMPS reconstructions (refine).

For missing-only (noise_level=0), alpha=0 always (use full reconstruction, same as original).

Based on DiffEM with MMPS E-step — STL Decomp Projection + Curriculum + Noise Support (DiEM-style).

Usage:
  --missing_rate 0.0 --noise_level 0.1 --gaussian_noise_level 0.1   -> noise only
  --missing_rate 0.5 --noise_level 0.0                               -> missing only (original)
  --missing_rate 0.3 --noise_level 0.1 --gaussian_noise_level 0.1   -> missing + noise
"""
...
def m_step(args, uncond_model, optimizer, reconstructions, em_iter, device, logger=None, noisy_data=None, em_iters_total=20):
    ...
    if noise_level > 0 and noisy_data is not None and em_iter >= 0:
        # Progressive blend: alpha goes from 1.0 to 0.0
        progress = min(1.0, (em_iter + 1) / max(em_iters_total * 0.7, 1))
        alpha = 1.0 - progress  # 1.0 -> 0.0
        # Zero-fill NaN in noisy data for blending
        noisy_filled = np.nan_to_num(noisy_data, nan=0.0)
        obs_mask = ~np.isnan(noisy_data).any(axis=-1, keepdims=True)
        # Only blend at observed positions
        blended = reconstructions.copy()
        blended[obs_mask.squeeze(-1)] = (
            (1 - alpha) * reconstructions[obs_mask.squeeze(-1)] +
            alpha * noisy_filled[obs_mask.squeeze(-1)]
        )
        reconstructions = blended
        print(f"  Progressive blend: alpha={alpha:.2f} (noisy weight)")
...
def main(args):
    ...
    name = f"noise_exp_progressive_noise_red_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        uncond_model = m_step(
            ...
            noisy_data=corrupted_data, em_iters_total=args.em_iters
        )
