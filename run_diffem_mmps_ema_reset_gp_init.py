"""
DiffEM with Unconditional Model + MMPS E-step — EMA Reset + GP Warm-Start

Same as run_diffem_mmps_ema_reset.py with ONE change:
  Gaussian initialization is replaced with GP-informed initialization.
  Missing values are filled using a Gaussian Process with composite kernel
  (RBF + ExpSineSquared + WhiteKernel) that encodes trend and seasonality.
"""
...
from utils.utils_stl import initialize_with_gp
...
def main(args):
    ...
    name = f"diffem_mmps_ema_reset_gp_init_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        # === Phase 1: Initialize with GP-informed prior ===
        print(f"\n{'='*60}")
        print("Phase 1: GP Warm-Start Initialization")
        print(f"{'='*60}")

        initial_reconstructions = initialize_with_gp(
            corrupted_data, obs_masks, seed=args.seed
        )

        # === Phase 1.5: Train unconditional model on GP-initialized data ===
        print(f"\n{'='*60}")
        print("Phase 1.5: Initial unconditional model training on GP-initialized data")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("DiffEM-MMPS + GP Warm-Start Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS + GP Warm-Start training is complete")
