...
Splits each posterior denoising step into a cascade: first correct the trend
(low-frequency), then correct the details conditioned on the corrected trend.
Two CG solves, each on an easier sub-problem.
"""
...
# =============================================================================
# MMPSDiffusionProcess — Multiscale Cascade
# =============================================================================

class MMPSDiffusionProcess:
    """
    MMPS-style diffusion sampler with multiscale cascade posterior denoising.

    Each posterior denoising step is split into two stages:
      Stage 1: Correct the trend (low-frequency) via CG on smoothed residual.
      Stage 2: Correct the details conditioned on the corrected trend via CG
               on the remaining (detail) residual.

    Two CG solves, each on an easier sub-problem, improve convergence.
    """

    def __init__(self, args, diffusion_fn, shape, sigma_y=0.01, cg_iters=1,
                 cascade_trend_window=None):
        self.args = args
        ...
        # Cascade parameters
        self.cascade_trend_window = cascade_trend_window

    def _extract_trend(self, x, window):
        """
        Extract low-frequency trend via moving average along W axis (time dimension).

        Uses F.conv2d with a uniform kernel along the W dimension.

        Args:
            x: input tensor (B, C, H, W), float64
        ...
        # Create 1D uniform kernel along W axis: shape (1, 1, 1, window)
        kernel = torch.ones(1, 1, 1, window, dtype=x.dtype, device=x.device) / window
        padding = window // 2

        # Apply per-channel using groups
        x_reshaped = x.reshape(B * C, 1, H, W)
        trend = F.conv2d(x_reshaped, kernel, padding=(0, padding))
        trend = trend.reshape(B, C, H, W)

        return trend

    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        """
        Posterior denoiser with multiscale cascade.

        Stage 1: CG solve on smoothed (trend) residual.
        Stage 2: CG solve on remaining detail residual after trend correction.
        ...
        # Auto-detect trend window from image width
        W = x_t.shape[-1]
        window = self.cascade_trend_window if self.cascade_trend_window is not None else max(3, W // 4)
        ...
        # CG linear operator (shared by both stages)
        def cg_operator(v):
            return sigma_y_sq * v + sigma_sq * mask * vjp_fn(mask * v)

        # === Stage 1: Trend correction ===
        # Compute trend residual by smoothing the full residual
        r_full = x_obs - mask * denoised
        r_trend = self._extract_trend(r_full, window)

        # CG solve for trend
        v_trend = conjugate_gradient(cg_operator, r_trend, maxiter=self.cg_iters)
        score_trend = vjp_fn(mask * v_trend)

        # === Stage 2: Detail correction ===
        # Apply trend correction to get intermediate denoised
        denoised_corrected = denoised + sigma_sq * score_trend

        # Compute detail residual (should be smaller since trend is corrected)
        r_detail = x_obs - mask * denoised_corrected

        # CG solve for details
        v_detail = conjugate_gradient(cg_operator, r_detail, maxiter=self.cg_iters)
        score_detail = vjp_fn(mask * v_detail)

        # Combine: denoised + sigma^2 * (score_trend + score_detail)
        result = denoised.detach() + sigma_sq * (score_trend.detach() + score_detail.detach())
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    ...
    cascade_trend_window = getattr(args, 'cascade_trend_window', None)
    ...
    with uncond_model.ema_scope():
        process = MMPSDiffusionProcess(
            ...
            cascade_trend_window=cascade_trend_window
        )
...
def main(args):
    ...
    name = f"diffem_mmps_multiscale_cascade_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print(f"\n{'='*60}")
        print(f"Starting DiffEM-MMPS + Multiscale Cascade + Curriculum with {args.em_iters} EM iterations")
        print(f"  MMPS sigma_y: {args.mmps_sigma_y}")
        print(f"  MMPS cg_iters: {args.mmps_cg_iters}")
        print(f"  Cascade trend window: {getattr(args, 'cascade_trend_window', None) or 'auto'}")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("DiffEM-MMPS + Multiscale Cascade + Curriculum Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS + Multiscale Cascade + Curriculum training is complete")
...
if __name__ == '__main__':
    ...
    # Multiscale cascade parameters
    if not hasattr(args, 'cascade_trend_window'):
        args.cascade_trend_window = None  # Auto-detect from image width
