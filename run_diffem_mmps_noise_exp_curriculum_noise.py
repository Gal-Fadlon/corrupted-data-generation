"""
Experiment 8: Curriculum Noise Schedule.

Start training with artificially increased noise level (2x actual) and reduce it
across EM iterations.
  Early EM: pretend noise is 2x → model learns aggressive denoising.
  Late EM: use true noise level → model refines.

This applies to both the E-step sigma_obs and the observation creation.
For missing-only (noise_level=0), no change.
"""
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    ...
    sigma_obs_base = getattr(args, 'noise_level', 0.0)
    if sigma_obs_base > 0 and em_iter >= 0:
        # Curriculum: start at 2x noise, decay to 1x
        progress = min(1.0, (em_iter + 1) / max(args.em_iters * 0.5, 1))
        curriculum_factor = 2.0 * (1.0 - progress) + 1.0 * progress  # 2.0 -> 1.0
        sigma_obs = sigma_obs_base * curriculum_factor
        print(f"  Curriculum noise: sigma_obs={sigma_obs:.4f} (base={sigma_obs_base}, factor={curriculum_factor:.2f})")
    else:
        sigma_obs = sigma_obs_base
...
def main(args):
    ...
    name = f"diffem_mmps_noise_exp_curriculum_noise_{name}"
    logging.info(args)
