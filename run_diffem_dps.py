"""
DiffEM with Unconditional Model + DPS E-step

This script implements EM with diffusion priors using a SINGLE unconditional model:
- E-step: DPS (Diffusion Posterior Sampling) imputation — approximates the posterior
  score via Bayes' rule using a simple Gaussian likelihood with the denoiser mean.
- M-step: Train the same unconditional model on fully imputed data

Paper: "An EM Algorithm for Training Clean Diffusion Models from Corrupted Observations"
       (Bai et al., NeurIPS 2024)

DPS core idea: At each reverse diffusion step, compute the unconditional denoised
estimate x̂₀ = D_θ(x_t), then add a gradient correction based on the measurement
likelihood ∇_{x_t} log p(y|x_t) ≈ -(1/2σ_y²) · ∇_{x_t} ‖A·D_θ(x_t) - y‖².
"""
...
class DPSDiffusionProcess:
    """
    DPS-style diffusion sampler for inpainting with an unconditional model.

    Uses the same EDM/Karras sigma schedule as DiffusionProcess. At each reverse
    step, the unconditional denoiser provides x̂₀, and a likelihood gradient
    correction steers the trajectory toward consistency with observations.

    Posterior score decomposition (Bayes' rule):
        ∇_x log q(x_t | y) = ∇_x log q(x_t) + ∇_x log p(y | x_t)

    DPS approximation (drops covariance, uses only the mean):
        p(y | x_t) ≈ N(y | A · D_θ(x_t), σ_y² · I)

    Likelihood gradient for masking (A = mask, diagonal):
        ∇_{x_t} log p(y|x_t) ≈ -(1/(2σ_y²)) · ∇_{x_t} ‖mask · D_θ(x_t) - x_obs‖²
    """

    def __init__(self, args, diffusion_fn, shape, guidance_scale=1.0, sigma_y=0.01):
        self.args = args
    ...
    def dps_sample(self, latents, x_obs_img, mask_img):
        """
        DPS sampling: inpaint missing regions using likelihood gradient guidance.

        At each denoising step (t_cur -> t_next):
        1. Fresh gradient tape: x_cur = x_next.detach().requires_grad_(True)
        2. Stochastic churn (same as standard EDM)
        3. Euler step WITH gradients: denoised = D_θ(x_hat, t_hat)
        4. Compute x_next via Euler ODE
        5. Heun correction under torch.no_grad() (no gradient needed)
        6. DPS guidance: loss = ‖mask * denoised - x_obs‖²; grad via autograd
        7. Apply: x_next -= (guidance_scale / (2 * sigma_y²)) * grad
        ...
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):
            # Fresh gradient tape per step — keeps memory bounded
            ...
            # Euler step WITH gradients enabled — denoised is x̂₀
            denoised = self.net(x_hat, t_hat, None).to(torch.float64)
            d_cur = (x_hat - denoised) / t_hat
            x_next = x_hat + (t_next - t_hat) * d_cur

            # Apply 2nd order correction under torch.no_grad() (saves memory)
            ...
            # DPS guidance: compute likelihood gradient
            measurement_loss = (mask * denoised - x_obs).square().sum()
            grad = torch.autograd.grad(measurement_loss, x_cur)[0]

            # Apply guidance correction
            x_next = x_next.detach() - (self.guidance_scale / (2 * self.sigma_y ** 2)) * grad

        return x_next

    def sampling_dps(self, x_obs_img, mask_img):
        """
        Convenience wrapper for DPS sampling.
        NOT decorated with @torch.no_grad() — DPS needs gradients.
        ...
        return self.dps_sample(latents, x_obs_img, mask_img)
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    """
    E-step: Impute missing values using DPS with the unconditional model.

    Uses DPSDiffusionProcess: at each reverse step, the denoised estimate is
    corrected via the measurement likelihood gradient.
    ...
    """
    print(f"\n=== E-Step (EM iter {em_iter}) — DPS Imputation ===")
    print("Imputing missing values via DPS with unconditional model...")
    ...
    guidance_scale = getattr(args, 'dps_guidance_scale', 1.0)
    sigma_y = getattr(args, 'dps_sigma_y', 0.01)

    # No torch.no_grad() outer context — DPS requires gradient computation
    with uncond_model.ema_scope():
        process = DPSDiffusionProcess(
            args, uncond_model.net, target_shape,
            guidance_scale=guidance_scale, sigma_y=sigma_y
        )

        for start_idx in tqdm(range(0, N, batch_size), desc="E-step DPS"):
            ...
            # DPS sampling
            x_img_imputed = process.sampling_dps(x_obs_img, mask_img)
...
def main(args):
    """Main DiffEM-Uncond training loop with DPS E-step."""

    # Model name and directory
    name = create_model_name_and_dir(args)
    name = f"diffem_dps_{name}"
    ...
    with CompositeLogger([NeptuneLogger()]) if args.neptune else PrintLogger() as logger:
        ...
        # === Phase 2: EM Loop ===
        print(f"\n{'='*60}")
        print(f"Starting DiffEM-DPS with {args.em_iters} EM iterations")
        print(f"  DPS guidance_scale: {args.dps_guidance_scale}")
        print(f"  DPS sigma_y: {args.dps_sigma_y}")
        print(f"{'='*60}")
        ...
        for em_iter in range(args.em_iters):
            ...
            # E-step: DPS imputation with current model
        ...
        print("\n" + "="*60)
        print("DiffEM-DPS Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-DPS training is complete")
