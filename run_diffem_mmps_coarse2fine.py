"""
DiffEM with Unconditional Model + MMPS E-step — Coarse-to-Fine variant

Based on run_diffem_mmps_ema_reset.py (EMA reset kept) with ONE change:
  The EM schedule is split into two phases:
    Phase A (first 1/3 of EM iters): M-step trains on low-pass filtered data
      (only dominant frequencies), forcing the model to learn core structure.
    Phase B (remaining iters): M-step trains on full-bandwidth data to refine
      high-frequency details.
  Inspired by FDEDiff (ICLR 2026 submission).
"""
...
# =============================================================================
# Low-pass filter for coarse-to-fine schedule
# =============================================================================

def low_pass_filter_ts(ts_data, cutoff_ratio=0.4):
    """
    Keep only the lowest cutoff_ratio fraction of frequency bins.

    Args:
        ts_data: (N, seq_len, features) numpy array
        cutoff_ratio: fraction of frequency bins to keep (0..1)

    Returns:
        filtered: (N, seq_len, features) low-pass filtered data
    """
    fft = np.fft.rfft(ts_data, axis=1)
    n_freq = fft.shape[1]
    cutoff = max(1, int(n_freq * cutoff_ratio))
    fft[:, cutoff:, :] = 0
    return np.fft.irfft(fft, n=ts_data.shape[1], axis=1).astype(np.float32)
...
def main(args):
    ...
    name = f"diffem_mmps_coarse2fine_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        # === Phase 2: Coarse-to-Fine EM Loop ===
        coarse_phase_iters = max(1, args.em_iters // 3)
        cutoff_ratio = 0.4

        print(f"\n{'='*60}")
        print(f"Starting Coarse-to-Fine DiffEM-MMPS with {args.em_iters} EM iterations")
        print(f"  Coarse phase: first {coarse_phase_iters} iters (cutoff_ratio={cutoff_ratio})")
        print(f"  Fine phase:   remaining {args.em_iters - coarse_phase_iters} iters (full bandwidth)")
        ...
        for em_iter in range(args.em_iters):
            is_coarse = em_iter < coarse_phase_iters
            phase_str = "COARSE" if is_coarse else "FINE"

            print(f"\n{'='*60}")
            print(f"EM Iteration {em_iter + 1}/{args.em_iters}  [{phase_str} phase]")
            print(f"{'='*60}")
            ...
            # In coarse phase, low-pass filter reconstructions before M-step
            if is_coarse:
                print(f"  Applying low-pass filter (cutoff_ratio={cutoff_ratio})...")
                recon_for_training = low_pass_filter_ts(reconstructions, cutoff_ratio=cutoff_ratio)
                N, L, C = recon_for_training.shape
                for i in range(N):
                    for t in range(L):
                        if obs_masks[i, t]:
                            obs_vals = corrupted_data[i, t, :]
                            valid = ~np.isnan(obs_vals)
                            recon_for_training[i, t, valid] = obs_vals[valid]
            else:
                recon_for_training = reconstructions

            # Reset EMA so it tracks only this iteration's training dynamics
            uncond_model.reset_ema()

            # M-step: Train unconditional model
            uncond_model = m_step(
                args, uncond_model, optimizer,
                recon_for_training, em_iter, args.device, logger
            )
            ...
            if logger is not None:
                ...
                logger.log('em/is_coarse_phase', float(is_coarse), em_iter)
