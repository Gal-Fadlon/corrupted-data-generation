"""
DiffEM with MMPS E-step — Hybrid Jacobian-Decomposition Covariance

Blends the Tweedie Jacobian covariance with a structured decomposition-based
prior covariance.  At high noise (sigma), the decomposition structure dominates;
at low noise, the learned Jacobian is trusted.

Blending schedule:  lam = lambda_max * (sigma / sigma_max)
  - High sigma -> lam ~ lambda_max  -> mostly decomposition prior
  - Low sigma  -> lam ~ 0           -> mostly learned Jacobian
"""
...
# =============================================================================
# MMPSDiffusionProcess — Hybrid Jacobian-Decomposition Covariance
# =============================================================================

class MMPSDiffusionProcess:
    """
    MMPS-style diffusion sampler with hybrid Jacobian-decomposition covariance.

    The CG operator interpolates between the learned Jacobian-based covariance
    and a structured decomposition-based prior covariance (low-pass filter
    along the W/time axis).

    At high noise levels, the decomposition prior dominates (stabilizes
    early sampling steps). At low noise, the learned Jacobian is trusted
    (preserves fine detail in later steps).

    Blending:  lam = lambda_max * (sigma / sigma_max)
    CG op:     sigma_y^2 * v + (1-lam) * sigma^2 * mask * J^T(mask*v)
                             + lam     * sigma^2 * mask * LP(mask*v)
    where LP is a low-pass (moving average) filter along W axis.
    """

    def __init__(self, args, diffusion_fn, shape, sigma_y=0.01, cg_iters=1,
                 lambda_max=0.8, decomp_period=None):
        self.args = args
    ...
    def _low_pass_filter(self, x, period):
        """
        Apply low-pass (moving average) filter along W axis.
        ...
        Returns:
            filtered: tensor (B, C, H, W) — smoothed along W
        """
        kernel = torch.ones(1, 1, 1, period, device=x.device, dtype=x.dtype) / period
        pad = period // 2
        B, C, H, W = x.shape
        x_reshaped = x.reshape(B * C, 1, H, W)
        padded = F.pad(x_reshaped, (pad, pad, 0, 0), mode='reflect')
        filtered = F.conv2d(padded, kernel, groups=1)
        # Handle potential size mismatch from even period
        if filtered.shape[-1] != W:
            filtered = filtered[..., :W]
        return filtered.reshape(B, C, H, W)

    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        """
        Posterior denoiser with hybrid Jacobian-decomposition covariance.

        The CG operator blends between the Jacobian-based term and a
        decomposition-based (low-pass) term. The blending weight depends
        on the current noise level sigma.
        ...
        # Blending weight: high at high sigma, low at low sigma
        lam = self.lambda_max * (float(sigma) / self.sigma_max)
        ...
        # Hybrid CG operator: blends Jacobian and decomposition covariance
        def cg_operator(v):
            # Jacobian-based term (standard MMPS)
            jac_term = sigma_sq * mask * vjp_fn(mask * v)
            # Decomposition-based term: apply structured covariance (low-pass)
            decomp_term = sigma_sq * mask * self._low_pass_filter(mask * v, period)
            # Blend
            return sigma_y_sq * v + (1 - lam) * jac_term + lam * decomp_term

        # Solve hybrid system via CG
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    ...
    Uses MMPSDiffusionProcess with hybrid Jacobian-decomposition covariance.
    ...
    """
    print(f"\n=== E-Step (EM iter {em_iter}) — MMPS Imputation (Hybrid Jacobian-Decomp) ===")
    print("Imputing missing values via MMPS with hybrid Jacobian-decomposition covariance...")
    ...
    lambda_max = getattr(args, 'lambda_max', 0.8)
    ...
    with uncond_model.ema_scope():
        process = MMPSDiffusionProcess(
            ...
            lambda_max=lambda_max, decomp_period=decomp_period
        )
...
def main(args):
    """Main DiffEM-Uncond training loop with MMPS E-step (Hybrid Jacobian-Decomp)."""

    # Model name and directory
    name = create_model_name_and_dir(args)
    name = f"diffem_mmps_hybrid_jacobian_decomp_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print(f"\n{'='*60}")
        print(f"Starting DiffEM-MMPS + Hybrid Jacobian-Decomp with {args.em_iters} EM iterations")
        ...
        print("\n" + "="*60)
        print("DiffEM-MMPS + Hybrid Jacobian-Decomp Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS + Hybrid Jacobian-Decomp training is complete")
...
if __name__ == '__main__':
    ...
    # Hybrid Jacobian-Decomp parameters
