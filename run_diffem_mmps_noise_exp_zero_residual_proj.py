"""
Experiment 9: Zero Residual Projection. In the decomposition projection step,
when noise_level > 0, completely zero out the residual component
(residual_keep_fraction=0) instead of the default 0.8. After STL decomposition
of noisy data, the residual IS the noise — remove it entirely. For missing-only
(noise_level=0), keep default 0.8.
"""
...
class DecompProjectedMMPSDiffusionProcess:
    ...
    def _project_denoised(self, denoised_img, sigma):
        ...
        noise_level = getattr(self.args, 'noise_level', 0.0)
        if noise_level > 0:
            # Zero out residual when noise is present — residual IS the noise
            projected_np = decompose_and_smooth_ts(ts_np, residual_keep_fraction=0.0)
        else:
...
def main(args):
    ...
    name = f"noise_exp_zero_residual_proj_{name}"
    logging.info(args)
