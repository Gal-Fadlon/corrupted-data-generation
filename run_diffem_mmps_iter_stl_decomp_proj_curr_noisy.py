"""
DiffEM with MMPS E-step — STL Decomp Projection + Curriculum + Noise Support (DiEM-style)

Same as stl_decomp_proj_curriculum but with proper noise handling following the
DiEM paper (Rozet et al., 2024, https://github.com/francois-rozet/diem):
...
CG operator (matching DiEM's PosteriorDenoiser):
  ...
  NOT: (sigma_y^2 + sigma_obs^2) * v  (which was our previous incorrect approach)
...
def main(args):
    ...
    name = f"diffem_mmps_iter_stl_decomp_proj_curr_noisy_{name}"
    logging.info(args)
