"""
DiffEM with Unconditional Model + MMPS E-step — EMA Reset + Iterative STL Warm-Start + Noise Support

Unified corruption handler: supports missing data, Gaussian noise, and their combination.
The E-step uses a generalized MMPS posterior denoiser where:
  - Missing positions: fully imputed by the posterior (hard mask)
  - Noisy observed positions: soft correction weighted by observation noise level
  - Clean observed positions: kept exact (sigma_obs=0 case, same as original)

Usage:
  --missing_rate 0.5 --noise_level 0.0   -> standard missing data (same as original)
  --missing_rate 0.0 --noise_level 0.1   -> pure Gaussian noise denoising
  --missing_rate 0.3 --noise_level 0.1   -> missing + noise on observed
"""
...
class MMPSDiffusionProcess:
    ...
    def __init__(self, args, diffusion_fn, shape, sigma_y=0.01, cg_iters=1, sigma_obs=0.0):
        self.args = args
        ...
        self.sigma_obs = sigma_obs  # observation noise level (0 = clean observations)
    ...
    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        ...
        # CG linear operator: M(v) = (σ_y² + σ_obs²) · v + σ² · mask · Jᵀ(mask · v)
        # σ_obs² accounts for observation noise — increases uncertainty on observed values,
        # making the posterior correction softer (less aggressive toward noisy observations).
        # When σ_obs=0 (clean observations), reduces to the original operator.
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    ...
    with uncond_model.ema_scope():
        process = MMPSDiffusionProcess(
            ...
            sigma_y=sigma_y, cg_iters=cg_iters, sigma_obs=sigma_obs
        )
...
def main(args):
    ...
    name = f"diffem_mmps_ema_reset_iter_stl_init_noisy_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print("\n" + "="*60)
        print("DiffEM-MMPS + Iterative STL Warm-Start Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS + Iterative STL Warm-Start training is complete")
