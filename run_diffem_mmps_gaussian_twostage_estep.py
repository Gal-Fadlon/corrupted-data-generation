"""
Gaussian Noise Handler — Approach 4: Two-Stage E-step

Stage 1 (Denoise): Use the trained diffusion model as an explicit denoiser.
  - Start from the noisy observation y
  - Add diffusion noise up to level sigma_obs (the observation noise level)
  - Run reverse diffusion from sigma_obs back to 0
  - This produces a denoised version of the observations

Stage 2 (Impute): Standard MMPS imputation using the denoised observations.
  - Use denoised data as the "clean" observations
  - Apply standard MMPS posterior sampling for missing positions
  - For pure noise (no missing), Stage 2 is a no-op

This leverages the diffusion model's learned prior to separate signal from noise,
then uses MMPS for any remaining imputation task.
...
def diffusion_denoise(model, noisy_img, sigma_obs, num_steps=10):
    """
    Use the diffusion model as an explicit denoiser.

    Given noisy data at noise level sigma_obs, run partial reverse diffusion
    starting from sigma_obs (not from sigma_max) to denoise.

    Args:
        model: TS2img_Karras model (uses model.net for denoising)
        noisy_img: (B, C, H, W) tensor — noisy image
        sigma_obs: observation noise level
        num_steps: number of reverse steps for denoising

    Returns:
        denoised_img: (B, C, H, W) tensor — denoised image
    """
    if sigma_obs <= 0:
        return noisy_img
    ...
    sigma_start = sigma_obs

    if sigma_start <= sigma_min:
        # Noise level too low, just do one-step denoising
        with torch.no_grad():
            sigma_t = torch.tensor([sigma_start], device=device, dtype=torch.float64)
            denoised = net(noisy_img.to(torch.float64), sigma_t, None).to(noisy_img.dtype)
        return denoised

    # Create time steps from sigma_obs down to sigma_min
    rho = 7
    step_indices = torch.arange(num_steps, dtype=torch.float64, device=device)
    t_steps = (sigma_start ** (1 / rho) + step_indices / (num_steps - 1) * (
                sigma_min ** (1 / rho) - sigma_start ** (1 / rho))) ** rho
    t_steps = torch.cat([net.round_sigma(t_steps), torch.zeros_like(t_steps[:1])])

    # Start from noisy data (already at noise level sigma_obs)
    x_next = noisy_img.to(torch.float64)
    ...
    with torch.no_grad():
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):
            ...
            if i < num_steps - 1 and t_next > 0:
                denoised_2 = net(x_next, t_next, None).to(torch.float64)
                d_prime = (x_next - denoised_2) / t_next
                x_next = x_cur + (t_next - t_cur) * (0.5 * d_cur + 0.5 * d_prime)

    return x_next.to(noisy_img.dtype)
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    ...
    if sigma_obs > 0:
        print(f"  Two-stage: Stage 1 = diffusion denoise (sigma_obs={sigma_obs}), Stage 2 = MMPS imputation")
    ...
    with uncond_model.ema_scope():
        ...
        for start_idx in tqdm(range(0, N, batch_size), desc="E-step Decomp-Projected"):
            ...
            # Stage 1: Denoise observed data using diffusion model
            if sigma_obs > 0:
                with torch.no_grad():
                    x_obs_img = diffusion_denoise(uncond_model, x_obs_img, sigma_obs)
...
def main(args):
    ...
    name = f"diffem_gaussian_twostage_estep_{name}"
    logging.info(args)
