"""
DiffEM with Unconditional Model + MMPS E-step — EMA Reset + Decomp Smoothing

Same as run_diffem_mmps_ema_reset.py with ONE enhancement:
  After each E-step (MMPS imputation), a decomposition-based post-processing
  step is applied:
    1. Hard-replace observed positions with actual observed values (MMPS
       introduces small deviations due to image ↔ TS round-trip averaging).
    2. At missing positions, apply MA-based smoothing: extract the MA trend
       and smooth only the residual, giving temporally coherent imputations.

  This gives the M-step cleaner training data without changing the model,
  MMPS, or evaluation.
"""
...
# =============================================================================
# Post-E-Step Decomposition Smoothing
# =============================================================================

def postprocess_with_decomp_smoothing(reconstructions, corrupted_data, obs_masks,
                                       smooth_window=5):
    """
    Post-process MMPS reconstructions using decomposition-based smoothing.

    1. Hard-replace observed positions with actual observations.
    2. At missing positions, smooth the high-frequency residual using a
       local moving average while preserving the MA trend.

    Args:
        reconstructions: (N, T, C) from MMPS (no NaN)
        corrupted_data:  (N, T, C) original data with NaN at missing
        obs_masks:       (N, T) boolean, True = observed
        smooth_window:   MA window for residual smoothing
    Returns:
        result: (N, T, C) post-processed reconstructions
    """
    N, T, C = reconstructions.shape
    result = reconstructions.copy()

    for i in range(N):
        obs_idx = obs_masks[i]

        obs_vals = corrupted_data[i, obs_idx, :]
        valid = ~np.isnan(obs_vals)
        result[i, obs_idx, :] = np.where(valid, obs_vals, result[i, obs_idx, :])

    half_w = smooth_window // 2
    for i in range(N):
        mis_positions = np.where(~obs_masks[i])[0]
        if len(mis_positions) == 0:
            continue

        for c in range(C):
            series = result[i, :, c].copy()

            trend = np.zeros(T)
            for t in range(T):
                start = max(0, t - half_w)
                end = min(T, t + half_w + 1)
                trend[t] = series[start:end].mean()
            residual = series - trend
            residual_orig = residual.copy()

            for t in mis_positions:
                start = max(0, t - half_w)
                end = min(T, t + half_w + 1)
                residual[t] = residual_orig[start:end].mean()

            result[i, :, c] = trend + residual
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    """
    E-step with decomposition-based post-processing:
    standard MMPS imputation followed by observation hard-replacement
    and MA-based residual smoothing at missing positions.
    """
    print(f"\n=== E-Step (EM iter {em_iter}) — MMPS + Decomp Smoothing ===")
    print("Imputing missing values via MMPS with decomp post-processing...")
    ...
    print(f"E-step MMPS complete. Imputed {len(reconstructions)} sequences.")

    smooth_window = getattr(args, 'smooth_window', 5)
    print(f"  Applying decomp smoothing (window={smooth_window})...")
    reconstructions = postprocess_with_decomp_smoothing(
...
def main(args):
    ...
    name = f"diffem_mmps_ema_reset_decomp_smooth_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        # === Phase 2: EM Loop ===
        print(f"\n{'='*60}")
        print(f"Starting DiffEM-MMPS + Decomp Smoothing with {args.em_iters} EM iterations")
        ...
        print("\n" + "="*60)
        print("DiffEM-MMPS + Decomp Smoothing Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS + Decomp Smoothing training is complete")
