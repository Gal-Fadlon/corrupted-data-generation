"""
DiffEM with MMPS E-step — Kalman Init + Curriculum + Ambient M-step

Extends kalman_curriculum by replacing the vanilla M-step with an Ambient
Diffusion-style M-step.  During M-step training, delta fraction of observed
positions are further masked in the network input, but the loss is evaluated
on all originally-observed positions.  This prevents the model from memorising
E-step imputation artifacts.

E-step (vanilla MMPS + curriculum) is unchanged from the base experiment.
"""
...
# =============================================================================
# MMPSDiffusionProcess (vanilla — unchanged from kalman_curriculum)
# =============================================================================
...
def main(args):
    ...
    name = f"diffem_mmps_kalman_curriculum_ambient_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print(f"\n{'='*60}")
        print(f"Starting Kalman + Curriculum + Ambient DiffEM-MMPS")
        ...
        if final_metrics:
            ...
            for k, v in final_metrics.items():
                print(f"  {k}: {v:.4f}")
        print("\n" + "="*60)
        print("Kalman + Curriculum + Ambient DiffEM-MMPS Complete!")
        print("="*60)
        if best_metrics:
