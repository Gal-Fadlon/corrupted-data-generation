...
@click.option('--mask_full_rgb', help='Whether to mask all the RGB channels together', metavar='BOOL', default=True, show_default=True)
...
# Neptune
@click.option('--neptune', help='Enable Neptune logging', type=bool, default=True, show_default=True)
...
def main(**kwargs):
    ...
    # Set up Neptune logger (only on rank 0)
    # Change to project root so neptune/ folder is found (and stay there for Neptune async ops)
    project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    os.chdir(project_root)
    
    if dist.get_rank() == 0 and opts.neptune:
        logger = CompositeLogger([NeptuneLogger()])
        logger.__enter__()
    ...
    # Random seed (use CPU tensor to avoid backend/device broadcast issues).
    ...
    if dist.get_rank() == 0:
        ...
        with dnnlib.util.open_url(os.path.join(c.run_dir, 'training_options.json'), read_mode='wt') as f:
            c_json = dnnlib.EasyDict(c)
            c_json.device = str(device)
