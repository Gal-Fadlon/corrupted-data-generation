...
@click.command()
@click.option('--with_wandb', help='Whether to report to wandb', metavar='BOOL', type=bool, default=False, show_default=True)
@click.option('--network', 'network_loc',  help='Location of the folder where the network is stored', metavar='PATH|URL',                      type=str, required=False, default='/Users/gal.fadlon/Git/corrupted-data-generation/checkpoints/checkpoints/cifar04')
@click.option('--training_options_loc', help='Location of the training options file', metavar='PATH|URL', type=str, required=False, default='/Users/gal.fadlon/Git/corrupted-data-generation/checkpoints/checkpoints/cifar04/training_options.json')
@click.option('--outdir',                  help='Where to save the output images', metavar='DIR',                   type=str, required=False, default='eval_output')
...
@click.option('--mask_full_rgb', help='Whether to mask the full RGB channel.', default=True, show_default=True, required=False)


@click.option('--experiment_name', help="Name of the experiment to log to wandb", type=str, required=False, default='eval_test')
@click.option('--wandb_id', help='Id of wandb run to resume', type=str, default='')
@click.option('--ref', 'ref_path',      help='Dataset reference statistics ', metavar='NPZ|URL',    type=str, required=False, default=None)
...
@click.option('--num_classes',             help='Number of classes', metavar='INT', type=int, default=10, show_default=True)
...
def main(with_wandb, network_loc, training_options_loc, outdir, subdirs, seeds, class_idx, max_batch_size, 
    ...
    if not skip_generation:
        ...
        while True:
            ...
            if not skip_calculation and ref_path is not None:
                ...
                dist.print0("Skipping calculation because no reference stats provided.")
            torch.distributed.barrier() 
...
def calc(image_path, ref_path, num_expected, seed, batch, num_rows=8, num_cols=8, image_size=32, with_wandb=True):
    ...
    elif dist.get_rank() == 0 and not with_wandb:
        # Plot image grid using matplotlib instead of saving
        if grid_image is not None:
            plt.figure(figsize=(10, 10))
            plt.imshow(grid_image)
            plt.axis('off')
            plt.title(f'Generated Images (Checkpoint {checkpoint_index})')
            plt.show()
            dist.print0(f"Plotted image grid for checkpoint {checkpoint_index}")
