...
def training_loop(
    ...
    logger              = None,     # Logger for metrics (e.g., NeptuneLogger).
):
    # Initialize.
    ...
    if device.type == 'cuda' and dist.get_world_size() > 1:
        ddp = torch.nn.parallel.DistributedDataParallel(net, device_ids=ddp_device_ids, broadcast_buffers=True)
    else:
        ddp = net
    ...
    while True:
        ...
        if device.type == 'cuda':
            ...
            gpu_mem_reserved = torch.cuda.max_memory_reserved(device) / 2**30
            fields += [f"gpumem {training_stats.report0('Resources/peak_gpu_mem_gb', gpu_mem):<6.2f}"]
            fields += [f"reserved {training_stats.report0('Resources/peak_gpu_mem_reserved_gb', gpu_mem_reserved):<6.2f}"]
        ...
        if dist.get_rank() == 0:
            ...
            # report to neptune logger
            if logger is not None:
                stats_dict = training_stats.default_collector.as_dict()
                if 'Loss/loss_train' in stats_dict:
                    logger.log('train/loss_train', stats_dict['Loss/loss_train'].mean, cur_tick)
                if 'Loss/loss' in stats_dict:
                    logger.log('train/loss_val', stats_dict['Loss/loss'].mean, cur_tick)
                if 'Loss/loss_test' in stats_dict:
                    logger.log('train/loss_test', stats_dict['Loss/loss_test'].mean, cur_tick)
            stats_jsonl.flush()
        dist.update_progress(cur_nimg // 1000, total_kimg)
