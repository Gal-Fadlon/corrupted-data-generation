    ...
    while True:
        ...
        for round_idx in range(num_accumulation_rounds):
            with misc.ddp_sync(ddp, (round_idx == num_accumulation_rounds - 1)):
                ...
                # ============================================================
                # DEBUG: Print what training loop received (only first batch)
                # ============================================================
                if cur_nimg == 0 and round_idx == 0:  # Only first batch
                    print("\n" + "="*80)
                    print("🔄 TRAINING LOOP - What we received from dataset")
                    print("="*80)
                    print(f"images shape: {images.shape}")
                    print(f"images dtype: {images.dtype}")
                    print(f"images device: {images.device}")
                    print(f"images min: {images.min().item():.4f}, max: {images.max().item():.4f}")
                    print(f"First 3 pixels images[0,0,:3]: {images[0,0,:3].cpu().numpy()}")
                    print(f"\n⚠️  Images are just .to(device).to(float32) - NO CORRUPTION APPLIED!")
                    print(f"\nNow passing 'images' directly to loss function...")
                    print("="*80 + "\n")
