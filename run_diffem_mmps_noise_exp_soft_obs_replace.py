"""
Experiment 7: Soft Observation Replacement. After the MMPS E-step, instead of
trusting observations fully, use a Bayesian blend:
  recon = (sigma_obs^2 * mmps + sigma_prior^2 * y_obs) / (sigma_obs^2 + sigma_prior^2).
This trusts the model more at high noise. sigma_prior is estimated from
reconstruction variance. For missing-only (noise_level=0), hard-replaces
observed positions as usual.
"""
...
def main(args):
    ...
    name = f"noise_exp_soft_obs_replace_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        # Soft observation replacement for Phase 1 init
        noise_level = getattr(args, 'noise_level', 0.0)
        if noise_level > 0:
            sigma_obs_sq = noise_level ** 2
            # Estimate model uncertainty from reconstruction variance across nearby EM iters
            sigma_prior_sq = max(0.001, np.var(initial_reconstructions))
            blend_weight = sigma_obs_sq / (sigma_obs_sq + sigma_prior_sq)
            # blend_weight close to 1 = trust model more, close to 0 = trust observation more
            obs_expanded = np.expand_dims(obs_masks, -1)
            noisy_filled = np.nan_to_num(corrupted_data, nan=0.0)
            initial_reconstructions = np.where(
                obs_expanded,
                blend_weight * initial_reconstructions + (1 - blend_weight) * noisy_filled,
                initial_reconstructions
            )
            print(f"  Soft replacement (init): blend_weight={blend_weight:.4f} (model trust)")
        ...
        for em_iter in range(args.em_iters):
            ...
            # Soft observation replacement
            noise_level = getattr(args, 'noise_level', 0.0)
            if noise_level > 0:
                sigma_obs_sq = noise_level ** 2
                # Estimate model uncertainty from reconstruction variance across nearby EM iters
                sigma_prior_sq = max(0.001, np.var(reconstructions))
                blend_weight = sigma_obs_sq / (sigma_obs_sq + sigma_prior_sq)
                # blend_weight close to 1 = trust model more, close to 0 = trust observation more
                obs_expanded = np.expand_dims(obs_masks, -1)
                noisy_filled = np.nan_to_num(corrupted_data, nan=0.0)
                reconstructions = np.where(
                    obs_expanded,
                    blend_weight * reconstructions + (1 - blend_weight) * noisy_filled,
                    reconstructions
                )
                print(f"  Soft replacement: blend_weight={blend_weight:.4f} (model trust)")
