"""
DiffEM with MMPS E-step — Trend-Seasonal Score Decomposition

Decompose the posterior score into trend and seasonal components. Weight them
differently based on noise level — trust trend corrections at all noise levels,
but only allow seasonal corrections when the model has resolved the trend (low noise).
"""
...
# =============================================================================
# MMPSDiffusionProcess — Trend-Seasonal Score
# =============================================================================

class MMPSDiffusionProcess:
    """
    MMPS-style diffusion sampler with trend-seasonal score decomposition.

    After computing the posterior score via MMPS, the score is decomposed into
    trend (low-frequency) and seasonal (high-frequency) components using a
    moving average filter along the W (time) axis. The seasonal component is
    down-weighted at high noise levels where the model cannot reliably estimate
    fine-grained periodic patterns.

    Weighting scheme:
      - Trend: always weight 1.0 (trusted at all noise levels)
      - Seasonal: weight = min(1.0, sigma_ref / sigma)
        - High σ: seasonal weight → 0 (suppress unreliable periodic corrections)
        - Low σ: seasonal weight → 1.0 (full seasonal correction)
    """

    def __init__(self, args, diffusion_fn, shape, sigma_y=0.01, cg_iters=1,
                 sigma_ref=1.0, trend_window=None):
        self.args = args
        ...
        # Trend-seasonal parameters
        self.sigma_ref = sigma_ref
        # Auto-detect trend window from image width if not provided
        if trend_window is None:
            self.trend_window = max(3, shape[-1] // 8)
    ...
    def _extract_trend(self, x, window):
        """
        Extract trend component via moving average along W (time) axis.
    ...
    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        """
        Posterior denoiser with trend-seasonal score decomposition.

        Standard MMPS posterior computation followed by trend-seasonal weighting:
        1. Forward pass with gradient: denoised = D_θ(x_t)
        2. Compute VJP, residual, CG solve
        3. Compute score = vjp_fn(mask * v)
        4. Decompose score into trend + seasonal
        5. Weight seasonal by min(1.0, sigma_ref / sigma)
        6. Return denoised + σ² * score_weighted
        ...
        score = vjp_fn(mask * v).detach()

        # Decompose score into trend and seasonal
        score_trend = self._extract_trend(score, self.trend_window)
        score_seasonal = score - score_trend

        # Compute seasonal weight: small at high σ, 1.0 at low σ
        sigma_val = float(sigma)
        w_seasonal = min(1.0, self.sigma_ref / (sigma_val + 1e-10))

        # Weighted score
        score_weighted = score_trend + w_seasonal * score_seasonal

        # Detach from computation graph
        result = denoised.detach() + sigma_sq * score_weighted
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    ...
    Uses MMPSDiffusionProcess with trend-seasonal score decomposition.
    ...
    """
    print(f"\n=== E-Step (EM iter {em_iter}) — MMPS Imputation (Trend-Seasonal Score) ===")
    print("Imputing missing values via MMPS with trend-seasonal score decomposition...")
    ...
    with uncond_model.ema_scope():
        process = MMPSDiffusionProcess(
            ...
            sigma_ref=sigma_ref, trend_window=trend_window
        )
...
def main(args):
    """Main DiffEM-Uncond training loop with MMPS E-step (Trend-Seasonal Score)."""

    # Model name and directory
    name = create_model_name_and_dir(args)
    name = f"diffem_mmps_trend_seasonal_score_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print(f"\n{'='*60}")
        print(f"Starting DiffEM-MMPS + Trend-Seasonal Score with {args.em_iters} EM iterations")
        ...
        print("\n" + "="*60)
        print("DiffEM-MMPS + Trend-Seasonal Score Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS + Trend-Seasonal Score training is complete")
...
if __name__ == '__main__':
    ...
    # Trend-seasonal parameters (defaults if not provided via CLI)
    if not hasattr(args, 'sigma_ref') or args.sigma_ref is None:
        args.sigma_ref = 1.0
    if not hasattr(args, 'trend_window') or args.trend_window is None:
        args.trend_window = None  # Auto-detect from image resolution
