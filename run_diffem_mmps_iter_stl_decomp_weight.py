"""
DiffEM with Unconditional Model + MMPS E-step — Iterative-STL Init + Decomp-Weighted Loss

Combines the best initialization (iterative STL from iter_stl_init) with the
best M-step variant (decomposition-weighted loss from decomp_weight):

  Phase 1:  initialize_with_iterative_stl — iterative STL decompose → impute
            cycle that refines missing values over n_iters rounds.
  M-step:   per-pixel importance weighting via MA decomposition.
...
def main(args):
    ...
    name = f"diffem_mmps_iter_stl_decomp_weight_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        # === Phase 2: EM Loop ===
        print(f"\n{'='*60}")
        print(f"Starting DiffEM-MMPS + Iter-STL Init + Decomp-Weighted Loss with {args.em_iters} EM iterations")
        ...
        if final_metrics:
            ...
            for k, v in final_metrics.items():
                print(f"  {k}: {v:.4f}")
        print("\n" + "="*60)
        print("DiffEM-MMPS + Iter-STL Init + Decomp-Weighted Loss Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS + Iter-STL Init + Decomp-Weighted Loss training is complete")
