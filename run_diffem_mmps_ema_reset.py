"""
DiffEM with Unconditional Model + MMPS E-step — EMA Reset variant

Same as run_diffem_mmps.py with ONE change:
  EMA shadow weights are reset to current model weights at the start of each
  EM iteration, preventing stale estimates from earlier (lower-quality) rounds
  from contaminating inference.
"""
...
def main(args):
    ...
    name = f"diffem_mmps_ema_reset_{name}"
