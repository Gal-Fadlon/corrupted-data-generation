"""
DiffEM with MMPS E-step — Iterative-STL Init + Anisotropic Sigma_y in CG

EM scaffolding is shared via utils.diffem_mmps_irregular_em (same as other
spectral MMPS run scripts).

ONLY DIFFERENCE: the MMPS class.  Replaces isotropic sigma_y^2 * I in the
CG linear operator with a frequency-dependent Sigma_y(f) in time-series
space (via img_to_ts / FFT / multiply / irfft / ts_to_img), then adds the
usual sigma_t^2 * mask * J^T(mask * v) term.

Theoretical basis:
  - DiEM Eq. 12-13: Sigma_y may be any positive-definite matrix; here
    we use a diagonal operator in the DFT basis of the reconstructed TS.
  - High frequencies get larger effective observation noise, down-weighting
    unreliable likelihood corrections at high diffusion noise (Wiener /
    SAGD motivation).
"""
...
class SpectralCGMMPSDiffusionProcess:
    """
    MMPS with anisotropic Sigma_y in the CG operator.

    cg_operator(v) = Sigma_y_freq(v) + sigma_t^2 * mask * J^T(mask * v)

    Sigma_y_freq is linear: TS unwrap (img_to_ts), per-bin scaling in rFFT
    domain by sigma_y^2 * (1 + (f/f_c)^order), then wrap (ts_to_img).  This
    matches the DiEM/MMPS story where Sigma_y may be structured PSD; here
    we use a diagonal PSD in the DFT basis of each univariate TS feature.

    CG assumes a symmetric positive-definite operator; the embed/unembed
    composition is only approximately so in image space.  With the default
    mmps_cg_iters=1 the step is robust; for larger cg_iters, treat as
    heuristic.  Differs from spectral_post (post-filter on the correction
    only) whenever cg_iters > 1.
    """
...
def _build_process(args, uncond_model):
    ...
    return SpectralCGMMPSDiffusionProcess(
...
SPEC = IrregularMMPSExperimentSpec(
    wandb_name_prefix="diffem_mmps_spectral_cg",
    build_process_fn=_build_process,
    e_step_title="Spectral CG (anisotropic Sigma_y) MMPS",
    tqdm_desc="E-step Spectral-CG",
    phase2_start_line=(
        "Starting Spectral CG (anisotropic Sigma_y) DiffEM-MMPS with {em_iters} EM iterations"
    ),
    completion_message="Spectral CG DiffEM-MMPS Complete!",
)
