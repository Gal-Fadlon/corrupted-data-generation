"""
DiffEM with MMPS E-step — Plug-and-Play Langevin Correction
...
Unified model that handles all corruption regimes:
  ...
  - Observation noise only (missing_rate=0): E-step actively denoises via Langevin MCMC
  - Missing + noise: imputes and denoises in one pass

Key idea: At each reverse diffusion step, after the unconditional denoiser produces
x_hat via standard MMPS CG solve, run K Langevin MCMC correction steps that combine
the prior score (from the diffusion denoiser) with the likelihood gradient
(data-fidelity term). The likelihood gradient naturally incorporates per-feature
observation noise, so anisotropic noise is handled correctly.

When sigma_obs=0: the likelihood gradient enforces hard data consistency, equivalent
to standard MMPS (no Langevin correction is applied). When sigma_obs>0: the Langevin
steps refine the MMPS output by balancing the learned prior against noisy observations,
with per-channel noise standard deviations used to weight the likelihood correctly.

Based on run_diffem_mmps_bayesian_target.py (which is based on
run_diffem_mmps_iter_stl_decomp_projection_curriculum.py).
"""
...
# =============================================================================
# Plug-and-Play Langevin Decomposition-Projected MMPS
# =============================================================================

class PnPLangevinMMPSDiffusionProcess:
    """
    MMPS sampler with Plug-and-Play Langevin correction steps and decomposition
    consistency projection.

    When sigma_obs=0, this is identical to DecompProjectedMMPSDiffusionProcess
    (standard MMPS with hard observation matching, no Langevin correction).

    When sigma_obs>0, after the standard MMPS CG solve, K Langevin MCMC correction
    steps are applied that combine:
      - Prior score: (denoiser(x) - x) / sigma_t^2
      - Likelihood gradient: -A^T(Ax - y) / sigma_obs^2 (per-channel)

    The Langevin steps refine the MMPS output by balancing the learned prior
    against noisy observations. Per-channel noise standard deviations ensure
    anisotropic observation noise is handled correctly.

    The denoising improves across EM iterations because the model (and thus the
    prior score) gets better each round.
    """
    ...
    def __init__(self, args, diffusion_fn, shape, model,
                 ...
                 langevin_steps=3, langevin_step_size=0.1):
        self.args = args
    ...
    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        """
        Plug-and-Play Langevin posterior denoiser.

        Step 1: Standard MMPS CG solve with per-channel noise awareness.
        Step 2 (sigma_obs>0 only): K Langevin MCMC correction steps combining
                prior score and per-channel likelihood gradient.

        When sigma_obs=0: only Step 1 runs (standard MMPS, hard data consistency).
        """
        sigma_sq = sigma ** 2
        ...
        # Langevin correction steps (only when noise is present)
        if self.sigma_obs > 0:
            x_lang = result.clone()
            for _ in range(self.langevin_steps):
                x_lang_input = x_lang.detach().requires_grad_(True)
                # Prior score via denoiser
                with torch.enable_grad():
                    d = self.net(x_lang_input.float(), sigma.float(), None).to(torch.float64)
                    prior_score = (d - x_lang_input) / sigma_sq

                # Likelihood gradient: -A^T(A*x - y) / sigma_obs^2
                if self.sigma_obs_per_channel is not None:
                    so_sq = (self.sigma_obs_per_channel.to(x_t.device).to(torch.float64) ** 2).reshape(1, -1, 1, 1)
                else:
                    so_sq = self.sigma_obs ** 2
                likelihood_grad = -mask * (x_lang - x_obs) / (so_sq + 1e-10)

                # Langevin step
                step_size = self.langevin_step_size * sigma_sq
                noise = torch.randn_like(x_lang) * (2 * step_size).sqrt()
                x_lang = x_lang.detach() + step_size * (prior_score.detach() + likelihood_grad) + noise

            result = x_lang.detach()
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    print(f"\n=== E-Step (EM iter {em_iter}) — PnP-Langevin MMPS ===")
    uncond_model.eval()
    ...
    with uncond_model.ema_scope():
        process = PnPLangevinMMPSDiffusionProcess(
        ...
        for start_idx in tqdm(range(0, N, batch_size), desc="E-step PnP-Langevin"):
...
def main(args):
    ...
    name = f"pnp_langevin_mmps_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        if sigma_obs > 0:
            print(f"  PnP Langevin correction ACTIVE (sigma_obs={sigma_obs})")
        else:
        ...
        print(f"\n{'='*60}")
        print(f"Starting PnP-Langevin DiffEM-MMPS with {args.em_iters} EM iterations")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("PnP-Langevin DiffEM-MMPS Complete!")
        print("="*60)
        if best_metrics:
