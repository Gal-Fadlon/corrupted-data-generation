"""
Gaussian Noise Handler — Approach 3: Modified Forward Operator

Properly models the observation as y = x + noise with a modified MMPS posterior.
When all positions are observed with noise, the posterior denoiser uses a
noise-aware residual that doesn't force exact match to noisy observations.

The key modification: instead of residual r = y - mask*D(x_t), we use a
weighted residual that blends toward the denoised estimate at high observation
noise, effectively trusting the model's prior more than the noisy observation.

The observation consistency after MMPS also uses soft blending:
  x_final = (σ_obs² * x_mmps + σ_prior² * y_obs) / (σ_obs² + σ_prior²)
instead of hard replacement of observed positions.
...
class DecompProjectedMMPSDiffusionProcess:
    ...
    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        ...
        # For noisy observations: scale down the residual by the noise precision
        # When sigma_obs > 0, we trust the observation less
        # noise_precision = 1 / (sigma_y² + sigma_obs²) — how much to trust observation
        total_obs_var = sigma_y_sq + sigma_obs_sq

        # Residual: difference between observation and predicted observation
        r = x_obs - mask * denoised

        # Scale residual by noise precision (soft consistency)
        if sigma_obs_sq > 0:
            precision_weight = sigma_y_sq / total_obs_var
            r = r * precision_weight

        def cg_operator(v):
            return total_obs_var * v + sigma_sq * mask * vjp_fn(mask * v)
...
def main(args):
    ...
    name = f"diffem_gaussian_noise_forward_op_{name}"
    logging.info(args)
