"""
DiffEM with MMPS E-step — Posterior Variance Weighting

Use the MMPS Jacobian to estimate per-pixel posterior variance. Where model is
confident (low variance), trust the model's denoised estimate. Where uncertain
(high variance), trust the observation. This is adaptive soft replacement
computed from the actual posterior at each diffusion step. When sigma_obs=0:
observation weight at observed positions -> 1 (hard replacement, unchanged).
...
# =============================================================================
# Posterior Variance Decomposition-Projected MMPS
# =============================================================================

class PosteriorVarianceMMPSDiffusionProcess:
    """
    MMPS sampler with posterior-variance-weighted observation blending and
    ...
    When sigma_obs>0, after the standard MMPS CG solve, the result at observed
    positions is blended between the model prediction and the observation using
    weights derived from the estimated per-pixel posterior variance (computed
    via the MMPS Jacobian diagonal). Where the model is confident (low posterior
    variance), trust the model; where uncertain (high variance), trust the
    observation.
    """
    ...
    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        """
        Posterior-variance-weighted denoiser.

        Standard MMPS CG solve, then apply posterior-variance-weighted blending
        at observed positions. Where model is confident (low posterior variance),
        trust the model's denoised estimate. Where uncertain (high variance),
        trust the observation.

        When sigma_obs=0: no blending (hard replacement, unchanged).
        """
        sigma_sq = sigma ** 2
        ...
        # Posterior variance weighting at observed positions
        if self.sigma_obs > 0:
            # Estimate per-pixel posterior variance using Jacobian diagonal
            # Probe: compute J * e_i for a random direction to estimate J_ii
            probe = torch.randn_like(denoised)
            J_probe = vjp_fn(probe).detach()
            # Approximate diagonal: |J_probe| / |probe| gives rough scale of J_ii
            # Posterior variance ≈ sigma_t^2 * (1 - J_ii)
            J_diag_approx = (J_probe * probe).abs() / (probe * probe + 1e-10)
            J_diag_approx = J_diag_approx.clamp(0, 1)
            posterior_var = sigma_sq * (1.0 - J_diag_approx)

            # Weighting: w_model = sigma_obs^2 / (sigma_obs^2 + posterior_var)
            #            w_obs   = posterior_var / (sigma_obs^2 + posterior_var)
            w_model = sigma_obs_sq / (sigma_obs_sq + posterior_var + 1e-10)
            w_obs = posterior_var / (sigma_obs_sq + posterior_var + 1e-10)

            # At observed positions: blend model prediction and observation
            result = mask * (w_model * result + w_obs * x_obs) + (1 - mask) * result
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    print(f"\n=== E-Step (EM iter {em_iter}) — Posterior Variance MMPS ===")
    uncond_model.eval()
    ...
    with uncond_model.ema_scope():
        process = PosteriorVarianceMMPSDiffusionProcess(
        ...
        for start_idx in tqdm(range(0, N, batch_size), desc="E-step Posterior-Variance"):
...
def main(args):
    ...
    name = f"posterior_variance_mmps_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        if sigma_obs > 0:
            print(f"  Posterior variance weighting ACTIVE (sigma_obs={sigma_obs})")
        else:
        ...
        print(f"\n{'='*60}")
        print(f"Starting Posterior-Variance DiffEM-MMPS with {args.em_iters} EM iterations")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("Posterior-Variance DiffEM-MMPS Complete!")
        print("="*60)
        if best_metrics:
