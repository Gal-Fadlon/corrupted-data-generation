"""
DiffEM with MMPS E-step — EMA Reset + STL Init + Curriculum Missing Rate

Fork of run_diffem_mmps_ema_reset_stl_init.py with ONE change:
  In early EM iterations, randomly reveal extra observations (from last recon)
  to make the problem easier.  Gradually increase to the real missing rate
  over 60% of EM iterations.  Prevents EM from getting stuck in bad basins.
"""
...
def main(args):
    ...
    name = f"diffem_mmps_stl_curriculum_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print("\n" + "="*60)
        print("DiffEM-MMPS + Curriculum Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS + Curriculum training is complete")
