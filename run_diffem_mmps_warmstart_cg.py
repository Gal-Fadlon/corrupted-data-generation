...
Instead of initializing CG at x0 = 0, warm-start with a temporal
interpolation-based estimate.  Use the observed points to create a smart
initial guess for the CG solver, enabling better convergence with the same
...
# =============================================================================
# Warm-Start Conjugate Gradient Solver
# =============================================================================

def conjugate_gradient_warmstart(A_fn, b, x0, maxiter=1):
    """
    Conjugate gradient solver with warm-start initial guess.

    Instead of starting from x = 0, uses an initial guess x0 derived from
    a diagonal preconditioner estimate.  This gives a better starting residual
    and enables faster convergence with the same iteration budget.

    Args:
        A_fn: callable, linear operator v -> A·v
        b: right-hand side tensor
        x0: initial guess tensor (same shape as b)
        maxiter: number of CG iterations (default=1)
    ...
    """
    x = x0.clone()
    r = b - A_fn(x)  # residual with warm start
    p = r.clone()
...
# =============================================================================
# MMPSDiffusionProcess — Warm-Start CG variant
# =============================================================================
...
class MMPSDiffusionProcess:
    ...
    This variant warm-starts the CG solver with a diagonal preconditioner
    estimate v0 = r / (sigma_y^2 + sigma^2) instead of x0 = 0, enabling
    better convergence in the same number of iterations.
    """
    ...
    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        """
        Posterior denoiser with warm-start CG.

        Instead of initializing CG at x0 = 0, we compute a diagonal
        preconditioner estimate: v0 = r / (sigma_y^2 + sigma^2).
        This is the exact solution when the Jacobian is the identity,
        providing a good warm start for real problems.
        ...
        # Warm-start: diagonal preconditioner estimate
        # v0 = r / (sigma_y^2 + sigma^2) approximates the solution when J ≈ I
        v0 = r / (sigma_y_sq + sigma_sq)

        # Solve with warm-start CG
        v = conjugate_gradient_warmstart(cg_operator, r, v0, maxiter=self.cg_iters)
...
def main(args):
    ...
    name = f"diffem_mmps_warmstart_cg_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print("\n" + "="*60)
        print("DiffEM-MMPS + Warm-Start CG Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS + Warm-Start CG training is complete")
