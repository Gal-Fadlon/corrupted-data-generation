"""
DiffEM with Unconditional Model + MMPS E-step — EMA Reset + STL Warm-Start

Same as run_diffem_mmps_ema_reset.py with ONE change:
  Gaussian initialization is replaced with STL-informed initialization.
  Missing values are filled using interpolated trend + seasonal from STL,
  with re-sampled residuals — giving a structurally richer starting point
  than pure Gaussian fills.
"""
...
def main(args):
    ...
    name = f"diffem_mmps_ema_reset_stl_init_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print("\n" + "="*60)
        print("DiffEM-MMPS + STL Warm-Start Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS + STL Warm-Start training is complete")
