"""
DiffEM Enhanced MMPS (Approach C)

Enhanced version of run_diffem_mmps.py with:
  1. PPCA initialization (replaces naive Gaussian init)
  2. Optimizer reset between EM laps (prevents stale momentum)
  3. EM early stopping (patience=2 on disc_mean, restores best checkpoint)
  4. Configurable e_step_sample_steps (default 64, decoupled from diffusion_steps)
  5. TF32 support for ~1.3x speedup on Ampere+ GPUs
"""
...
def initialize_with_gaussian(corrupted_data, obs_masks, seed=None):
    """Fallback Gaussian initialization."""
    if seed is not None:
        np.random.seed(seed)
...
def initialize_with_ppca(corrupted_data, obs_masks, rank=32, n_iters=8, seed=None):
    """
    PPCA-based initialization: fit a low-rank Gaussian (W, sigma^2) to observed
    data via EM, then sample missing values from the PPCA conditional posterior.
    Falls back to Gaussian init if PPCA fails numerically.
    """
    if seed is not None:
        np.random.seed(seed)
    ...
    flat_data = corrupted_data.reshape(N, D)
    flat_mask = np.repeat(obs_masks[:, :, np.newaxis], features, axis=2).reshape(N, D)

    mu = np.nanmean(flat_data, axis=0)
    mu = np.where(np.isnan(mu), 0.5, mu)

    try:
        W = np.random.randn(D, rank).astype(np.float64) * 0.01
        sigma2 = 0.1

        for it in range(n_iters):
            WtW = W.T @ W
            M_inv = np.linalg.inv(sigma2 * np.eye(rank) + WtW)

            sum_Ez = np.zeros((D, rank), dtype=np.float64)
            sum_EzzT = np.zeros((rank, rank), dtype=np.float64)
            sum_xxT_diag = np.zeros(D, dtype=np.float64)
            total_obs = 0

            for i in range(N):
                obs_idx = flat_mask[i] > 0.5
                n_obs = obs_idx.sum()
                if n_obs < 2:
                    continue

                x_obs = flat_data[i, obs_idx] - mu[obs_idx]
                W_obs = W[obs_idx, :]

                C_obs_inv_part = W_obs.T @ W_obs + sigma2 * np.eye(rank)
                C_obs_inv_part_inv = np.linalg.inv(C_obs_inv_part)

                Ez = C_obs_inv_part_inv @ (W_obs.T @ x_obs)
                EzzT = sigma2 * C_obs_inv_part_inv + np.outer(Ez, Ez)

                sum_Ez[obs_idx, :] += np.outer(x_obs, Ez)
                sum_EzzT += EzzT
                sum_xxT_diag[obs_idx] += x_obs ** 2
                total_obs += n_obs

            if total_obs < rank:
                raise ValueError("Too few observations for PPCA")

            W_new = sum_Ez @ np.linalg.inv(sum_EzzT + 1e-6 * np.eye(rank))
            sigma2_new = (sum_xxT_diag.sum() -
                          np.trace(sum_Ez.T @ W_new)) / max(total_obs, 1)
            sigma2_new = max(sigma2_new, 1e-6)

            W = W_new
            sigma2 = sigma2_new

            print(f"  PPCA iter {it+1}/{n_iters}: sigma2={sigma2:.6f}")
        ...
        for i in range(N):
            ...
            if not miss_idx.any():
                continue
            if not obs_idx.any():
                initial_recon[i] = np.random.normal(mu, np.sqrt(sigma2)).reshape(seq_len, features)
                continue
            ...
            C_obs_inv_part = np.linalg.inv(W_obs.T @ W_obs + sigma2 * np.eye(rank))
            Ez = C_obs_inv_part @ (W_obs.T @ x_obs)

            x_miss_mean = mu[miss_idx] + W_miss @ Ez
            x_miss_std = np.sqrt(sigma2) * 0.1
            x_miss = x_miss_mean + np.random.randn(miss_idx.sum()) * x_miss_std

            flat_recon = flat_data[i].copy()
            flat_recon[miss_idx] = x_miss
            initial_recon[i] = flat_recon.reshape(seq_len, features)

        initial_recon = np.clip(initial_recon, 0, 1)
        initial_recon = np.nan_to_num(initial_recon, nan=0.5)
        print(f"PPCA initialization complete (rank={rank}). Generated {N} sequences.")
        return initial_recon

    except Exception as e:
        print(f"PPCA failed ({e}), falling back to Gaussian init")
        return initialize_with_gaussian(corrupted_data, obs_masks, seed=seed)
...
def main(args):
    """Main DiffEM-MMPS Enhanced training loop."""
    ...
    name = f"diffem_mmps_enhanced_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        ppca_rank = getattr(args, 'ppca_rank', 32)
        ppca_iters = getattr(args, 'ppca_iters', 8)
        use_ppca = getattr(args, 'use_ppca_init', True)

        print(f"\n{'='*60}")
        if use_ppca:
            print(f"Phase 1: PPCA Initialization (rank={ppca_rank}, iters={ppca_iters})")
        else:
        ...
        if use_ppca:
            initial_reconstructions = initialize_with_ppca(
                ...
                rank=ppca_rank, n_iters=ppca_iters, seed=args.seed,
            )
        else:
        ...
        # === Phase 1.5: Train on initial reconstructions ===
        print(f"\n{'='*60}")
        ...
        # === Phase 2: EM Loop with optimizer reset + early stopping ===
        e_step_steps = getattr(args, 'e_step_sample_steps', 64)

        print(f"\n{'='*60}")
        print(f"Starting DiffEM-MMPS Enhanced with {args.em_iters} EM iterations")
        ...
        print(f"  Optimizer reset each lap: ON")
        ...
        for em_iter in range(args.em_iters):
            ...
            # Optimizer reset: fresh AdamW state each lap
            optimizer = torch.optim.AdamW(
                uncond_model.parameters(),
                lr=args.learning_rate,
                weight_decay=args.weight_decay
            )
            print("  Optimizer reset (fresh AdamW state)")
        ...
        print("\n" + "="*60)
        print("DiffEM-MMPS Enhanced Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS Enhanced training is complete")
