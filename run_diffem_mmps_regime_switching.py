"""
DiffEM with MMPS E-step — Regime-Switching Posterior Denoiser
...
Unified model that handles all corruption regimes:
  ...
  - Observation noise only (missing_rate=0): E-step switches between MMPS and unconditional
  - Missing + noise: regime-switching blend of MMPS and unconditional denoiser

Key idea: At each reverse diffusion step, compare the current noise level sigma_t
against the mean observation noise sigma_obs. This determines a regime:

  - When sigma_t > sigma_obs (model uncertain, observations cleaner):
    Standard MMPS — observations provide useful signal for conditioning.
  - When sigma_t < sigma_obs (model confident, observations noisier):
    Switch to pure unconditional denoiser — observations would corrupt the result.

The transition between regimes is smoothed via a sigmoid blend:
    w_mmps = sigmoid((sigma_t - mean_sigma_obs) / (temperature * mean_sigma_obs))

When sigma_obs=0: sigma_t > 0 always, so w_mmps ~ 1 and standard MMPS is used
throughout (unchanged behavior).

KEY ANISOTROPIC NOISE FIX: Uses per-feature noise std via sigma_obs_per_channel,
so that each feature dimension can have a different observation noise level.

Based on run_diffem_mmps_bayesian_target.py.
"""
...
from utils.utils_data import gen_dataloader, compute_per_feature_noise_std
...
# =============================================================================
# Regime-Switching Decomposition-Projected MMPS
# =============================================================================

class RegimeSwitchingMMPSDiffusionProcess:
    """
    MMPS sampler with regime-switching posterior denoiser and decomposition
    consistency projection.

    At each reverse diffusion step, the sampler compares sigma_t (current
    diffusion noise level) against the mean observation noise sigma_obs to
    decide whether observations are informative:

      - sigma_t >> sigma_obs: observations are cleaner than the model's current
        estimate, so full MMPS conditioning is applied.
      - sigma_t << sigma_obs: the model is more confident than the observations,
        so the unconditional denoiser is used instead.

    A sigmoid blend ensures a smooth transition between regimes:
        w_mmps = sigmoid((sigma_t - mean_sigma_obs) / (temperature * mean_sigma_obs))

    When sigma_obs=0 (pure imputation), sigma_t > 0 always, so this reduces
    to standard MMPS throughout.

    Uses per-feature (anisotropic) observation noise via sigma_obs_per_channel.
    """

    def __init__(self, args, diffusion_fn, shape, model,
                 sigma_y=0.01, cg_iters=1,
                 projection_strength=0.5, sigma_threshold_frac=0.5,
                 sigma_obs_per_channel=None, switch_temperature=0.1):
        self.args = args
        ...
        self.sigma_obs_per_channel = sigma_obs_per_channel
    ...
    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        """
        Regime-switching posterior denoiser.

        When sigma_obs=0: standard MMPS (full conditioning throughout).
        When sigma_obs>0: blend MMPS result with unconditional denoiser based
        on the ratio of sigma_t to mean_sigma_obs:
            w_mmps = sigmoid((sigma_t - mean_sigma_obs) / (temperature * mean_sigma_obs))
        """
        sigma_sq = sigma ** 2
        ...
        # Compute mean sigma_obs for regime comparison
        if self.sigma_obs_per_channel is not None:
            mean_sigma_obs = self.sigma_obs_per_channel.mean().item()
            sigma_obs_sq = (self.sigma_obs_per_channel.to(x_t.device).to(torch.float64) ** 2).reshape(1, -1, 1, 1)
        else:
            mean_sigma_obs = 0.0
            sigma_obs_sq = 0.0

        if mean_sigma_obs == 0:
            # Pure imputation: full MMPS
            ...
            # Regime switching: blend MMPS result with unconditional denoiser
            # w = sigmoid((sigma_t - sigma_obs) / temperature)
            # w close to 1 when sigma_t >> sigma_obs: trust MMPS (observations useful)
            # w close to 0 when sigma_t << sigma_obs: trust unconditional (observations noisy)
            sigma_t_val = float(sigma)
            w_mmps = 1.0 / (1.0 + np.exp(-(sigma_t_val - mean_sigma_obs) / (self.switch_temperature * mean_sigma_obs + 1e-10)))
            ...
            if w_mmps > 0.01:
                ...
                v = conjugate_gradient(cg_operator, r, maxiter=self.cg_iters)
                score = vjp_fn(mask * v)
                mmps_result = denoised.detach() + sigma_sq * score.detach()
            else:
                mmps_result = denoised.detach()

            uncond_result = denoised.detach()
            result = w_mmps * mmps_result + (1 - w_mmps) * uncond_result
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    print(f"\n=== E-Step (EM iter {em_iter}) — Regime-Switching MMPS ===")
    uncond_model.eval()
    ...
    # Compute per-feature noise std for anisotropic noise handling
    if sigma_obs > 0:
        sigma_obs_per_channel = compute_per_feature_noise_std(corrupted_data, obs_masks)
        sigma_obs_per_channel = torch.tensor(sigma_obs_per_channel, dtype=torch.float32)
        print(f"  Per-feature noise std: mean={sigma_obs_per_channel.mean().item():.4f}, "
              f"min={sigma_obs_per_channel.min().item():.4f}, max={sigma_obs_per_channel.max().item():.4f}")
    else:
        sigma_obs_per_channel = None

    with uncond_model.ema_scope():
        process = RegimeSwitchingMMPSDiffusionProcess(
            args, uncond_model.net, target_shape, uncond_model,
            sigma_y=sigma_y, cg_iters=cg_iters,
            projection_strength=0.5, sigma_threshold_frac=0.5,
            sigma_obs_per_channel=sigma_obs_per_channel
        )

        for start_idx in tqdm(range(0, N, batch_size), desc="E-step Regime-Switch"):
...
def main(args):
    ...
    name = f"regime_switching_mmps_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        if sigma_obs > 0:
            print(f"  Regime-switching denoiser ACTIVE (sigma_obs={sigma_obs})")
        else:
        ...
        print(f"\n{'='*60}")
        print(f"Starting Regime-Switching DiffEM-MMPS with {args.em_iters} EM iterations")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("Regime-Switching DiffEM-MMPS Complete!")
        print("="*60)
        if best_metrics:
