"""
DiffEM with Unconditional Model + MMPS E-step — GMRF Smoothness Prior

Same as run_diffem_mmps_ema_reset_iter_stl_init.py with ONE enhancement:
  A Gaussian Markov Random Field (GMRF) smoothness prior is injected into the
  MMPS reverse diffusion loop. After each posterior_denoise call, the denoised
  estimate is converted to time-series space, a discrete Laplacian [1, -2, 1]
  is applied along the temporal axis (per feature), and the resulting gradient
  is projected back to image space and added as a score-level correction:

      denoised += sigma^2 * lambda * ts_to_img(conv1d(img_to_ts(denoised), [1,-2,1]))

  This penalises first-order roughness sum(x[t+1] - x[t])^2 in the time
  domain, producing temporally coherent imputations without altering the
  VJP / CG internals of MMPS.
"""
...
# =============================================================================
# MMPSDiffusionProcess with GMRF smoothness prior
# =============================================================================

class MMPSDiffusionProcess:
    """
    MMPS-style diffusion sampler with a GMRF temporal smoothness prior.

    Identical to the vanilla MMPS posterior denoiser (covariance correction via
    Tweedie's Jacobian + CG), but after each posterior_denoise call the denoised
    estimate is regularised by a discrete-Laplacian smoothness term applied in
    the native time-series domain:

        denoised += sigma^2 * lambda * ts_to_img(Laplacian(img_to_ts(denoised)))

    where Laplacian = conv1d([1, -2, 1]) along the temporal axis.
    """

    def __init__(self, args, diffusion_fn, shape, sigma_y=0.01, cg_iters=1,
                 smooth_lambda=0.0, ts_to_img_fn=None, img_to_ts_fn=None):
        self.args = args
        ...
        # GMRF smoothness prior
        self.smooth_lambda = smooth_lambda
        self.ts_to_img_fn = ts_to_img_fn
        self.img_to_ts_fn = img_to_ts_fn

        # Laplacian kernel [1, -2, 1] — created once, reused every step.
        # Shape (C, 1, 3) for groups=C depthwise conv along the time axis.
        n_features = shape[0]  # input_channels
        self._lap_kernel = torch.tensor(
            [[[1.0, -2.0, 1.0]]], dtype=torch.float64, device=self.device
        ).repeat(n_features, 1, 1)  # (C, 1, 3)

    # -----------------------------------------------------------------
    # GMRF correction (applied on detached tensors — no autograd impact)
    # -----------------------------------------------------------------

    def _apply_gmrf_correction(self, denoised, sigma_sq):
        """
        Add the GMRF smoothness score to *denoised* in-place (detached).

        Steps
        -----
        1. img_to_ts  → (B, T, C)
        2. conv1d([1,-2,1]) per feature along T  → Laplacian (B, T, C)
        3. ts_to_img  → back to (B, C, H, W)
        4. denoised += sigma^2 * lambda * laplacian_img

        Returns the corrected denoised tensor.
        """
        if self.smooth_lambda == 0.0 or self.ts_to_img_fn is None:
            return denoised

        # 1. Convert image-space denoised estimate to time-series space
        denoised_ts = self.img_to_ts_fn(denoised)          # (B, T, C)

        # 2. Apply discrete Laplacian per-feature along time axis
        #    F.conv1d expects (B, C, T) input.
        #    Replicate-pad boundaries (Neumann BC) so endpoints get the correct
        #    one-sided difference instead of being pulled toward zero.
        ts_bct = denoised_ts.transpose(1, 2)                # (B, C, T)
        ts_bct_padded = F.pad(ts_bct, (1, 1), mode='replicate')
        lap_bct = F.conv1d(
            ts_bct_padded, self._lap_kernel,
            padding=0, groups=ts_bct.shape[1],
        )                                                    # (B, C, T)
        lap_ts = lap_bct.transpose(1, 2)                     # (B, T, C)

        # 3. Project the TS-space Laplacian back to image space
        smooth_img = self.ts_to_img_fn(lap_ts)               # (B, C, H, W)

        # 4. Score-level correction: denoised += sigma^2 * lambda * Delta(x)
        return denoised + sigma_sq * self.smooth_lambda * smooth_img.to(denoised.dtype)

    # -----------------------------------------------------------------
    # MMPS posterior denoiser (unchanged from vanilla MMPS)
    # -----------------------------------------------------------------
    ...
    # -----------------------------------------------------------------
    # MMPS sampling loop — GMRF correction injected after each denoise
    # -----------------------------------------------------------------
    ...
    def mmps_sample(self, latents, x_obs_img, mask_img):
        ...
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):
            ...
            # Euler step — posterior denoise + GMRF correction
            denoised = self.posterior_denoise(x_hat, t_hat, x_obs, mask)
            denoised = self._apply_gmrf_correction(denoised, t_hat ** 2)
            d_cur = (x_hat - denoised) / t_hat
            x_next = x_hat + (t_next - t_hat) * d_cur

            # Heun corrector — posterior denoise + GMRF correction
            if i < self.num_steps - 1:
                denoised_2 = self.posterior_denoise(x_next, t_next, x_obs, mask)
                denoised_2 = self._apply_gmrf_correction(denoised_2, t_next ** 2)
    ...
    def sampling(self, sampling_number=16):
        """Unconditional sampling (no GMRF) — used for evaluation only."""
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    print(f"\n=== E-Step (EM iter {em_iter}) — MMPS + GMRF Imputation ===")
    print("Imputing missing values via MMPS with GMRF smoothness prior...")
    ...
    smooth_lambda = getattr(args, 'gmrf_lambda', 0.1)
    ...
    with uncond_model.ema_scope():
        ...
        for start_idx in tqdm(range(0, N, batch_size), desc="E-step MMPS+GMRF"):
...
def main(args):
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        # === Phase 2: EM Loop ===
        print(f"\n{'='*60}")
        print(f"Starting DiffEM-MMPS + GMRF with {args.em_iters} EM iterations")
        print(f"  MMPS sigma_y: {args.mmps_sigma_y}")
        print(f"  MMPS cg_iters: {args.mmps_cg_iters}")
        print(f"  GMRF lambda:  {getattr(args, 'gmrf_lambda', 0.1)}")
        print(f"{'='*60}")
        ...
        if final_metrics:
            ...
            for k, v in final_metrics.items():
                print(f"  {k}: {v:.4f}")
        print("\n" + "="*60)
        print("DiffEM-MMPS + GMRF Smoothness Prior Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS + GMRF training is complete")
