"""
DiffEM with MMPS E-step — Consensus Multi-Sample

Run MMPS K times with different random initializations and average.
Signal is consistent across samples; noise is not.
When sigma_obs=0: reduces posterior variance for imputation but main benefit
is for noise.
...
# =============================================================================
# Consensus Multi-Sample Decomposition-Projected MMPS
# =============================================================================

class ConsensusMMPSDiffusionProcess:
    """
    MMPS sampler that runs K times with different random initializations and
    averages the results. Signal is consistent across samples; noise is not.
    ...
    def __init__(self, args, diffusion_fn, shape, model,
                 ...
                 sigma_obs=0.0, num_consensus_samples=3):
        self.args = args
        ...
        self.num_consensus_samples = num_consensus_samples
    ...
    def sampling_mmps(self, x_obs_img, mask_img):
        ...
        all_samples = []
        for k in range(self.num_consensus_samples):
            latents = torch.randn([batch_size, *self.shape], device=self.device)
            sample = self.mmps_sample(latents, x_obs_img, mask_img)
            all_samples.append(sample)
        # Average across K samples — signal preserved, noise cancels
        return torch.stack(all_samples, dim=0).mean(dim=0)
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    print(f"\n=== E-Step (EM iter {em_iter}) — Consensus Multi-Sample MMPS ===")
    uncond_model.eval()
    ...
    num_consensus = 3 if sigma_obs > 0 else 1
    print(f"  Using {num_consensus} consensus samples")

    with uncond_model.ema_scope():
        process = ConsensusMMPSDiffusionProcess(
            args, uncond_model.net, target_shape, uncond_model,
            sigma_y=sigma_y, cg_iters=cg_iters,
            projection_strength=0.5, sigma_threshold_frac=0.5,
            sigma_obs=sigma_obs, num_consensus_samples=num_consensus
        )

        for start_idx in tqdm(range(0, N, batch_size), desc="E-step Consensus Multi-Sample"):
...
def main(args):
    ...
    name = f"consensus_multisample_mmps_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        if sigma_obs > 0:
            print(f"  Consensus multi-sample ACTIVE (sigma_obs={sigma_obs})")
        else:
        ...
        print(f"\n{'='*60}")
        print(f"Starting Consensus Multi-Sample DiffEM-MMPS with {args.em_iters} EM iterations")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("Consensus Multi-Sample DiffEM-MMPS Complete!")
        print("="*60)
        if best_metrics:
