"""
DiffEM with Unconditional Model + MMPS E-step — EMA Reset + STL Consensus E-Step

Same as run_diffem_mmps_ema_reset_stl_init.py with ONE enhancement:
  The E-step generates K MMPS samples per series and combines them via
  STL-aligned consensus:
    1. Generate K imputations per batch (different latent noise).
    2. STL-decompose each imputation.
    3. Average the K trends and K seasonals (structural components should
       be consistent across samples).
    4. For residuals at missing positions: use the median across K samples.
    5. Reconstruct: avg_trend + avg_seasonal + median_residual.
  This reduces E-step variance in the structural components while
  preserving realistic noise levels.

  New parameter: consensus_k (default 3), accessed via getattr.
"""
...
# =============================================================================
# STL Consensus Combiner (NEW)
# =============================================================================

def consensus_combine(samples_list, corrupted_data_batch, obs_mask_batch, period=None):
    """
    Combine K MMPS imputations via STL-aligned averaging.

    Structural components (trend, seasonal) are averaged across samples for
    low-variance estimates.  Residuals at missing positions use the median
    across samples.  Observed positions are hard-replaced with original values.

    Args:
        samples_list: list of K numpy arrays, each (batch, T, C)
        corrupted_data_batch: (batch, T, C) with NaN for missing
        obs_mask_batch: (batch, T) boolean, True = observed
        period: STL period (auto-detected if None)

    Returns:
        consensus: (batch, T, C) consensus reconstruction
    """
    K = len(samples_list)
    B, T, C = samples_list[0].shape
    if period is None:
        period = auto_detect_period(T)

    consensus = np.zeros((B, T, C), dtype=np.float64)

    for i in range(B):
        mask_i = obs_mask_batch[i]
        obs_idx = np.where(mask_i)[0]
        mis_idx = np.where(~mask_i)[0]

        for c in range(C):
            if len(obs_idx) < 3 or len(mis_idx) == 0:
                # Not enough observations for STL — just average the raw samples
                stacked = np.stack([samples_list[k][i, :, c] for k in range(K)])
                consensus[i, :, c] = np.median(stacked, axis=0)
                continue

            all_trends = np.zeros((K, T))
            all_seasons = np.zeros((K, T))
            all_resids = np.zeros((K, T))

            for k in range(K):
                series_k = samples_list[k][i, :, c]
                trend_k, season_k, resid_k = stl_decompose_single(series_k, period)
                all_trends[k] = trend_k
                all_seasons[k] = season_k
                all_resids[k] = resid_k

            avg_trend = all_trends.mean(axis=0)
            avg_season = all_seasons.mean(axis=0)

            # For residuals: use median across K samples (robust to outliers)
            median_resid = np.median(all_resids, axis=0)

            consensus[i, :, c] = avg_trend + avg_season + median_resid

        # Observation consistency: hard-replace observed positions
        for c in range(C):
            obs_vals = corrupted_data_batch[i, obs_idx, c]
            valid = ~np.isnan(obs_vals)
            consensus[i, obs_idx[valid], c] = obs_vals[valid]

    consensus = np.clip(consensus, 0, 1).astype(np.float32)
    return consensus
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    """
    E-step: Impute missing values using K MMPS samples with consensus combining.

    Generates K independent MMPS imputations per batch, then combines them via
    STL-aligned averaging (trend/seasonal averaged, residual median).
    ...
    """
    consensus_k = getattr(args, 'consensus_k', 3)

    print(f"\n=== E-Step (EM iter {em_iter}) — MMPS Consensus (K={consensus_k}) ===")
    print(f"Generating {consensus_k} MMPS samples per batch and combining via STL consensus...")
    ...
    with uncond_model.ema_scope():
        ...
        for start_idx in tqdm(range(0, N, batch_size), desc="E-step MMPS consensus"):
            ...
            # Generate K MMPS samples
            samples_ts_list = []
            for k_idx in range(consensus_k):
                x_img_imputed = process.sampling_mmps(x_obs_img, mask_img)
                x_ts_recon = uncond_model.img_to_ts(x_img_imputed)
                samples_ts_list.append(x_ts_recon.cpu().numpy())
                torch.cuda.empty_cache()

            # Combine via STL consensus
            consensus_recon = consensus_combine(
                samples_ts_list, corrupted_batch, mask_batch
            )
            all_reconstructions.append(consensus_recon)

    reconstructions = np.vstack(all_reconstructions)
    print(f"E-step complete. Imputed {len(reconstructions)} sequences via {consensus_k}-sample consensus.")
    ...
    if logger is not None:
        ...
        logger.log('em/consensus_k', consensus_k, em_iter)
...
def main(args):
    """Main DiffEM-Uncond training loop with MMPS consensus E-step."""

    # Model name and directory
    name = create_model_name_and_dir(args)
    consensus_k = getattr(args, 'consensus_k', 3)
    name = f"diffem_mmps_ema_reset_stl_consensus_k{consensus_k}_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        # === Phase 2: EM Loop with Consensus E-Step ===
        print(f"\n{'='*60}")
        ...
        for em_iter in range(args.em_iters):
            ...
            # E-step: Multi-sample MMPS with consensus combining
            ...
            # M-step: Train unconditional model on consensus-imputed data
            uncond_model = m_step(
                args, uncond_model, optimizer,
        ...
        print("\n" + "="*60)
        print("DiffEM-MMPS + STL Consensus Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS + STL Consensus training is complete")
...
if __name__ == '__main__':
    ...
    # Consensus-specific defaults (can be overridden via CLI if added to args parser)
    if not hasattr(args, 'consensus_k'):
        args.consensus_k = 3
