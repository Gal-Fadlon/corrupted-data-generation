"""
DiffEM with MMPS E-step — Iterative Refinement (Multi-Pass MMPS)

Within a single E-step, run MMPS P=2-3 times. Pass 1 conditions on raw y_obs.
Pass 2 conditions on a blend of MMPS output and y_obs (cleaner target). Each pass
uses the same model, same mask. A mixing parameter α blends current output with
original y to prevent drift. When σ_obs=0: observations are clean, so subsequent
passes are near-identity (unchanged).
...
# =============================================================================
# Iterative Refinement Decomposition-Projected MMPS
# =============================================================================

class IterativeRefinementMMPSDiffusionProcess:
    """
    MMPS sampler with iterative multi-pass refinement and decomposition
    consistency projection.

    When sigma_obs=0, this runs a single MMPS pass (standard behavior).

    When sigma_obs>0, multiple refinement passes are performed within each
    E-step call. Each subsequent pass conditions on a blend of the previous
    MMPS output and the original observations, progressively cleaning the
    conditioning target.
    """
    ...
    def __init__(self, args, diffusion_fn, shape, model,
                 ...
                 sigma_obs=0.0,
                 num_refinement_passes=2, refinement_alpha=0.7):
        self.args = args
    ...
    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        """
        Standard MMPS posterior denoiser.
        """
        sigma_sq = sigma ** 2
    ...
    def sampling_mmps(self, x_obs_img, mask_img):
        ...
        original_obs = x_obs_img.clone()
        current_obs = x_obs_img.clone()
        mask = mask_img

        num_passes = self.num_refinement_passes if self.sigma_obs > 0 else 1

        for p in range(num_passes):
            latents = torch.randn([batch_size, *self.shape], device=self.device)
            result = self.mmps_sample(latents, current_obs, mask)

            if p < num_passes - 1 and self.sigma_obs > 0:
                # Blend MMPS output with original obs for next pass
                # At observed positions: use refined observation
                result_float = result.float()
                original_float = original_obs.float()
                mask_float = mask.float()
                refined = self.refinement_alpha * result_float + (1 - self.refinement_alpha) * original_float
                current_obs = (mask_float * refined + (1 - mask_float) * result_float).to(x_obs_img.dtype)
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    print(f"\n=== E-Step (EM iter {em_iter}) — Iterative Refinement MMPS ===")
    uncond_model.eval()
    ...
    num_passes = 2 if sigma_obs > 0 else 1
    print(f"  Using {num_passes} refinement passes")

    with uncond_model.ema_scope():
        process = IterativeRefinementMMPSDiffusionProcess(
            args, uncond_model.net, target_shape, uncond_model,
            sigma_y=sigma_y, cg_iters=cg_iters,
            projection_strength=0.5, sigma_threshold_frac=0.5,
            sigma_obs=sigma_obs,
            num_refinement_passes=num_passes, refinement_alpha=0.7
        )

        for start_idx in tqdm(range(0, N, batch_size), desc="E-step Iterative Refinement"):
...
def main(args):
    ...
    name = f"iterative_refinement_mmps_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        if sigma_obs > 0:
            print(f"  Iterative refinement ACTIVE (sigma_obs={sigma_obs})")
        else:
        ...
        print(f"\n{'='*60}")
        print(f"Starting Iterative Refinement DiffEM-MMPS with {args.em_iters} EM iterations")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("Iterative Refinement DiffEM-MMPS Complete!")
        print("="*60)
        if best_metrics:
