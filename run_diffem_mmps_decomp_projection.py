"""
DiffEM with MMPS E-step — Decomposition Consistency Projection

Fork of run_diffem_mmps_stl_curriculum.py with one key change:
  After each MMPS posterior denoising step, the denoised estimate is
  projected through a decomposition-recomposition that enforces
  structural constraints: trend is smoothed, seasonal periodicity is
  preserved.  The projection strength depends on sigma (strong at
  high noise, vanishes at low noise) so fine details are not suppressed.

  Uses fast_decompose_batch (vectorized moving-average + periodic
  averaging) rather than iterative STL, keeping overhead manageable.
"""
...
def main(args):
    ...
    name = f"diffem_mmps_decomp_projection_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print(f"\n{'='*60}")
        print(f"Starting Decomp-Projected DiffEM-MMPS with {args.em_iters} EM iterations")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("Decomp-Projected DiffEM-MMPS Complete!")
        print("="*60)
        if best_metrics:
