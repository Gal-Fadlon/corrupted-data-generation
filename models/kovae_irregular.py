...
class VKDecoder(nn.Module):
    def __init__(self, args):
        ...
        self.linear = nn.Linear(self.hidden_dim * 2, self.args.inp_dim)

    def forward(self, z):
        # decode
        h, _ = self.rnn(z)
        x_rec = nn.functional.sigmoid(self.linear(h))
        return x_rec
...
class VK(nn.Module):
    ...
    def forward(self, time, train_coeffs, final_index):
        ...
        # ------------- ENCODING PART -------------
        z = self.encoder(time, train_coeffs, final_index)
        ...
        # # ------------- PRIOR PART -------------
        z_mean_prior, z_logvar_prior, z_out, _ = self.sample_prior(z.size(0), self.seq_len, random_sampling=True)
    ...
    def unroll_hidden(self, time, train_coeffs, final_index):
        z = self.encoder(time, train_coeffs, final_index)
        z = self.proj_to_koop_space(z)
        return z

    def unroll_hidden_prior(self, n_sample):
        _, _, _, h_agg = self.sample_prior(n_sample, self.seq_len, random_sampling=True)
        return h_agg

    def compute_operator_and_pred(self, z, reverse_dynamic=False):
        z_dim = z.shape[-1]
        z_past, z_future = z[:, :-1], z[:, 1:]  # split latent
        if reverse_dynamic:
            z_future, z_past = z[:, :-1], z[:, 1:]  # split latent

        # solve linear system (broadcast)
        if self.pinv_solver:
            Ct = torch.linalg.pinv(z_past.reshape(-1, z_dim)) @ z_future.reshape(-1, z_dim)

        else:
            # self.qr_solver
            Q, R = torch.linalg.qr(z_past.reshape(-1, z_dim))
            B = Q.T @ z_future.reshape(-1, z_dim)
    ...
    def sample_data(self, n_sample):
        # sample from prior
        z_mean_prior, z_logvar_prior, z_out, _ = self.sample_prior(n_sample, self.seq_len, random_sampling=True)
    ...
    def sample_prior(self, n_sample, seq_len, random_sampling=True):
        ...
        z_logvars, z_means, z_out, h_agg = self.zeros_init(batch_size, seq_len)
        ...
        for i in range(seq_len):
            ...
            # h_t = self.proj_to_koop_space(h_t)
        ...
        return z_means, z_logvars, z_out, h_agg
    ...
    def zeros_init(self, batch_size, seq_len):
        ...
        h_agg = torch.zeros(batch_size, seq_len, self.hidden_dim).cuda()
        return z_logvars, z_means, z_out, h_agg