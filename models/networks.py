...
"""Model architectures and preconditioning schemes used in the diffusion_paper
...
class Conv2d(torch.nn.Module):
    def __init__(self,
        ...
        if gated:
            self.gate_weight = torch.nn.Parameter(weight_init([out_channels, in_channels, kernel, kernel],
    ...
    def forward(self, x):
        ...
        if self.fused_resample and self.up and w is not None:
            x = torch.nn.functional.conv_transpose2d(x, f.mul(4).tile([self.in_channels, 1, 1, 1]),
                                                     groups=self.in_channels, stride=2, padding=w_pad + f_pad)
            if self.gated:
...
class UNetBlock(torch.nn.Module):
    def __init__(self,
        ...
        self.conv0 = Conv2d(in_channels=in_channels, out_channels=out_channels, kernel=3, up=up, down=down,
                            resample_filter=resample_filter, gated=gated, **init)
        ...
        if self.num_heads:
            ...
            self.qkv = Conv2d(in_channels=out_channels, out_channels=out_channels * 3, kernel=1, gated=gated,
...
# ----------------------------------------------------------------------------
# Reimplementation of the DDPM++ and NCSN++ architectures from the diffusion_paper
    ...
    def __init__(self,
        ...
        for level, mult in enumerate(channel_mult):
            ...
            if level == 0:
                cin = cout
                cout = model_channels
                self.enc[f'{res}x{res}_conv'] = Conv2d(in_channels=cin, out_channels=cout, kernel=3, gated=gated,
                                                       **init)
            else:
                self.enc[f'{res}x{res}_down'] = UNetBlock(in_channels=cout, out_channels=cout, down=True, gated=gated,
                                                          **block_kwargs)
                if encoder_type == 'skip':
                    self.enc[f'{res}x{res}_aux_down'] = Conv2d(in_channels=caux, out_channels=caux, kernel=0, down=True,
                                                               resample_filter=resample_filter, gated=gated)
                ...
                if encoder_type == 'residual':
                    self.enc[f'{res}x{res}_aux_residual'] = Conv2d(in_channels=caux, out_channels=cout, kernel=3,
                                                                   ...
                                                                   fused_resample=True, gated=gated, **init)
                    caux = cout
        ...
        for level, mult in reversed(list(enumerate(channel_mult))):
            ...
            if level == len(channel_mult) - 1:
                self.dec[f'{res}x{res}_in0'] = UNetBlock(in_channels=cout, out_channels=cout, attention=True,
                                                         gated=gated, **block_kwargs)
                self.dec[f'{res}x{res}_in1'] = UNetBlock(in_channels=cout, out_channels=cout, gated=gated,
                                                         **block_kwargs)
            else:
                self.dec[f'{res}x{res}_up'] = UNetBlock(in_channels=cout, out_channels=cout, up=True, gated=gated,
                                                        **block_kwargs)
...
# ----------------------------------------------------------------------------
# Reimplementation of the ADM architecture from the diffusion_paper
        ...
        for level, mult in enumerate(channel_mult):
            ...
            for idx in range(num_blocks):
                ...
                self.enc[f'{res}x{res}_block{idx}'] = UNetBlock(in_channels=cin, out_channels=cout,
                                                                attention=(res in attn_resolutions), gated=gated,
                                                                **block_kwargs)
...
# from the diffusion_paper "Score-Based Generative Modeling through Stochastic
...
class VPPrecond(torch.nn.Module):
    def __init__(self,
        ...
        self.model = globals()[model_type](img_resolution=img_resolution, in_channels=img_channels,
                                           out_channels=img_channels, label_dim=label_dim, gated=gated, **model_kwargs)
...
# the diffusion_paper "Improved Denoising Diffusion Probabilistic Models".
...
class iDDPMPrecond(torch.nn.Module):
    def __init__(self,
        ...
        self.M = M
        self.model = globals()[model_type](img_resolution=img_resolution, in_channels=img_channels,
                                           out_channels=img_channels * 2, label_dim=label_dim, gated=gated,
                                           **model_kwargs)
...
# ----------------------------------------------------------------------------
# Improved preconditioning proposed in the diffusion_paper "Elucidating the Design
