"""
DiffEM with Unconditional Model + MMPS E-step — EMA Reset + LOWESS+STL Warm-Start

Same as run_diffem_mmps_ema_reset.py with ONE change:
  Gaussian initialization is replaced with LOWESS+STL initialization.
  Missing values are filled using locally weighted regression (LOWESS)
  instead of linear interpolation before STL decomposition.
"""
...
from utils.utils_stl import initialize_with_lowess_stl
...
def main(args):
    ...
    name = f"diffem_mmps_ema_reset_lowess_init_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print(f"\n{'='*60}")
        print("Phase 1: LOWESS+STL Warm-Start Initialization")
        print(f"{'='*60}")

        initial_reconstructions = initialize_with_lowess_stl(
            corrupted_data, obs_masks, seed=args.seed
        )

        # === Phase 1.5: Train unconditional model on STL-initialized data ===
        print(f"\n{'='*60}")
        print("Phase 1.5: Initial unconditional model training on LOWESS+STL-initialized data")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("DiffEM-MMPS + LOWESS+STL Warm-Start Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS + LOWESS+STL Warm-Start training is complete")
