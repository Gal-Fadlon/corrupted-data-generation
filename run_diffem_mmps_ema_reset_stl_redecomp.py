"""
DiffEM with Unconditional Model + MMPS E-step — EMA Reset + STL Re-Decomposition

Same as run_diffem_mmps_ema_reset_stl_init.py with ONE enhancement:
  After every E-step, a structural regularization pass is applied:
    1. Hard-replace observed positions with original values (obs consistency).
    2. STL-decompose the full imputed series.
    3. At missing positions: keep the new (MMPS-informed) trend + seasonal,
       but re-sample the residual from the observed-residual distribution.
  This prevents the MMPS imputations from drifting into structurally
  unrealistic territory (broken trends, inconsistent seasonality) across
...
# =============================================================================
# STL Re-Decomposition Regularizer (NEW)
# =============================================================================

def stl_regularize(reconstructions, corrupted_data, obs_masks, period=None):
    """
    Structural regularization of E-step output via STL re-decomposition.

    Combines MMPS's local consistency with STL's global structural prior:
      1. Hard-replace observed positions with original values.
      2. STL-decompose the full (now complete) imputed series.
      3. At missing positions: keep trend + seasonal, re-sample residual
         from the distribution of observed residuals.

    This prevents drift in trend and seasonality across EM iterations while
    preserving realistic noise characteristics.

    Args:
        reconstructions: (N, T, C) numpy array, fully imputed from E-step
    ...
    Returns:
        regularized: (N, T, C) structurally corrected reconstructions
    """
    N, T, C = reconstructions.shape
    if period is None:
        period = auto_detect_period(T)

    regularized = reconstructions.copy()
    ...
    for i in range(N):
        ...
        # Step 1: observation consistency — hard-replace observed positions
        for c in range(C):
            obs_vals = corrupted_data[i, obs_idx, c]
            valid = ~np.isnan(obs_vals)
            regularized[i, obs_idx[valid], c] = obs_vals[valid]

        if len(obs_idx) < 3:
            continue

        for c in range(C):
            series = regularized[i, :, c].copy()

            # Step 2: STL-decompose the full imputed series
            trend_c, season_c, resid_c = stl_decompose_single(series, period)

            # Step 3: at missing positions only, replace with trend + seasonal
            # + re-sampled residual (observed positions are left untouched)
            resid_obs = resid_c[obs_idx]
            mu_r = resid_obs.mean()
            std_r = max(resid_obs.std(), 1e-4)
            new_resid_mis = np.random.normal(mu_r, std_r, size=len(mis_idx))

            regularized[i, mis_idx, c] = (
                trend_c[mis_idx] + season_c[mis_idx] + new_resid_mis
            )

    regularized = np.clip(regularized, 0, 1)

    n_missing = (~obs_masks).sum()
    print(f"STL re-decomposition complete. Regularized {n_missing} missing positions "
          f"across {N} sequences (period={period}).")
...
def main(args):
    """Main DiffEM-Uncond training loop with MMPS E-step + STL re-decomposition."""

    # Model name and directory
    name = create_model_name_and_dir(args)
    name = f"diffem_mmps_ema_reset_stl_redecomp_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        # === Phase 2: EM Loop with STL Re-Decomposition ===
        print(f"\n{'='*60}")
        ...
        print(f"  STL re-decomposition: ENABLED (every EM iteration)")
        print(f"{'='*60}")
        ...
        for em_iter in range(args.em_iters):
            ...
            # STL re-decomposition: structural regularization of E-step output
            reconstructions = stl_regularize(
            ...
            # M-step: Train unconditional model on regularized data
            uncond_model = m_step(
                args, uncond_model, optimizer,
        ...
        print("\n" + "="*60)
        print("DiffEM-MMPS + STL Re-Decomposition Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS + STL Re-Decomposition training is complete")
