"""
Experiment 3: Warm-start with Denoised Data. Before the first M-step, applies aggressive
STL decomposition smoothing to the initial reconstructions to remove noise. This gives
the diffusion model a cleaner starting point. Subsequent EM iterations use MMPS
reconstructions normally. For missing-only, uses default STL init behavior.
"""
...
def main(args):
    ...
    name = f"noise_exp_warmstart_denoised_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        # Warm-start: aggressively denoise initial reconstructions
        noise_level = getattr(args, 'noise_level', 0.0)
        if noise_level > 0:
            # Double-smooth: first regular STL init, then aggressive smoothing
            initial_reconstructions = decompose_and_smooth_ts(
                initial_reconstructions, residual_keep_fraction=0.2, trend_smooth_window=7
            )
            print(f"  Warm-start: applied aggressive denoising to initial reconstructions")
