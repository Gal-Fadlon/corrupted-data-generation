"""
DiffEM with MMPS E-step — Decomposition Consistency Regularization

After computing the standard MMPS correction, add a penalty that pulls the
result toward decomposition-consistent (smooth) solutions. At high noise,
strongly enforce consistency (trust structure). At low noise, allow the full
MMPS correction with fine details.

This is equivalent to adding a log-prior term nabla log p_structure(x) to the
posterior score.
"""
...
# =============================================================================
# MMPSDiffusionProcess — Decomposition Consistency
# =============================================================================

class MMPSDiffusionProcess:
    """
    MMPS-style diffusion sampler with decomposition consistency regularization.

    After computing the standard MMPS result (denoised + sigma^2 * score), the
    result is blended with a "consistent" version obtained by smoothing along
    the W (time) axis via moving average:

        alpha = exp(-sigma / sigma_ref)
        result = (1 - alpha) * consistent + alpha * result_mmps

    At high sigma: alpha ~ 0 -> result ~ consistent (smooth, structure-respecting)
    At low sigma:  alpha ~ 1 -> result ~ result_mmps (full MMPS with fine details)

    This acts as a structural prior that regularizes the posterior at high noise
    levels where the Jacobian-based correction is unreliable.
    """

    def __init__(self, args, diffusion_fn, shape, sigma_y=0.01, cg_iters=1,
                 sigma_ref=1.0, consistency_window=None):
        self.args = args
        ...
        # Auto-detect consistency window from image width if not provided
        if consistency_window is None:
            self.consistency_window = max(3, shape[-1] // 8)
            # Ensure odd window for symmetric padding
            if self.consistency_window % 2 == 0:
                self.consistency_window += 1
        else:
            self.consistency_window = consistency_window

    def _extract_trend(self, x, window):
        """
        Extract trend (smooth) component via moving average along W (time) axis.

        Uses F.conv2d with a box kernel for efficient computation.
        ...
        Returns:
            trend: smoothed tensor (B, C, H, W), same shape as input
        """
        B, C, H, W = x.shape
        kernel = torch.ones(1, 1, 1, window, device=x.device, dtype=x.dtype) / window
        pad = window // 2
        x_padded = F.pad(x, (pad, pad, 0, 0), mode='reflect')
        x_reshaped = x_padded.reshape(B * C, 1, H, W + 2 * pad)
        trend = F.conv2d(x_reshaped, kernel)
        return trend.reshape(B, C, H, W)

    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        """
        Posterior denoiser with decomposition consistency regularization.

        1. Compute standard MMPS result: result_mmps = denoised + sigma^2 * score
        2. Extract consistent (smooth) version via moving average
        3. Blend: alpha = exp(-sigma / sigma_ref)
           result = (1 - alpha) * consistent + alpha * result_mmps
        4. At high sigma: result ~ consistent (smooth, decomposition-respecting)
        5. At low sigma: result ~ result_mmps (full MMPS with fine details)
        ...
        # Standard MMPS result
        result_mmps = denoised.detach() + sigma_sq * score.detach()

        # Extract consistent (smooth) version — trend only, no residual
        consistent = self._extract_trend(result_mmps, self.consistency_window)

        # Blend based on sigma: alpha ~ 0 at high noise, ~ 1 at low noise
        sigma_val = float(sigma)
        alpha = torch.exp(torch.tensor(-sigma_val / self.sigma_ref,
                                        device=result_mmps.device, dtype=result_mmps.dtype))
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    ...
    Uses MMPSDiffusionProcess with decomposition consistency regularization.
    ...
    """
    print(f"\n=== E-Step (EM iter {em_iter}) — MMPS Imputation (Decomp Consistency) ===")
    print("Imputing missing values via MMPS with decomposition consistency...")
    ...
    sigma_ref = getattr(args, 'sigma_ref', 1.0)
    consistency_window = getattr(args, 'consistency_window', None)
    ...
    with uncond_model.ema_scope():
        process = MMPSDiffusionProcess(
            ...
            sigma_ref=sigma_ref, consistency_window=consistency_window
        )
...
def main(args):
    """Main DiffEM-Uncond training loop with MMPS E-step (Decomp Consistency)."""

    # Model name and directory
    name = create_model_name_and_dir(args)
    name = f"diffem_mmps_decomp_consistency_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print(f"\n{'='*60}")
        print(f"Starting DiffEM-MMPS + Decomp Consistency with {args.em_iters} EM iterations")
        print(f"  MMPS sigma_y: {args.mmps_sigma_y}")
        print(f"  MMPS cg_iters: {args.mmps_cg_iters}")
        print(f"  Sigma ref: {args.sigma_ref}")
        print(f"  Consistency window: {args.consistency_window}")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("DiffEM-MMPS + Decomp Consistency Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS + Decomp Consistency training is complete")
...
if __name__ == '__main__':
    ...
    # Consistency parameters (defaults if not provided via CLI)
    if not hasattr(args, 'sigma_ref') or args.sigma_ref is None:
        args.sigma_ref = 1.0
    if not hasattr(args, 'consistency_window') or args.consistency_window is None:
        args.consistency_window = None  # Auto-detect from image resolution
