"""
Shared DiffEM + MMPS pipeline for irregular time series (iterative STL init).

Used by run_diffem_mmps_spectral_*.py and run_diffem_mmps_bandlimited.py so that
Phase 1 (STL), curriculum, EMA reset, M-step, evaluation, and Phase 3 differ only
via the injected MMPS sampler (build_process_fn), not copy-pasted loops.
"""
...
def e_step(
    ...
    device,
    logger,
    build_process_fn: Callable[[Any, Any], Any],
    e_step_title: str,
    tqdm_desc: str,
):
    print(f"\n=== E-Step (EM iter {em_iter}) — {e_step_title} ===")
    uncond_model.eval()
    ...
    with uncond_model.ema_scope():
        ...
        for start_idx in tqdm(range(0, n_seq, batch_size), desc=tqdm_desc):
...
def m_step(args, uncond_model, optimizer, reconstructions, em_iter, device, logger=None):
    ...
    recon_loader = Data.DataLoader(
        recon_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers
    )
...
def evaluate_uncond(args, uncond_model, test_loader, em_iter, device, logger=None):
    ...
    mem_stats = compute_memorization_metric(
        real_data=real_sig, generated_data=gen_sig, device=device, plot_path=mem_plot_path
    )
...
@dataclass
class IrregularMMPSExperimentSpec:
    """Wiring for a fair comparison: same EM scaffolding, different MMPS only."""

    wandb_name_prefix: str
    build_process_fn: Callable[[Any, Any], Any]
    e_step_title: str
    tqdm_desc: str
    phase2_start_line: str
    completion_message: str
    phase2_extra_printer: Optional[Callable[[Any], None]] = None


def diffem_mmps_irregular_main(args, spec: IrregularMMPSExperimentSpec) -> None:
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        for em_iter in range(args.em_iters):
            ...
            if n_revealed > 0:
                print(
                    f"  Curriculum: revealed {n_revealed} extra positions (progress={progress:.2f})"
                )

            reconstructions = e_step(
                args,
                uncond_model,
                curriculum_corrupted,
                curriculum_masks,
                em_iter,
                args.device,
                logger,
                spec.build_process_fn,
                spec.e_step_title,
                spec.tqdm_desc,
            )
            ...
            uncond_model = m_step(
                args, uncond_model, optimizer, reconstructions, em_iter, args.device, logger
            )
            ...
            if (em_iter + 1) % args.em_eval_interval == 0 or em_iter == args.em_iters - 1:
                ...
                if best_metrics is None or metrics.get("disc_mean", float("inf")) < best_metrics.get(
                    "disc_mean", float("inf")
                ):
