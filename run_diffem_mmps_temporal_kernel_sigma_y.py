"""
DiffEM with MMPS E-step — Temporal Kernel Σ_y

Replaces isotropic σ_y²·I with a temporal exponential decay kernel:
    Σ_y[i,j] = σ_y² · exp(-|i-j|/ℓ)
where ℓ is a lengthscale parameter.  Nearby time-points share correlated
observation noise, implemented efficiently as a 1-D convolution along the
W (time) axis of the delay-embedding image.
"""
...
# =============================================================================
# MMPSDiffusionProcess — Temporal Kernel Σ_y
# =============================================================================

class MMPSDiffusionProcess:
    """
    MMPS-style diffusion sampler with temporally-correlated observation noise.

    Instead of the isotropic covariance σ_y²·I, the observation noise
    covariance is Σ_y[i,j] = σ_y² · exp(-|i-j|/ℓ), applied along the
    W (time) axis of the delay-embedding image via 1-D grouped convolution.
    """
    ...
    def __init__(self, args, diffusion_fn, shape, sigma_y=0.01, cg_iters=1,
        ...
        # Pre-build the temporal convolution kernel
        self._build_temporal_kernel()

    def _build_temporal_kernel(self):
        """Build a 1-D exponential-decay kernel for temporal correlation."""
        half_size = int(math.ceil(3 * self.lengthscale))
        # Ensure the image width doesn't constrain us too much
        img_w = self.shape[-1]  # W dimension of (C, H, W)
        half_size = min(half_size, (img_w - 1) // 2)
        kernel_size = 2 * half_size + 1

        offsets = torch.arange(-half_size, half_size + 1, dtype=torch.float64)
        kernel_1d = torch.exp(-offsets.abs() / self.lengthscale)  # unnormalised

        # Normalise so the kernel sums to 1 (energy-preserving); the overall
        # scale σ_y² is applied separately.
        kernel_1d = kernel_1d / kernel_1d.sum()

        # Shape for F.conv2d with groups=C:  (C, 1, 1, kernel_size)
        # We will expand to the correct number of channels at call time.
        self._kernel_1d = kernel_1d  # (kernel_size,)
        self._kernel_size = kernel_size
        self._kernel_padding = half_size

    def _apply_temporal_kernel(self, v):
        """Apply Σ_y to v:  σ_y² · K_temporal ⊛ v  along the W axis."""
        sigma_y_sq = self.sigma_y ** 2
        C = v.shape[1]

        # Build grouped-conv kernel: (C, 1, 1, kernel_size)
        kernel = self._kernel_1d.to(v.device).reshape(1, 1, 1, -1)
        kernel = kernel.expand(C, 1, 1, -1).contiguous()

        # Grouped 2-D convolution along W (height kernel = 1, width kernel = K)
        out = F.conv2d(v, kernel, padding=(0, self._kernel_padding), groups=C)
        return sigma_y_sq * out

    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        """
        Posterior denoiser with temporally-correlated observation noise.

        The CG operator becomes:
            M(v) = Σ_y ⊛ v  +  σ² · mask · Jᵀ(mask · v)
        where Σ_y ⊛ v is the temporal-kernel convolution replacing σ_y²·v.
        """
        sigma_sq = sigma ** 2
        ...
        def cg_operator(v):
            return self._apply_temporal_kernel(v) + sigma_sq * mask * vjp_fn(mask * v)

        # Solve (Σ_y + σ² · mask · Jᵀ · mask) · v = r
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    ...
    lengthscale = getattr(args, 'temporal_lengthscale', 3.0)
...
def main(args):
    ...
    name = f"diffem_mmps_temporal_kernel_sigma_y_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print(f"  Temporal lengthscale: {args.temporal_lengthscale}")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("DiffEM-MMPS + Temporal Kernel Σ_y Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS + Temporal Kernel Σ_y training is complete")
...
if __name__ == '__main__':
    ...
    # Temporal kernel parameters (defaults)
    if not hasattr(args, 'temporal_lengthscale'):
        args.temporal_lengthscale = 3.0
