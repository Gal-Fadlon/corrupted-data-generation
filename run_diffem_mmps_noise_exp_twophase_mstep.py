"""
Experiment 10: Two-Phase M-step. Split each M-step into two phases:
Phase A (first 20% of epochs): train with masked loss on observed positions only (learn clean signal).
Phase B (remaining 80%): train on full reconstruction (learn full distribution).
For missing-only, Phase A uses the observation mask; the approach still benefits from
seeing clean data first.
"""
...
def m_step(args, uncond_model, optimizer, reconstructions, em_iter, device, logger=None, corrupted_data=None):
    ...
    uncond_model.train()
    phase_a_epochs = max(1, int(args.m_step_epochs * 0.2))  # First 20%

    # Prepare masked data if corrupted_data available
    if corrupted_data is not None:
        corrupted_tensor = torch.tensor(corrupted_data, dtype=torch.float32)
        masked_dataset = Data.TensorDataset(recon_tensor, corrupted_tensor)
        masked_loader = Data.DataLoader(
            masked_dataset, batch_size=args.batch_size,
            shuffle=True, num_workers=args.num_workers
        )
    ...
    for epoch in range(args.m_step_epochs):
        ...
        if epoch < phase_a_epochs and corrupted_data is not None:
            # Phase A: masked loss on observed positions only
            for x_recon_batch, x_corrupt_batch in masked_loader:
                x_recon_batch = x_recon_batch.to(device)
                x_corrupt_batch = x_corrupt_batch.to(device)
                x_img = uncond_model.ts_to_img(x_recon_batch)
                x_corrupt_img = uncond_model.ts_to_img(x_corrupt_batch)
                mask_img = torch.isnan(x_corrupt_img).float() * -1 + 1
            ...
            # Phase B: standard full reconstruction loss
        ...
        phase = "A(masked)" if epoch < phase_a_epochs and corrupted_data is not None else "B(full)"
        if (epoch + 1) % 10 == 0 or epoch == 0:
            print(f"  M-step epoch {epoch+1}/{args.m_step_epochs} [{phase}], loss: {avg_loss:.4f}")
        if logger is not None:
...
def main(args):
    ...
    name = f"noise_exp_twophase_mstep_{name}"
    logging.info(args)
