"""
DiffEM with MMPS E-step — Frequency-Gated MMPS Guidance

Fork of run_diffem_mmps_stl_curriculum.py with one key change:
  The MMPS posterior denoiser applies observation guidance selectively
  based on the current noise level (sigma).  At high sigma (early in
  reverse process), only low-frequency (trend) guidance is active.
  As sigma decreases, seasonal and then residual guidance are added.

  This aligns with the natural coarse-to-fine dynamics of diffusion:
  low-frequency content crystallizes first, high-frequency details later.
...
def main(args):
    ...
    name = f"diffem_mmps_freq_gated_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print(f"\n{'='*60}")
        print(f"Starting Frequency-Gated DiffEM-MMPS with {args.em_iters} EM iterations")
        print(f"{'='*60}")
        ...
        if final_metrics:
            ...
            for k, v in final_metrics.items():
                print(f"  {k}: {v:.4f}")
        print("\n" + "="*60)
        print("Frequency-Gated DiffEM-MMPS Training Complete!")
        print("="*60)
        if best_metrics:
            print("Best metrics:")
