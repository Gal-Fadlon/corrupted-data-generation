"""
Gaussian Noise Handler — Approach 2: Ambient Diffusion M-step

The M-step uses an ambient-aware training loss that accounts for observation noise.
Instead of standard EDM denoising loss on noisy reconstructions, we:
  1. Add diffusion noise on top of the (possibly noisy) reconstruction
  2. Train the denoiser with a modified weight that accounts for observation noise
  3. The effective noise level is sqrt(sigma_diffusion² + sigma_obs²)

This allows the model to learn p(x_clean) even when training data has noise,
because the loss implicitly deconvolves the observation noise.

Usage:
  --missing_rate 0.0 --noise_level 0.1 --gaussian_noise_level 0.1
"""
...
# =============================================================================
# Ambient-aware EDM training loss
# =============================================================================

def ambient_loss_fn(net, x_noisy_img, sigma_obs, P_mean=-1.2, P_std=1.2, sigma_data=0.5):
    """
    Ambient-aware EDM training loss.

    Given data x that has observation noise sigma_obs, the forward diffusion adds
    sigma_diffusion noise. The effective noise seen by the network is:
        sigma_eff = sqrt(sigma_diffusion² + sigma_obs²)

    We train with the standard EDM loss but use sigma_eff for the network input
    while keeping the original EDM weight based on sigma_diffusion.
    This implicitly teaches the model to separate diffusion noise from observation noise.

    Args:
        net: the EDM preconditioned network
        x_noisy_img: training image (may contain observation noise)
        sigma_obs: observation noise standard deviation
        P_mean, P_std: log-normal parameters for sampling sigma_diffusion
        sigma_data: EDM sigma_data parameter

    Returns:
        loss: scalar loss
        to_log: dict with diagnostic values
    """
    to_log = {}

    # Sample diffusion noise level
    rnd_normal = torch.randn([x_noisy_img.shape[0], 1, 1, 1], device=x_noisy_img.device)
    sigma_diff = (rnd_normal * P_std + P_mean).exp()

    # Effective noise level (observation + diffusion)
    sigma_eff = torch.sqrt(sigma_diff ** 2 + sigma_obs ** 2)

    # EDM weight (based on diffusion noise only)
    weight = (sigma_diff ** 2 + sigma_data ** 2) / (sigma_diff * sigma_data) ** 2

    # Forward diffusion: add noise at effective level
    # x_noisy already has sigma_obs noise, adding sigma_diff noise gives sigma_eff total
    noise = torch.randn_like(x_noisy_img) * sigma_diff
    x_input = x_noisy_img + noise

    # Network prediction (told the effective noise level)
    D_x = net(x_input, sigma_eff, None)

    # Loss: predict the noisy input (the network learns to remove diffusion noise,
    # and over many iterations with varying sigma_diff, implicitly also learns
    # to handle the constant sigma_obs component)
    loss = (weight * (D_x - x_noisy_img).square()).mean()

    to_log['ambient_loss'] = loss.detach().item()
    to_log['sigma_diff_mean'] = sigma_diff.mean().item()
    to_log['sigma_eff_mean'] = sigma_eff.mean().item()
...
def m_step(args, uncond_model, optimizer, reconstructions, em_iter, device, logger=None):
    ...
    for epoch in range(args.m_step_epochs):
        ...
        for (x_clean,) in recon_loader:
            ...
            if noise_level > 0:
                loss, to_log = ambient_loss_fn(
                    uncond_model.net, x_img, sigma_obs=noise_level
                )
            else:
...
def main(args):
    ...
    name = f"diffem_gaussian_ambient_mstep_{name}"
    logging.info(args)
