"""
DiffEM with Unconditional Model + MMPS E-step — EMA Reset + PCHIP+STL Warm-Start

Same as run_diffem_mmps_ema_reset.py with ONE change:
  Gaussian initialization is replaced with PCHIP+STL initialization.
  Missing values are filled using shape-preserving cubic Hermite (PCHIP)
  interpolation instead of linear interpolation before STL decomposition.
"""
...
from utils.utils_stl import initialize_with_pchip_stl
...
def main(args):
    ...
    name = f"diffem_mmps_ema_reset_pchip_init_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print(f"\n{'='*60}")
        print("Phase 1: PCHIP+STL Warm-Start Initialization")
        print(f"{'='*60}")

        initial_reconstructions = initialize_with_pchip_stl(
            corrupted_data, obs_masks, seed=args.seed
        )

        # === Phase 1.5: Train unconditional model on STL-initialized data ===
        print(f"\n{'='*60}")
        print("Phase 1.5: Initial unconditional model training on PCHIP+STL-initialized data")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("DiffEM-MMPS + PCHIP+STL Warm-Start Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS + PCHIP+STL Warm-Start training is complete")
