"""
DiffEM with Unconditional Model + MMPS E-step — EMA Reset + MA Detrending

Same EM framework as run_diffem_mmps_ema_reset.py with ONE enhancement:
  Moving-average detrending is applied before modelling:
    - M-step: subtract per-sample MA trend, train on detrended data.
    - E-step: subtract estimated trend from observations, MMPS on detrended,
              add trend back.
    - Evaluation: generate in detrended space, add population trend back.

  The diffusion model learns the stationary residual (zero-centred, lower
  variance) which is an easier distribution to model than raw time series.

  Model architecture, channels, MMPS, and EMA reset are all unchanged.
"""
...
# =============================================================================
# Moving-Average Detrending Helpers
# =============================================================================

def compute_ma_trend(data, window=5):
    """
    Per-sample, per-feature moving-average trend.

    Args:
        data: (N, T, C) numpy array, no NaN
        window: MA window size (odd recommended)
    Returns:
        trend: (N, T, C) same shape
    """
    N, T, C = data.shape
...
def linear_interpolate_missing(corrupted_data, obs_masks):
    """
    Fill NaN values via per-feature linear interpolation.
    ...
    filled = corrupted_data.copy()
    for i in range(N):
        obs_idx = np.where(obs_masks[i])[0]
        mis_idx = np.where(~obs_masks[i])[0]
        if len(obs_idx) < 2 or len(mis_idx) == 0:
            mu = np.nanmean(corrupted_data[i], axis=0)
            filled[i] = np.where(np.isnan(filled[i]), mu, filled[i])
            continue
        for c in range(C):
            obs_vals = corrupted_data[i, obs_idx, c]
            valid = ~np.isnan(obs_vals)
            if valid.sum() < 2:
                filled[i, mis_idx, c] = np.nanmean(obs_vals)
            else:
                filled[i, mis_idx, c] = np.interp(
                    mis_idx, obs_idx[valid], obs_vals[valid]
                )
    return np.nan_to_num(filled, nan=0.5)
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    """
    E-step with MA detrending: estimate trend from corrupted data,
    subtract it, run MMPS on the detrended signal, then add trend back.
    """
    print(f"\n=== E-Step (EM iter {em_iter}) — MMPS + MA Detrending ===")
    print("Imputing missing values via MMPS on detrended data...")

    uncond_model.eval()

    ma_window = getattr(args, 'ma_window', 5)

    filled_data = linear_interpolate_missing(corrupted_data, obs_masks)
    trend_full = compute_ma_trend(filled_data, window=ma_window)

    detrended_corrupted = corrupted_data - trend_full
    ...
    with uncond_model.ema_scope():
        ...
        for start_idx in tqdm(range(0, N, batch_size), desc="E-step MMPS"):
            ...
            detrended_batch = detrended_corrupted[start_idx:end_idx]
            mask_batch = obs_masks[start_idx:end_idx]
            trend_batch = trend_full[start_idx:end_idx]

            detrended_ts = torch.tensor(detrended_batch, dtype=torch.float32, device=device)
            mask_ts = torch.tensor(mask_batch, dtype=torch.float32, device=device)

            obs_ts = torch.nan_to_num(detrended_ts, nan=0.0)

            x_obs_img = uncond_model.ts_to_img(obs_ts)

            mask_ts_expanded = mask_ts.unsqueeze(-1).expand(-1, -1, detrended_ts.shape[-1])
            mask_img = uncond_model.ts_to_img(mask_ts_expanded)
            mask_img = mask_img[:, :1, :, :]

            x_img_imputed = process.sampling_mmps(x_obs_img, mask_img)

            x_ts_detrended = uncond_model.img_to_ts(x_img_imputed)

            x_ts_recon = x_ts_detrended.cpu().numpy() + trend_batch

            all_reconstructions.append(x_ts_recon)
            torch.cuda.empty_cache()

    reconstructions = np.clip(np.vstack(all_reconstructions), 0, 1)
...
def m_step(args, uncond_model, optimizer, reconstructions, em_iter, device, logger=None):
    """
    M-step with MA detrending: subtract per-sample MA trend,
    train on detrended data, store population trend for evaluation.
    """
    print(f"\n=== M-Step (EM iter {em_iter}) ===")
    print(f"Training unconditional model for {args.m_step_epochs} epochs (detrended)...")

    ma_window = getattr(args, 'ma_window', 5)
    trend = compute_ma_trend(reconstructions, window=ma_window)
    detrended = reconstructions - trend

    uncond_model._pop_trend = trend.mean(axis=0)  # (T, C) for evaluation

    recon_tensor = torch.tensor(detrended, dtype=torch.float32)
...
def evaluate_uncond(args, uncond_model, test_loader, em_iter, device, logger=None):
    """
    Evaluate: generate in detrended space, add population trend back,
    clip to [0,1], then compute disc_mean.
    """
    print(f"\n=== Evaluation (EM iter {em_iter}) ===")
    print("Evaluating unconditional model (detrended + re-trended)...")

    uncond_model.eval()
    gen_sig = []
    real_sig = []

    pop_trend = getattr(uncond_model, '_pop_trend', None)
    ...
    with torch.no_grad():
        with uncond_model.ema_scope():
            ...
            for data in tqdm(test_loader, desc="Evaluating"):
                ...
                gen_np = x_ts.cpu().numpy()

                if pop_trend is not None:
                    gen_np = gen_np + pop_trend[np.newaxis, :, :]
                    gen_np = np.clip(gen_np, 0, 1)

                gen_sig.append(gen_np)
...
def main(args):
    ...
    name = f"diffem_mmps_ema_reset_detrend_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        # === Phase 2: EM Loop ===
        print(f"\n{'='*60}")
        print(f"Starting DiffEM-MMPS + MA Detrending with {args.em_iters} EM iterations")
        ...
        if final_metrics:
            ...
            for k, v in final_metrics.items():
                print(f"  {k}: {v:.4f}")
        print("\n" + "="*60)
        print("DiffEM-MMPS + MA Detrending Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS + MA Detrending training is complete")
