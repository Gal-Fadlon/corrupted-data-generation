"""
Experiment 1: Noise-Aware Initialization.

When noise_level > 0, use more aggressive STL smoothing during initialization
(residual_keep_fraction=0.3 instead of 0.8) because the residual component is
mostly noise. For missing-only (noise_level=0), keep default behavior.

Based on DiffEM with MMPS E-step — STL Decomp Projection + Curriculum + Noise Support (DiEM-style).

Observation model: y = A * x + noise, where noise ~ N(0, sigma_obs^2 * I)
  - A is the forward operator (mask for missing data, identity for noise-only)
  - sigma_obs is the observation noise level
  - Noise is added to ALL positions (both observed and missing), matching DiEM

CG operator (matching DiEM's PosteriorDenoiser):
  cov_y_xt(v) = sigma_obs^2 * v + sigma_t^2 * A * J^T * (A^T * v)

For missing data only (sigma_obs=0): reduces to sigma_y^2 * v + ... (original behavior)
For noise only (A=I): reduces to sigma_obs^2 * v + sigma_t^2 * J^T * v
For missing + noise: the mask and noise covariance work independently
...
def main(args):
    ...
    name = f"diffem_mmps_noise_exp_noise_aware_init_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        # Noise-aware init: more aggressive smoothing when noise is present
        noise_level = getattr(args, 'noise_level', 0.0)
        if noise_level > 0:
            from scipy.ndimage import uniform_filter1d
            keep_frac = max(0.1, 1.0 - noise_level * 3)  # More noise = less residual
            initial_reconstructions = decompose_and_smooth_ts(
                initial_reconstructions, residual_keep_fraction=keep_frac
            )
            print(f"  Applied noise-aware smoothing (keep_frac={keep_frac:.2f})")
