...
+ Curriculum + Ambient M-step with Noise Augmentation

Extends the ambient M-step by also adding small Gaussian noise to the
reconstructions during training.  This addresses a complementary failure mode:
the model may overfit to the exact values of E-step reconstructions.  Adding
noise creates a distribution around each reconstruction, encouraging the model
to learn the conditional expectation rather than memorising point estimates.

Combines:
  - Ambient mask corruption (delta = 0.1)
  - Additive Gaussian noise (sigma_noise = 0.02)
"""
...
def m_step_ambient_noise(args, uncond_model, optimizer, reconstructions, obs_masks,
                         em_iter, device, delta=0.1, sigma_noise=0.02, logger=None):
    """Ambient M-step with additional Gaussian noise on reconstructions.

    Two corruption mechanisms applied simultaneously:
    1. Further mask corruption (delta fraction of observed positions masked)
    2. Small additive Gaussian noise (sigma_noise) on the data before diffusion
    """
    print(f"\n=== Ambient+Noise M-Step (EM iter {em_iter}, delta={delta:.2f}, "
          f"sigma_noise={sigma_noise:.3f}) ===")
    ...
    for epoch in range(args.m_step_epochs):
        ...
        for x_clean, mask_batch in recon_loader:
            ...
            data_noise = torch.randn_like(x_clean) * sigma_noise
            x_noisy_data = x_clean + data_noise

            x_img = uncond_model.ts_to_img(x_noisy_data)
            ...
            B = (torch.rand_like(mask_img_full) >= delta).float()
            further_mask = mask_img_full * B
            ...
            x_unpad = uncond_model.unpad(x_img_clean * mask_img_full, x_img.shape)
    ...
    print(f"Ambient+Noise M-step complete for EM iteration {em_iter}.")
    return uncond_model
...
def main(args):
    ...
    name = f"diffem_mmps_kalman_freqgate_decomp_proj_ambient_noise_{name}"
    logging.info(args)

    ambient_delta = getattr(args, 'further_corrupt_delta', 0.1)
    sigma_noise = 0.02
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print(f"\n{'='*60}")
        print(f"Starting Kalman + FreqGate + DecompProj + Ambient+Noise DiffEM-MMPS")
        print(f"  EM iterations: {args.em_iters}")
        print(f"  Ambient delta: {ambient_delta}, sigma_noise: {sigma_noise}")
        print(f"{'='*60}")
        ...
        for em_iter in range(args.em_iters):
            ...
            uncond_model = m_step_ambient_noise(
                args, uncond_model, optimizer,
                reconstructions, obs_masks,
                em_iter, args.device,
                delta=ambient_delta, sigma_noise=sigma_noise, logger=logger
            )
        ...
        if final_metrics:
            ...
            for k, v in final_metrics.items():
                print(f"  {k}: {v:.4f}")
        print("\n" + "="*60)
        print("Kalman + FreqGate + DecompProj + Ambient+Noise DiffEM-MMPS Complete!")
        print("="*60)
        if best_metrics:
