"""
DiffEM with MMPS E-step — Trend-Anchored Residual MMPS

Fork of run_diffem_mmps_stl_curriculum.py with one key change:
  The E-step runs standard MMPS on the *full signal* (matching the
  model's training distribution), then decomposes the MMPS output and
  replaces its trend+seasonal with an analytical estimate derived from
  observed data.  This anchors the reconstruction to a robust structural
  estimate while letting MMPS handle the stochastic residual.

Algorithm:
  1. Estimate trend + seasonal analytically from observed positions
  2. Run standard MMPS on the full signal (model operates in its trained distribution)
  3. Decompose MMPS output into trend_mmps + seasonal_mmps + residual_mmps
  4. reconstruction = analytical_trend + analytical_seasonal + residual_mmps
"""
...
# =============================================================================
# Trend / Seasonal estimation from partially observed data
# =============================================================================

def estimate_trend_seasonal_from_observed(corrupted_data, obs_masks, period=None):
    """
    Estimate trend and seasonal components from partially observed data.

    For each sample and feature:
      1. Linear-interpolate missing values
      2. Moving-average to get trend
      3. Periodic averaging of detrended signal to get seasonal

    Args:
        corrupted_data: (N, T, C) with NaN at missing positions
        obs_masks: (N, T) boolean, True = observed
        period: decomposition period (auto-detected if None)

    Returns:
        trends: (N, T, C) estimated trend (complete, no NaN)
        seasonals: (N, T, C) estimated seasonal (complete, no NaN)
        residuals_obs: (N, T, C) residual at observed positions, NaN elsewhere
    """
    ...
    trends = np.zeros((N, T, C), dtype=np.float64)
    seasonals = np.zeros((N, T, C), dtype=np.float64)
    residuals_obs = np.full((N, T, C), np.nan, dtype=np.float64)

    for i in range(N):
        obs_idx = np.where(obs_masks[i])[0]
        if len(obs_idx) < 2:
            trends[i] = np.nanmean(corrupted_data[i])
            continue
        ...
        for c in range(C):
            ...
            if len(mis_idx) > 0:
                interpolated[mis_idx] = np.interp(mis_idx, obs_idx, obs_vals)

            trend = uniform_filter1d(interpolated, size=window, mode='nearest')
            trends[i, :, c] = trend

            detrended = interpolated - trend
            seasonal = np.zeros(T, dtype=np.float64)
            for k in range(period):
                idx = np.arange(k, T, period)
                seasonal[idx] = detrended[idx].mean()
            seasonals[i, :, c] = seasonal

            residuals_obs[i, obs_idx, c] = (
                corrupted_data[i, obs_idx, c] - trend[obs_idx] - seasonal[obs_idx]
            )

    return trends, seasonals, residuals_obs
...
def e_step_trend_anchored(args, uncond_model, corrupted_data, obs_masks,
                          trends, seasonals, em_iter, device, logger=None):
    """
    Trend-Anchored E-step: run MMPS on full signal, then anchor trend+seasonal.

    1. Run standard MMPS on full signal (model operates in its trained distribution)
    2. Decompose MMPS output to extract its residual
    3. Return analytical_trend + analytical_seasonal + MMPS_residual
    """
    print(f"\n=== E-Step (EM iter {em_iter}) — Trend-Anchored Residual MMPS ===")
    uncond_model.eval()
    ...
    with uncond_model.ema_scope():
        ...
        for start_idx in tqdm(range(0, N, batch_size), desc="E-step Trend-Anchored"):
            end_idx = min(start_idx + batch_size, N)

            corrupted_batch = corrupted_data[start_idx:end_idx]
            mask_batch = obs_masks[start_idx:end_idx]
            trend_batch = trends[start_idx:end_idx]
            seasonal_batch = seasonals[start_idx:end_idx]
            ...
            trend_ts = torch.tensor(trend_batch, dtype=torch.float32, device=device)
            seasonal_ts = torch.tensor(seasonal_batch, dtype=torch.float32, device=device)

            obs_ts = torch.nan_to_num(corrupted_ts, nan=0.0)

            # Standard MMPS on the full signal (matching model's training distribution)
            ...
            x_ts_mmps = uncond_model.img_to_ts(x_img_imputed)

            # Decompose MMPS output and extract its residual
            mmps_np = x_ts_mmps.detach().cpu().numpy()
            _, _, resid_mmps_np = fast_decompose_batch(mmps_np)
            resid_mmps = torch.tensor(resid_mmps_np, dtype=torch.float32, device=device)

            # Anchor: analytical trend+seasonal + MMPS residual
            x_ts_recon = trend_ts + seasonal_ts + resid_mmps
...
def main(args):
    ...
    name = f"diffem_mmps_trend_anchored_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print(f"\n{'='*60}")
        print("Phase 1.5: Initial model training on STL-initialized data")
        print(f"{'='*60}")
        uncond_model = m_step(
        ...
        # Phase 2: EM loop with trend-anchored E-step
        print(f"\n{'='*60}")
        print(f"Starting Trend-Anchored DiffEM-MMPS with {args.em_iters} EM iterations")
        print(f"{'='*60}")
        ...
        for em_iter in range(args.em_iters):
            ...
            # Curriculum: reveal extra observations early
            ...
            # Estimate trend + seasonal from (curriculum-augmented) observed data
            trends, seasonals, _ = estimate_trend_seasonal_from_observed(
                curriculum_corrupted, curriculum_masks
            )

            # E-step: trend-anchored MMPS
            reconstructions = e_step_trend_anchored(
        ...
        if final_metrics:
            ...
            for k, v in final_metrics.items():
                print(f"  {k}: {v:.4f}")
        print("\n" + "="*60)
        print("Trend-Anchored DiffEM-MMPS Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("Trend-Anchored DiffEM-MMPS training is complete")
