"""
DiffEM with MMPS E-step — EMA Reset + STL Init + Observation Consistency

Fork of run_diffem_mmps_ema_reset_stl_init.py with ONE change:
  After MMPS imputation in the E-step, observed values are overwritten with
  the original clean observations.  This prevents the image-space roundtrip
  (ts_to_img -> MMPS -> img_to_ts) from distorting the few precious observed
  values, which is critical at high missing rates (70-90%).
"""
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    """
    E-step with observation consistency: after MMPS imputation, observed
    values are overwritten with the original clean observations.
    """
    print(f"\n=== E-Step (EM iter {em_iter}) — MMPS + Obs Consistency ===")
    ...
    # =====================================================================
    # OBSERVATION CONSISTENCY: overwrite observed positions with originals
    # =====================================================================
    ...
    obs_enforced = obs_masks.sum()
    print(f"E-step complete. Imputed {N} sequences, enforced {obs_enforced} observed positions.")
...
def main(args):
    ...
    name = f"diffem_mmps_stl_obs_consist_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print(f"\n{'='*60}")
        print(f"Starting DiffEM-MMPS + Obs Consistency with {args.em_iters} EM iterations")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("DiffEM-MMPS + Obs Consistency Training Complete!")
        print("="*60)
        if best_metrics:
