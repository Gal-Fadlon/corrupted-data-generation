"""
DiffEM with MMPS E-step — Adaptive σ_obs per Diffusion Step

Instead of fixed σ_obs, use σ_obs_effective(t) = min(σ_obs, σ_t).
When σ_t >> σ_obs: observations are cleaner than model, trust them.
When σ_t << σ_obs: model is more confident than noisy observations, don't trust them.
When σ_obs=0: min(0, σ_t) = 0 always (unchanged).
...
# =============================================================================
# Adaptive σ_obs Decomposition-Projected MMPS
# =============================================================================

class AdaptiveSigmaObsMMPSDiffusionProcess:
    """
    MMPS sampler with adaptive σ_obs that caps observation noise at the current
    diffusion noise level, and decomposition consistency projection.

    When sigma_obs=0, this is identical to DecompProjectedMMPSDiffusionProcess
    (standard MMPS with hard observation matching).

    When sigma_obs>0, σ_obs_effective(t) = min(σ_obs, σ_t):
      - When σ_t >> σ_obs: observations are cleaner than model, trust them
      - When σ_t << σ_obs: model is more confident, don't trust noisy observations
      - When σ_obs=0: min(0, σ_t) = 0 always (unchanged)
    ...
    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        ...
        # Adaptive: cap sigma_obs at current diffusion noise level
        sigma_obs_effective = min(self.sigma_obs, float(sigma))
        sigma_obs_eff_sq = sigma_obs_effective ** 2
        ...
        def cg_operator(v):
            return (sigma_y_sq + sigma_obs_eff_sq) * v + sigma_sq * mask * vjp_fn(mask * v)
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    print(f"\n=== E-Step (EM iter {em_iter}) — Adaptive σ_obs MMPS ===")
    uncond_model.eval()
    ...
    with uncond_model.ema_scope():
        process = AdaptiveSigmaObsMMPSDiffusionProcess(
        ...
        for start_idx in tqdm(range(0, N, batch_size), desc="E-step Adaptive-σ_obs"):
...
def main(args):
    ...
    name = f"adaptive_sigma_obs_mmps_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        if sigma_obs > 0:
            print(f"  Adaptive sigma_obs refinement ACTIVE (sigma_obs={sigma_obs})")
        else:
        ...
        print(f"\n{'='*60}")
        print(f"Starting Adaptive-σ_obs DiffEM-MMPS with {args.em_iters} EM iterations")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("Adaptive-σ_obs DiffEM-MMPS Complete!")
        print("="*60)
        if best_metrics:
