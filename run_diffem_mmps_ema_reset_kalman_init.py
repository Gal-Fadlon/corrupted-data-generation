"""
DiffEM with Unconditional Model + MMPS E-step — EMA Reset + Kalman Warm-Start

Same as run_diffem_mmps_ema_reset.py with ONE change:
  Gaussian initialization is replaced with Kalman-smoother initialization.
  Missing values are filled using a structural time series model
  (UnobservedComponents) with Kalman smoothing that natively handles NaN.
"""
...
def main(args):
    ...
    name = f"diffem_mmps_ema_reset_kalman_init_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print(f"\n{'='*60}")
        print("Phase 1: Kalman Warm-Start Initialization")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("DiffEM-MMPS + Kalman Warm-Start Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS + Kalman Warm-Start training is complete")
