...
+ Curriculum + Ambient M-step

Extends the best-performing kalman_freqgate_decomp_proj experiment by replacing
the vanilla M-step (loss_fn_irregular on full reconstructions) with an
Ambient Diffusion-style M-step (loss_fn_ambient_masked with further corruption).

The key idea (Daras et al., NeurIPS 2023): during M-step training, randomly mask
out delta fraction of *observed* positions before feeding to the network, but
evaluate the loss on *all* originally-observed positions.  This forces the model
to predict beyond what it directly sees, preventing memorisation of E-step
imputation artifacts and encouraging learning of the true distribution.

All E-step mechanisms (Kalman init, freq-gated MMPS, decomp projection,
curriculum) are unchanged from the base experiment.
"""
...
# =============================================================================
# Freq-Gated + Decomp-Projected MMPSDiffusionProcess (unchanged from base)
# =============================================================================
...
def m_step_ambient(args, uncond_model, optimizer, reconstructions, obs_masks,
                   em_iter, device, delta=0.1, logger=None):
    """Ambient M-step: train with further corruption on observed positions.

    Instead of treating E-step reconstructions as clean ground truth, we carry
    the original observation mask and apply the Ambient Diffusion further-
    corruption trick: randomly mask delta fraction of observed positions in the
    network input, but evaluate loss on ALL originally-observed positions.
    """
    print(f"\n=== Ambient M-Step (EM iter {em_iter}, delta={delta:.2f}) ===")
    ...
    for epoch in range(args.m_step_epochs):
        ...
        for x_clean, mask_batch in recon_loader:
            ...
            loss, _ = uncond_model.loss_fn_ambient_masked(x_img, mask_img, delta=delta)
...
def m_step_vanilla(args, uncond_model, optimizer, reconstructions, em_iter, device, logger=None):
    """Standard M-step for Phase 1.5 (no ambient corruption needed for init)."""
...
def main(args):
    ...
    name = f"diffem_mmps_kalman_freqgate_decomp_proj_ambient_{name}"
    logging.info(args)

    ambient_delta = getattr(args, 'further_corrupt_delta', 0.1)
    # Paper / official code: extra UNet input channel = further-corruption mask.
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print(f"\n{'='*60}")
        print(f"Starting Kalman + FreqGate + DecompProj + Ambient DiffEM-MMPS")
        print(f"  EM iterations: {args.em_iters}")
        print(f"  Ambient delta: {ambient_delta}")
        print(f"{'='*60}")
        ...
        for em_iter in range(args.em_iters):
            ...
            uncond_model = m_step_ambient(
                ...
                em_iter, args.device, delta=ambient_delta, logger=logger
            )
        ...
        print("\n" + "="*60)
        print("Kalman + FreqGate + DecompProj + Ambient DiffEM-MMPS Complete!")
        print("="*60)
        if best_metrics:
