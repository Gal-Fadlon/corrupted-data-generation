"""
DiffEM with MMPS E-step — EMA Reset + k-NN-STL Initialization

Fork of run_diffem_mmps_ema_reset_stl_init.py with ONE change:
  STL initialization is replaced with k-NN + STL: for each sample, find k
  nearest neighbors and use neighbor-averaged values to fill gaps before
  STL decomposition.  Improves initialization quality at high missing rates.
  Use --knn_k 5 (default) or add to config.
"""
...
def main(args):
    ...
    name = f"diffem_mmps_stl_knn_init_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print(f"\n{'='*60}")
        print("Phase 1.5: Initial unconditional model training on k-NN-STL-initialized data")
        print(f"{'='*60}")
        ...
        # === Phase 2: EM Loop ===
        print(f"\n{'='*60}")
        print(f"Starting DiffEM-MMPS + k-NN-STL with {args.em_iters} EM iterations")
        ...
        print("\n" + "="*60)
        print("DiffEM-MMPS + k-NN-STL Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS + k-NN-STL training is complete")
