"""
DiffEM with MMPS E-step — Frequency-Gated Score

Decompose the posterior score into frequency bands and gate each band based on σ.
At high noise (high σ), only allow low-frequency corrections. As σ decreases,
progressively unlock higher frequencies. This prevents the Jacobian from injecting
unreliable high-frequency corrections at high noise levels.
"""
...
# =============================================================================
# MMPSDiffusionProcess — Frequency-Gated Score
# =============================================================================

class MMPSDiffusionProcess:
    """
    MMPS-style diffusion sampler with frequency-gated posterior score.

    At each reverse step, the posterior score is decomposed into frequency bands
    via FFT along the W (time) axis. Each frequency band is gated by a sigmoid
    function of the current noise level σ:
      - Low-frequency bands are unlocked early (high σ)
      - High-frequency bands are only unlocked at low σ

    This prevents the Jacobian from injecting unreliable high-frequency
    corrections when the model is uncertain (high noise).
    """

    def __init__(self, args, diffusion_fn, shape, sigma_y=0.01, cg_iters=1,
                 gate_temperature=5.0, n_freq_bands=4):
        self.args = args
        ...
        # Frequency gating parameters
        self.gate_temperature = gate_temperature
        self.n_freq_bands = n_freq_bands

    def _frequency_gate(self, score, sigma):
        """
        Gate the score in frequency space along the W (time) axis.

        For each frequency bin k (out of K total), compute:
            sigma_threshold_k = sigma_max * (1 - k / K)
            gate_k = sigmoid((sigma_threshold_k - sigma) / temperature)

        At high sigma, high-frequency gates are ~0 (gated out).
        At low sigma, all gates are ~1 (all frequencies pass).

        Args:
            score: posterior score tensor (B, C, H, W)
            sigma: current noise level (scalar)

        Returns:
            score_gated: frequency-gated score (B, C, H, W)
        """
        W = score.shape[-1]

        # FFT along W axis (time dimension)
        score_freq = torch.fft.rfft(score, dim=-1)
        K = score_freq.shape[-1]  # number of frequency bins

        # Build per-frequency-bin gates
        k_indices = torch.arange(K, device=score.device, dtype=score.dtype)
        sigma_thresholds = self.sigma_max * (1.0 - k_indices / K)
        gates = torch.sigmoid((sigma_thresholds - sigma) / self.gate_temperature)

        # Reshape gates for broadcasting: (1, 1, 1, K)
        gates = gates.reshape(1, 1, 1, K)

        # Apply gates in frequency domain
        score_freq = score_freq * gates

        # IFFT back to spatial domain
        score_gated = torch.fft.irfft(score_freq, n=W, dim=-1)

        return score_gated

    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        """
        Posterior denoiser with frequency-gated score correction.

        Standard MMPS posterior computation followed by frequency gating:
        1. Forward pass with gradient: denoised = D_θ(x_t)
        2. Compute VJP, residual, CG solve
        3. Compute score = vjp_fn(mask * v)
        4. Frequency-gate the score: suppress high-freq at high σ
        5. Return denoised + σ² * score_gated
        ...
        # Frequency-gate the score
        score_gated = self._frequency_gate(score.detach(), sigma)

        # Detach from computation graph
        result = denoised.detach() + sigma_sq * score_gated
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    ...
    Uses MMPSDiffusionProcess with frequency-gated score: at each reverse step,
    the posterior score is decomposed into frequency bands and gated by σ.
    ...
    """
    print(f"\n=== E-Step (EM iter {em_iter}) — MMPS Imputation (Freq-Gated Score) ===")
    print("Imputing missing values via MMPS with frequency-gated posterior score...")
    ...
    gate_temperature = getattr(args, 'gate_temperature', 5.0)
    n_freq_bands = getattr(args, 'n_freq_bands', 4)
    ...
    with uncond_model.ema_scope():
        process = MMPSDiffusionProcess(
            ...
            gate_temperature=gate_temperature, n_freq_bands=n_freq_bands
        )
...
def main(args):
    """Main DiffEM-Uncond training loop with MMPS E-step (Freq-Gated Score)."""

    # Model name and directory
    name = create_model_name_and_dir(args)
    name = f"diffem_mmps_freq_gated_score_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print(f"\n{'='*60}")
        print(f"Starting DiffEM-MMPS + Freq-Gated Score with {args.em_iters} EM iterations")
        ...
        print(f"  N freq bands: {args.n_freq_bands}")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("DiffEM-MMPS + Freq-Gated Score Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS + Freq-Gated Score training is complete")
...
if __name__ == '__main__':
    ...
    # Frequency gating parameters (defaults if not provided via CLI)
    if not hasattr(args, 'gate_temperature') or args.gate_temperature is None:
        args.gate_temperature = 5.0
    if not hasattr(args, 'n_freq_bands') or args.n_freq_bands is None:
        args.n_freq_bands = 4
