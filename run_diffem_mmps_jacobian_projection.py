"""
DiffEM with MMPS E-step — Jacobian-Projected Likelihood (JAPS-inspired)
...
Unified model that handles all corruption regimes:
  ...
  - Observation noise only (missing_rate=0): E-step actively denoises via Jacobian projection
  - Missing + noise: imputes and denoises in one pass

Key idea: Instead of solving the CG system (which requires multiple VJPs), project
the likelihood gradient onto the Jacobian subspace using a single VJP. This gives
a closed-form correction that is cheaper than CG and naturally scales with
observation noise:

    1. Compute likelihood gradient: g = A^T(y - A*denoised) / sigma_obs^2
    2. Map through Jacobian: J*g  (one VJP call)
    3. Orthogonal projection coefficient: alpha = <g, J*g> / <J*g, J*g>
    4. Correction: x_hat + sigma_t^2 * alpha * J*g

When sigma_obs=0: correction becomes hard data consistency (standard MMPS, no change).
When sigma_obs>0: the projection naturally down-weights noisy observations because
the likelihood gradient is divided by sigma_obs^2, and the Jacobian projection
keeps the correction in the learned manifold.

KEY ANISOTROPIC NOISE FIX: Uses per-feature noise std so that features with
different scales receive appropriately scaled corrections.
...
# =============================================================================
# Jacobian-Projected Likelihood Decomposition-Projected MMPS
# =============================================================================

class JacobianProjectionMMPSDiffusionProcess:
    """
    MMPS sampler with Jacobian-projected likelihood correction and decomposition
    consistency projection.

    When sigma_obs=0, this is identical to DecompProjectedMMPSDiffusionProcess
    (standard MMPS with hard observation matching via CG).

    When sigma_obs>0, instead of solving the full CG system, the likelihood
    gradient is projected onto the Jacobian subspace using a single VJP:

        1. g = A^T(y - A*denoised) / sigma_obs^2   (likelihood gradient)
        2. J*g = VJP(denoised, x_t, g)              (one backward pass)
        3. alpha = <g, J*g> / <J*g, J*g>            (projection coefficient)
        4. result = denoised + sigma_t^2 * alpha * J*g

    This is cheaper than CG (one VJP instead of cg_iters VJPs) and naturally
    scales with observation noise. Supports per-feature anisotropic noise via
    sigma_obs_per_channel.
    """
    ...
    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        """
        Jacobian-projected likelihood posterior denoiser.

        When sigma_obs=0: standard MMPS via CG (residual = y_obs - mask * denoised).
        When sigma_obs>0: project the likelihood gradient onto the Jacobian subspace
        using a single VJP, yielding a cheap closed-form correction that naturally
        scales with (possibly per-feature) observation noise.
        """
        sigma_sq = sigma ** 2
        ...
        if self.sigma_obs > 0:
            # Likelihood gradient: g = A^T(y - A*denoised) / sigma_obs^2
            residual = x_obs - mask * denoised
            likelihood_grad = mask * residual / (sigma_obs_sq + 1e-10)

            # Project onto Jacobian subspace: J * g
            J_g = vjp_fn(likelihood_grad)

            # Orthogonal projection coefficient: <g, J*g> / <J*g, J*g>
            g_dot_Jg = (likelihood_grad * J_g).sum()
            Jg_dot_Jg = (J_g * J_g).sum() + 1e-30

            proj_coeff = g_dot_Jg / Jg_dot_Jg

            # Projected correction
            result = denoised.detach() + sigma_sq * proj_coeff * J_g.detach()
        else:
            # Standard MMPS for pure imputation
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    print(f"\n=== E-Step (EM iter {em_iter}) — Jacobian-Projected Likelihood MMPS ===")
    uncond_model.eval()
    ...
    with uncond_model.ema_scope():
        process = JacobianProjectionMMPSDiffusionProcess(
        ...
        for start_idx in tqdm(range(0, N, batch_size), desc="E-step Jacobian-Proj"):
...
def main(args):
    ...
    name = f"jacobian_projection_mmps_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        if sigma_obs > 0:
            print(f"  Jacobian-projected likelihood correction ACTIVE (sigma_obs={sigma_obs})")
        else:
        ...
        print(f"\n{'='*60}")
        print(f"Starting Jacobian-Projected DiffEM-MMPS with {args.em_iters} EM iterations")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("Jacobian-Projected DiffEM-MMPS Complete!")
        print("="*60)
        if best_metrics:
