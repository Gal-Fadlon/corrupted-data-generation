...
Transform the entire CG system into the Fourier basis.  Solve the CG problem
in frequency domain where the operator is better conditioned (closer to
diagonal for time series).  This gives a better posterior approximation with
...
# =============================================================================
# Complex-Aware Conjugate Gradient Solver (for Fourier-domain CG)
# =============================================================================

def conjugate_gradient_complex(A_fn, b, maxiter=1):
    """
    Conjugate gradient solver for complex-valued linear system A·x = b.

    Uses complex inner products: <u, v> = sum(conj(u) * v).real
    This is needed when solving the CG system in the Fourier basis,
    where all quantities are complex-valued.

    Args:
        A_fn: callable, linear operator v -> A·v (complex tensors)
        b: right-hand side tensor (complex)
        maxiter: number of CG iterations (default=1)

    Returns:
        x: approximate solution tensor (same shape as b, complex)
    """
    x = torch.zeros_like(b)
    r = b.clone()
    p = r.clone()
    ...
    for _ in range(maxiter):
        Ap = A_fn(p)
        rTr = (r.conj() * r).real.sum()
        pTAp = (p.conj() * Ap).real.sum()
        alpha = rTr / (pTAp + 1e-30)
        x = x + alpha * p
        r_new = r - alpha * Ap
        rTr_new = (r_new.conj() * r_new).real.sum()
        beta = rTr_new / (rTr + 1e-30)
        p = r_new + beta * p
        r = r_new
...
# =============================================================================
# MMPSDiffusionProcess — Fourier-Domain CG variant
# =============================================================================
...
class MMPSDiffusionProcess:
    ...
    This variant transforms the CG system into the Fourier basis, where the
    operator is better conditioned (closer to diagonal) for time series data.
    The CG solve happens entirely in frequency domain, then the result is
    transformed back.
    """
    ...
    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        """
        Posterior denoiser with Fourier-domain CG.

        Steps:
        1. Forward pass with gradient: denoised = D_theta(x_t)
        2. Compute residual r = x_obs - mask * denoised
        3. Transform residual to frequency: r_freq = rfft(r)
        4. Define CG operator in frequency domain:
           A_freq(v_freq) = rfft(sigma_y^2 * irfft(v_freq) + sigma^2 * mask * J^T(mask * irfft(v_freq)))
        5. Solve in frequency domain with complex CG
        6. Transform back: v = irfft(v_freq)
        7. Compute score = J^T(mask * v)
        8. Return: denoised + sigma^2 * score
        ...
        # Width of last dimension (time axis in image space)
        W = r.shape[-1]

        # Transform residual to frequency domain
        r_freq = torch.fft.rfft(r, dim=-1)

        # CG operator in frequency domain:
        # Takes v_freq, converts to spatial, applies the covariance operator,
        # converts back to frequency
        def cg_operator_freq(v_freq):
            v = torch.fft.irfft(v_freq, n=W, dim=-1)
            result = sigma_y_sq * v + sigma_sq * mask * vjp_fn(mask * v)
            return torch.fft.rfft(result, dim=-1)

        # Solve in frequency domain with complex-aware CG
        v_freq = conjugate_gradient_complex(cg_operator_freq, r_freq, maxiter=self.cg_iters)
...
def main(args):
    ...
    name = f"diffem_mmps_fourier_cg_{name}"
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        print("\n" + "="*60)
        print("DiffEM-MMPS + Fourier CG Training Complete!")
        print("="*60)
        if best_metrics:
        ...
        logging.info("DiffEM-MMPS + Fourier CG training is complete")
