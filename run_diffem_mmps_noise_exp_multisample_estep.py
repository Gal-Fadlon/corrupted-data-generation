"""
Experiment 6: Multi-Sample E-step Averaging. Runs MMPS posterior sampling K times
(default K=3) per sequence and averages the results. Averaging reduces noise in
reconstructions — at observed positions, noise partially cancels out. Works for all
corruption types. Controlled by --estep_n_samples arg.
"""
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    ...
    if n_samples > 1:
        print(f"  Multi-sample E-step: averaging {n_samples} posterior samples")
    uncond_model.eval()
    ...
    with uncond_model.ema_scope():
        ...
        for start_idx in tqdm(range(0, N, batch_size), desc="E-step Decomp-Projected"):
            ...
            # Multi-sample E-step: average K posterior samples
            n_samples = getattr(args, 'estep_n_samples', 3)
            if n_samples > 1:
                samples = []
                for k in range(n_samples):
                    x_sample = process.sampling_mmps(x_obs_img, mask_img)
                    samples.append(x_sample)
                x_img_imputed = torch.stack(samples).mean(dim=0)
            else:
...
def main(args):
    ...
    name = f"noise_exp_multisample_estep_{name}"
    logging.info(args)
