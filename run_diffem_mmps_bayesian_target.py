"""
DiffEM with MMPS E-step — Bayesian-Target Posterior Denoiser
...
Unified model that handles all corruption regimes:
  ...
  - Observation noise only (missing_rate=0): E-step actively denoises via refined target
  - Missing + noise: imputes and denoises in one pass

Key idea: Instead of conditioning MMPS on the raw (possibly noisy) observation y,
condition on a Bayesian-refined target that blends y with the model's current
denoiser estimate x_hat:

    y_refined = (sigma_t^2 * y + sigma_obs^2 * x_hat) / (sigma_t^2 + sigma_obs^2)

When sigma_obs=0: y_refined = y (standard MMPS, no change).
When sigma_obs>0: at high sigma_t the target trusts y; at low sigma_t it trusts the
model — so the observation anchor itself gets progressively denoised across reverse
diffusion steps. Across EM iterations the model improves, making x_hat cleaner,
which makes y_refined cleaner, creating the same virtuous cycle as imputation.
...
# =============================================================================
# Bayesian-Target Decomposition-Projected MMPS
# =============================================================================

class BayesianTargetMMPSDiffusionProcess:
    """
    MMPS sampler with Bayesian-refined observation target and decomposition
    ...
    When sigma_obs>0, the observation target at each reverse diffusion step is
    refined via Bayesian fusion:
        y_refined = (sigma_t^2 * y_obs + sigma_obs^2 * x_hat) / (sigma_t^2 + sigma_obs^2)

    This means:
      - At high sigma_t (early): y_refined ≈ y_obs (model is uncertain, trust data)
      - At low sigma_t (late):   y_refined ≈ x_hat (model is confident, ignore noise)

    The denoising improves across EM iterations because the model (and thus x_hat)
    gets better each round.
    """
    ...
    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        """
        Bayesian-target posterior denoiser.

        When sigma_obs=0: standard MMPS (residual = y_obs - mask * denoised).
        When sigma_obs>0: refine the observation target before computing residual:
            y_refined = w_obs * y_obs + w_model * denoised
            where w_obs = sigma_t^2 / (sigma_t^2 + sigma_obs^2)
                  w_model = sigma_obs^2 / (sigma_t^2 + sigma_obs^2)
        """
        sigma_sq = sigma ** 2
        ...
        # Bayesian target refinement
        if sigma_obs_sq > 0:
            w_obs = sigma_sq / (sigma_sq + sigma_obs_sq)
            w_model = sigma_obs_sq / (sigma_sq + sigma_obs_sq)
            y_refined = mask * (w_obs * x_obs + w_model * denoised) + (1 - mask) * x_obs
        else:
            y_refined = x_obs

        r = y_refined - mask * denoised

        # CG operator: sigma_y^2 regularizer (always present for numerical stability)
        def cg_operator(v):
            return self.sigma_y ** 2 * v + sigma_sq * mask * vjp_fn(mask * v)
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    print(f"\n=== E-Step (EM iter {em_iter}) — Bayesian-Target MMPS ===")
    uncond_model.eval()
    ...
    with uncond_model.ema_scope():
        process = BayesianTargetMMPSDiffusionProcess(
        ...
        for start_idx in tqdm(range(0, N, batch_size), desc="E-step Bayesian-Target"):
...
def main(args):
    ...
    name = f"bayesian_target_mmps_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        if sigma_obs > 0:
            print(f"  Bayesian target refinement ACTIVE (sigma_obs={sigma_obs})")
        else:
        ...
        print(f"\n{'='*60}")
        print(f"Starting Bayesian-Target DiffEM-MMPS with {args.em_iters} EM iterations")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("Bayesian-Target DiffEM-MMPS Complete!")
        print("="*60)
        if best_metrics:
