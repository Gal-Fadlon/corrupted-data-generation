"""
DiffEM with MMPS E-step — Kalman Init + Curriculum Missing Rate (Noisy)

Unified corruption handler for missing data, Gaussian noise, and their
combination.  Extends the Kalman + Curriculum variant with observation-noise
support: when --noise_level > 0 the MMPS posterior accounts for additive
Gaussian noise on observed values (sigma_obs), preventing the solver from
over-fitting to noisy observations.
...
class MMPSDiffusionProcess:
    ...
    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        ...
        # This is the covariance matrix ((σ_y² + σ_obs²)I + A·V·Aᵀ) applied to v,
...
def main(args):
    ...
    name = f"diffem_mmps_kalman_curriculum_noisy_{name}"
