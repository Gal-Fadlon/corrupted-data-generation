"""
DiffEM with MMPS E-step — Tweedie Covariance-Aware MMPS

Uses more CG iterations (5 instead of 1) with proper per-feature observation
noise in the CG system, giving the solver enough iterations to correctly balance
anisotropic observation noise against the model's posterior covariance.

KEY ANISOTROPIC NOISE FIX: The actual noise added to data is
    noise_level * std(feature) per feature,
NOT a uniform scalar. So sigma_obs in the CG operator must be a per-channel
tensor, not a scalar.

When sigma_obs=0: 1 CG iteration suffices (unchanged behavior, identical to
standard decomposition-projected MMPS).

When sigma_obs>0: the CG system incorporates per-channel observation noise
covariance and runs 5 iterations to resolve the anisotropic structure.
...
# =============================================================================
# Tweedie Covariance-Aware Decomposition-Projected MMPS
# =============================================================================

class TweedieCovarMMPSDiffusionProcess:
    """
    MMPS sampler with per-channel observation noise covariance and
    decomposition consistency projection.

    When sigma_obs=0, this is identical to DecompProjectedMMPSDiffusionProcess
    (standard MMPS with hard observation matching, 1 CG iteration).

    When sigma_obs>0, the CG system uses per-channel observation noise
    covariance (anisotropic) and runs more CG iterations (default 5) to
    properly resolve the anisotropic structure:

        CG operator: (sigma_y^2 + sigma_obs_per_channel^2) * v + sigma_t^2 * mask * J^T(mask * v)

    This correctly accounts for the fact that actual noise is
    noise_level * std(feature) per feature, not a uniform scalar.
    """
    ...
    def __init__(self, args, diffusion_fn, shape, model,
        ...
        self.cg_iters_noisy = cg_iters_noisy
    ...
    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        ...
        # Per-channel observation noise covariance
        if self.sigma_obs_per_channel is not None:
            # sigma_obs_per_channel shape: (C,) -> reshape to (1, C, 1, 1) for broadcasting
            sigma_obs_sq = (self.sigma_obs_per_channel.to(x_t.device).to(torch.float64) ** 2).reshape(1, -1, 1, 1)
            effective_cg_iters = self.cg_iters_noisy
        else:
            sigma_obs_sq = 0.0
            effective_cg_iters = self.cg_iters

        def cg_operator(v):
            return (sigma_y_sq + sigma_obs_sq) * v + sigma_sq * mask * vjp_fn(mask * v)

        v = conjugate_gradient(cg_operator, r, maxiter=effective_cg_iters)
        score = vjp_fn(mask * v)
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    print(f"\n=== E-Step (EM iter {em_iter}) — Tweedie Covariance MMPS ===")
    uncond_model.eval()
    ...
    # Compute per-feature noise std (anisotropic)
    sigma_obs_per_channel = None
    if sigma_obs > 0:
        # corrupted_data has shape (N, seq_len, features) with last col being time index
        # We need features without the time index
        data_features = corrupted_data[:, :, :-1] if corrupted_data.shape[-1] > args.input_channels else corrupted_data
        noise_std = compute_per_feature_noise_std(data_features, sigma_obs)
        sigma_obs_per_channel = torch.tensor(noise_std, dtype=torch.float32)
        print(f"  Anisotropic noise std per feature: {noise_std}")

    with uncond_model.ema_scope():
        process = TweedieCovarMMPSDiffusionProcess(
        ...
        for start_idx in tqdm(range(0, N, batch_size), desc="E-step Tweedie-Covar"):
...
def main(args):
    ...
    name = f"tweedie_covariance_mmps_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        if sigma_obs > 0:
            print(f"  Tweedie covariance-aware CG ACTIVE (sigma_obs={sigma_obs}, cg_iters_noisy=5)")
        else:
        ...
        print(f"\n{'='*60}")
        print(f"Starting Tweedie Covariance-Aware DiffEM-MMPS with {args.em_iters} EM iterations")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("Tweedie Covariance-Aware DiffEM-MMPS Complete!")
        print("="*60)
        if best_metrics:
