"""
Gaussian Noise Handler — Approach 1: Denoise Between E and M

After the E-step produces reconstructions (which may be noisy since the model
learned from noisy data), apply an explicit denoising filter before the M-step.
This progressively cleans the data across EM iterations.

Denoiser options (--denoiser_type):
  - stl: STL decomposition smoothing (decompose → smooth trend → attenuate residual → recompose)
  - wavelet: Wavelet soft-thresholding denoising (using scipy)

Usage:
  --missing_rate 0.0 --noise_level 0.1 --gaussian_noise_level 0.1 --denoiser_type stl
"""
...
def denoise_wavelet(ts_batch, noise_level=0.1):
    """
    Wavelet soft-thresholding denoising for a batch of time series.
    Uses universal threshold: sigma * sqrt(2 * log(n))

    Args:
        ts_batch: (B, T, C) numpy array
        noise_level: estimated noise std for threshold computation
    Returns:
        denoised: (B, T, C) numpy array
    """
    try:
        import pywt
    except ImportError:
        # Fallback: simple moving average if pywt not available
        print("WARNING: pywt not installed, falling back to moving average denoising")
        from scipy.ndimage import uniform_filter1d
        return uniform_filter1d(ts_batch, size=3, axis=1, mode='nearest')

    B, T, C = ts_batch.shape
    denoised = np.zeros_like(ts_batch)
    threshold = noise_level * np.sqrt(2 * np.log(T))

    for b in range(B):
        for c in range(C):
            coeffs = pywt.wavedec(ts_batch[b, :, c], 'db4', mode='symmetric')
            # Soft threshold all detail coefficients (keep approximation)
            coeffs_thresh = [coeffs[0]]  # keep approximation
            for detail in coeffs[1:]:
                coeffs_thresh.append(pywt.threshold(detail, threshold, mode='soft'))
            denoised[b, :, c] = pywt.waverec(coeffs_thresh, 'db4', mode='symmetric')[:T]

    return denoised


def denoise_stl(ts_batch, residual_keep_fraction=0.3):
    """
    STL-based denoising: decompose and attenuate residual (which contains noise).
    Lower residual_keep_fraction = more aggressive denoising.
    """
    return decompose_and_smooth_ts(ts_batch, residual_keep_fraction=residual_keep_fraction)
...
def main(args):
    ...
    name = f"diffem_gaussian_denoise_between_em_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        for em_iter in range(args.em_iters):
            ...
            # Denoise reconstructions before M-step
            denoiser_type = getattr(args, 'denoiser_type', 'stl')
            noise_level = getattr(args, 'noise_level', 0.0)
            if noise_level > 0:
                if denoiser_type == 'wavelet':
                    reconstructions = denoise_wavelet(reconstructions, noise_level=noise_level)
                    print(f"  Applied wavelet denoising (noise_level={noise_level})")
                else:
                    # STL denoising - more aggressive at higher noise
                    keep_frac = max(0.1, 1.0 - noise_level * 3)
                    reconstructions = denoise_stl(reconstructions, residual_keep_fraction=keep_frac)
                    print(f"  Applied STL denoising (keep_frac={keep_frac:.2f})")
