...
(Operator-generalized variant for continuous-time irregular sampling).

This file is a clone of `run_co_evolving_em_aggregate.py` specialized for the
third corruption type -- time-continuous irregular sampling.  The observation
operator S (see `ContinuousResampleOperator` in `obs_operators.py`) is per-
sample linear interpolation at N_obs continuous sample points t_k in [0, T-1];
it degenerates to PointMask when t_k is integer-valued (numerical regression
test in `tests/test_obs_operators.py::test_continuous_resample_integer_*`).

Corruption types supported:
  * --corruption_type missing    : PointMaskOperator (unchanged backward-compat)
  * --corruption_type continuous : ContinuousResampleOperator (new, this file)

Sampling distribution for the continuous timestamps is controlled by
--t_distribution (uniform / poisson / clustered) and density rho by
--n_obs_ratio in (0, 1].

Failure modes and fixes (paper Section 5):

  5.1  Cold start              → Kalman (continuous) or STL warm start
...
def build_continuous_corruption(seed, rho, dist, nts):
    ...
    Returns a callable sampler(N) -> np.ndarray [N, N_obs] of floats in
    [0, nts - 1], sorted per row.

    Distributions:
      * uniform:   N_obs iid U(0, T-1) then sort.
      * poisson:   Poisson process with rate N_obs / (T-1); pad/truncate to
                   exactly N_obs points (deterministic count for batch shape).
      * clustered: two Gaussian clusters centered at T/4 and 3T/4 with
                   sigma = T/8; a harder regime for the ablation.
    """
    ...
    def _sample(N: int) -> np.ndarray:
        if dist == 'uniform':
            ...
            # Sample more than n_obs points per sample, truncate to n_obs.
            ...
            ts = ts[:, :n_obs]
            # Pad with uniform if any row is short (rare given overs=1.5x).
            if ts.shape[1] < n_obs:
...
def get_continuous_data_from_loader(train_loader, rho, dist, seed, device):
    """Extract clean TS, draw per-sample continuous timestamps, apply S.

    Used for --corruption_type continuous.  Only the training loader is
    passed; downstream code treats the irregular observations as the
    "corrupted" data for the E-step while the test loader remains clean
    (for generation metrics).

    Returns:
      y_obs:       [N, N_obs, F]   continuous-time irregular observations.
      timestamps:  [N, N_obs]      float sample times in [0, T-1], sorted.
      x_clean:     [N, T, F]       clean training sequences that S was
                                   applied to (for diagnostic logging only).
    """
    all_clean = []
    for batch_idx, data in enumerate(train_loader):
        x_irregular = data[0]
        ...
        if torch.isnan(x_ts).any():
            raise ValueError(
                "get_continuous_data_from_loader: the training loader "
                "contains NaNs; --corruption_type continuous expects a "
                "clean (non-missing) training source.  Set --missing_rate "
                "0.0 and check the dataset config."
            )
    ...
    timestamps = sampler(N)                                    # [N, N_obs]
    N_obs = timestamps.shape[1]
    print(f"  [continuous] rho={rho}, dist={dist}, N_obs={N_obs}/{T}")

    # Apply S per-sample via a tiny torch op (mirrors ContinuousResampleOperator).
    ...
    op.set_grid(t)
    with torch.no_grad():
        y_obs = op.apply(x).cpu().numpy()                       # [N, N_obs, F]
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device,
    ...
    corrupted_data :
      ...
      * ContinuousResample:    [N, N_obs, F] float32 continuous-time obs y_k.
    obs_masks :
      * PointMask:            [N, T] bool, True where observed.
      * BlockMean / Continuous: None.
    timestamps_data :
      * ContinuousResample:    [N, N_obs] float array of sample times in
                               [0, T-1], sorted per sample.
      * Other ops:             None.
    op :
      ObservationOperator instance (required for this script).
    """
    assert op is not None, \
        "run_co_evolving_em_continuous.e_step requires an ObservationOperator"
    ...
    is_continuous = isinstance(op, ContinuousResampleOperator)

    if is_stft and (is_block_mean or is_continuous):
        raise NotImplementedError(
            "STFT embedder + non-pointwise operator (block-mean / continuous "
            "resample) is not supported yet.  Use --embedder delay."
        )
    if is_continuous and timestamps_data is None:
        raise ValueError(
            "e_step(continuous): timestamps_data must be provided for "
            "ContinuousResampleOperator.  The sampler is built in "
            "get_continuous_data_from_loader."
        )
    ...
    with uncond_model.ema_scope():
        ...
        for start_idx in tqdm(range(0, N, batch_size), desc="E-step"):
            ...
            if is_continuous:
                # y_batch: [B, N_obs, F] in y-space; also push the per-batch
                # continuous timestamps into the operator so apply / adjoint /
                # enforce act with this batch's S matrix inside DualSpaceMMPS.
                obs_ts = torch.tensor(corrupted_batch, dtype=torch.float32,
                                      device=device)
                ts_batch = torch.tensor(
                    timestamps_data[start_idx:end_idx],
                    dtype=torch.float32, device=device,
                )
                op.set_grid(ts_batch)
                # Placeholder image-space tensors (unused on obs-space CG path
                # for continuous resample).
    ...
    elif corruption_type == 'continuous':
        _rho = float(getattr(args, 'n_obs_ratio', 0.5))
        _dist = str(getattr(args, 't_distribution', 'uniform'))
        cache_key = f'continuous_rho{int(_rho * 100):02d}_{_dist}'
    else:
...
def m_step(args, uncond_model, optimizer, reconstructions,
    ...
    For --corruption_type continuous, L_obs uses the operator-exact form
        ||S · D_theta^ts(x_img) - y||^2
    where S is the per-sample linear-interpolation matrix, computed by
    op.set_grid(timestamps) on each mini-batch.  L_obs is still gated in v1
    for --corruption_type aggregate because the SNR weighting has not been
    re-derived for the block-mean A_ts.
    """
    ...
    if is_continuous and lambda_obs > 0 and timestamps_data is None:
        raise ValueError(
            "m_step(continuous, lambda_obs>0) requires timestamps_data."
        )
    ...
    if is_block_mean:
        ...
        # Carry y_obs and timestamps per sample so L_obs can compute
        # ||S · D_theta^ts - y|| on each batch (set_grid called inside loop).
        y_tensor = torch.tensor(corrupted_data, dtype=torch.float32)   # [N, N_obs, F]
        ts_tensor = torch.tensor(
            timestamps_data if timestamps_data is not None else
            np.zeros((recon_tensor.shape[0], 1), dtype=np.float32),
            dtype=torch.float32,
        )                                                               # [N, N_obs]
    ...
    for epoch in range(args.m_step_epochs):
        ...
        for batch_idx, (x_clean, y_ts_batch, mask_ts_batch) in enumerate(recon_loader):
            ...
            if is_continuous and lambda_obs > 0:
                # Second slot is [B, N_obs, F] y_obs; third slot is
                # [B, N_obs] timestamps.  Push the per-batch timestamps into
                # the operator so op.apply acts with this batch's S below.
                op.set_grid(mask_ts_batch)
            ...
            if lambda_obs > 0:
                ...
                if is_continuous:
                    # Operator-exact L_obs: || S · D_theta^ts - y ||^2.
...
def main(args):
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        n_obs_continuous = max(1, int(round(n_obs_ratio * seq_len)))

        # The operator is built unconditionally per corruption type; we pass
        # n_obs / kernel kwargs so the factory can construct a
        # ContinuousResampleOperator when requested.
        op = build_operator(
            corruption_type=corruption_type,
            seq_len=seq_len,
            n_features=n_features,
            agg_window=agg_window,
            n_obs=n_obs_continuous if corruption_type == 'continuous' else None,
            interp_kernel=interp_kernel,
        )
        _suffix = ""
        if corruption_type == 'aggregate':
            _suffix = f" window={agg_window}"
        elif corruption_type == 'continuous':
            _suffix = (f" rho={n_obs_ratio} dist={t_distribution} "
                       f"kernel={interp_kernel}")
        print(f"[operator] corruption_type={corruption_type} "
              f"name={op.name} n_obs={op.n_obs}{_suffix}")

        # Per-sample timestamps; only populated for --corruption_type continuous.
        timestamps_all = None
        ...
        if corruption_type == 'aggregate':
            ...
            print(f"Extracting clean TS and applying continuous-time "
                  f"resampling (rho={n_obs_ratio}, dist={t_distribution})...")
            y_obs, timestamps_all, _x_clean_dbg = get_continuous_data_from_loader(
                train_loader, rho=n_obs_ratio, dist=t_distribution,
                seed=args.seed, device=args.device,
            )
            corrupted_data = y_obs                # shape [N, N_obs, F]
            obs_masks = None                       # unused for continuous
            print(f"  {len(corrupted_data)} sequences, y shape = "
                  f"{corrupted_data.shape}, timestamps shape = "
                  f"{timestamps_all.shape}")

            # --- Oracle disc sanity: disc_mean(x, S^+(S x)) on a test batch ---
            # Parallels the STFT oracle check in run_co_evolving_em.py / the
            # aggregate script (lines ~1255).  Gives an honest upper bound on
            # what EM can achieve: if the continuous-resample op itself is
            # lossy at this density, no amount of training drives disc to 0.
            try:
                from metrics import evaluate_model_irregular as _eval_ir
                oracle_sampler = build_continuous_corruption(
                    seed=args.seed + 9999,
                    rho=n_obs_ratio, dist=t_distribution, nts=seq_len,
                )
                op_oracle = ContinuousResampleOperator(
                    seq_len=seq_len, n_features=n_features,
                    n_obs=n_obs_continuous,
                )
                oracle_reals = []
                oracle_rt = []
                with torch.no_grad():
                    for data in test_loader:
                        xb = data[0][:, :, :-1]                      # drop time col
                        B_ = xb.shape[0]
                        ts_b = torch.as_tensor(
                            oracle_sampler(B_), dtype=torch.float32,
                        )
        ...
        if corruption_type == 'aggregate':
            ...
            if init_method == 'kalman':
                print("Continuous-resample warm start: Kalman on interpolated grid")
                initial_reconstructions = initialize_with_kalman_continuous(
                    corrupted_data, timestamps_all, seq_len=seq_len, seed=args.seed,
                ...
                # Tier-1 improvement: snap continuous timestamps to the nearest
                # integer grid cell, build a NaN-masked [N, T, F] array, and
                # route through the PointMask-style Kalman init that handles
                # missing values natively via UnobservedComponents.  This
                # avoids the pre-smoothing bias introduced by linear-interp
                # onto the grid: the Kalman smoother sees high-fidelity anchor
                # values and fills the gaps using the structural TS model
                # (level + seasonal) rather than interpolating twice.
                print("Continuous-resample warm start: Kalman with snap-to-grid "
                      "(NaN-masked UnobservedComponents)")
                N_total = corrupted_data.shape[0]
                F_chan = corrupted_data.shape[-1]
                grid_series = np.full((N_total, seq_len, F_chan), np.nan,
                                      dtype=np.float32)
                grid_mask = np.zeros((N_total, seq_len), dtype=bool)
                # Snap each (t_k, y_k) onto the nearest integer grid cell;
                # if multiple observations collide on the same cell, average
                # them so the cell value reflects all colliding anchors.
                for i in range(N_total):
                    t_i = timestamps_all[i]
                    y_i = corrupted_data[i]
                ...
                collisions = (grid_mask.sum(axis=1) < corrupted_data.shape[1]).sum()
                if collisions > 0:
                    print(f"  Snap-to-grid: {collisions}/{N_total} samples had "
                          ">=1 timestamp collision (averaged on the cell).")
            else:
                use_stl_warm = init_method != 'replicate'
                print(f"Continuous-resample warm start: linear interp "
                      f"(use_stl={use_stl_warm})")
                initial_reconstructions = op.warm_start(
                    corrupted_data, timestamps_all, seed=args.seed,
        ...
        for em_iter in range(args.em_iters):
            ...
            # PointMask: reveal extra grid positions from last_recon.
            # BlockMean: no-op (every window is observed already).
            # ContinuousResample: optional timestamp-curriculum (opt-in via
            #   --continuous_curriculum).  We sample π(k)·T extra continuous
            #   timestamps from the same distribution as the base observations,
            #   evaluate last_recon at those times via the ContinuousResample
            #   operator (2-tap linear-interp), and augment (y_obs, timestamps).
            #   A fresh op_curr with the enlarged n_obs is threaded through
            #   e_step / m_step for this iteration only; op.set_grid is called
            #   per-batch so the enlarged matrix S is rebuilt automatically.
            op_iter = op
            curriculum_timestamps = timestamps_all
            if op.supports_curriculum:
            ...
            elif (isinstance(op, ContinuousResampleOperator)
                  and getattr(args, 'continuous_curriculum', False)):
                progress = min(1.0, (em_iter + 1) / max(args.em_iters * 0.6, 1))
                reveal_frac = (1.0 - progress) * 0.3
                n_extra = int(round(reveal_frac * seq_len))
                if n_extra > 0:
                    t_dist = getattr(args, 't_distribution', 'uniform')
                    ...
                    if ts_extra.shape != (N_total, n_extra):
                        if ts_extra.ndim == 2 and ts_extra.shape[1] >= n_extra:
                ...
                if n_extra > 0:
                    ...
                    curriculum_corrupted = np.concatenate(
                        [corrupted_data, y_extra], axis=1,
                    )  # [N, N_obs_base + n_extra, F]
                    curriculum_timestamps = np.concatenate(
                        [timestamps_all, ts_extra], axis=1,
                    )  # [N, N_obs_base + n_extra]
                    curriculum_masks = None
                    op_iter = ContinuousResampleOperator(
                        seq_len=seq_len, n_features=n_features,
                        n_obs=corrupted_data.shape[1] + n_extra,
                    )
                    print(
                        f"  Continuous curriculum: +{n_extra} timestamps "
                        f"(progress={progress:.2f}, N_obs "
                        f"{corrupted_data.shape[1]} -> "
                        f"{curriculum_corrupted.shape[1]})"
                    )
                else:
                    curriculum_masks = None
            ...
            reconstructions = e_step(
                ...
                timestamps_data=curriculum_timestamps,
            )
            ...
            # --- M-step (Sections 5.5–5.6) ---
            # M-step's L_obs uses the augmented (y, ts) so the model learns to
            # match both the real observations AND the curriculum-revealed
            # positions; when reveal_frac anneals to 0, L_obs degenerates back
            # to the base observations only.
            uncond_model = m_step(
                args, uncond_model, optimizer,
                reconstructions, curriculum_corrupted, curriculum_masks,
                em_iter, args.device, logger, op=op_iter,
                timestamps_data=curriculum_timestamps,
            )
...
# =============================================================================
# CLI shim for continuous-specific flags (keeps utils_args.py untouched)
# =============================================================================


def _parse_continuous_args():
    """Strip continuous-only CLI flags out of sys.argv before forwarding to
    utils.utils_args.parse_args_irregular.

    Mirrors the shim pattern in run_co_evolving_em_noise.py and
    run_co_evolving_em_aggregate.py and avoids touching the shared argparse
    used by every other script in the repo.
    """
    import argparse as _argparse
    p = _argparse.ArgumentParser(add_help=False)
    _SUPPRESS = _argparse.SUPPRESS
    p.add_argument(
        '--corruption_type', type=str, default='continuous',
        choices=['missing', 'continuous'],
        help='Corruption model: missing (PointMask) or continuous '
             '(ContinuousResampleOperator, this script\'s specialty).',
    )
    p.add_argument(
        '--n_obs_ratio', type=float, default=0.5,
        help='Density rho in (0, 1].  N_obs = round(rho * seq_len) '
             'observations per sample.  Matches the --missing_rate semantic '
             'from PointMask when rho = 1 - missing_rate.',
    )
    p.add_argument(
        '--t_distribution', type=str, default='uniform',
        choices=['uniform', 'poisson', 'clustered'],
        help='How the per-sample sample times {t_k} are drawn.',
    )
    p.add_argument(
        '--interp_kernel', type=str, default='linear',
        choices=['linear', 'cubic', 'sinc'],
        help='Interpolation kernel of S.  Only linear is implemented in v1; '
             'cubic / sinc raise NotImplementedError (v2 ablation rows).',
    )
    # --agg_window kept so the shared factory call in main() does not trip on
    # an attribute-missing path; it is ignored by the continuous pipeline.
    p.add_argument('--agg_window', type=int, default=1, help=_SUPPRESS)
    p.add_argument(
        '--continuous_curriculum', action='store_true',
        help='Enable the timestamp-curriculum for ContinuousResample (Tier-1 '
             'improvement): draw pi(k)*T extra continuous timestamps per EM '
             'iteration, evaluate the previous reconstruction at them, and '
             'augment (y_obs, timestamps) so the operator sees more '
             'observations early.  pi(k) anneals linearly from 0.3 to 0 over '
             'the first 60%% of EM iterations.',
    )
