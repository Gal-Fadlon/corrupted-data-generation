"""
DiffEM with MMPS E-step — Kalman Init + Frequency-Gated MMPS Guidance + Curriculum

Combines Kalman smoother initialization with frequency-gated MMPS:
  1. Kalman smoother initialization — optimal linear state-space imputation
     via UnobservedComponents with Kalman smoothing that handles NaN natively.
  2. Frequency-gated MMPS E-step — observation guidance is low-pass filtered
     based on the current noise level (sigma). At high sigma only low-frequency
     (trend) guidance is active; high-frequency details are injected only at
     low noise where the model has settled the coarse structure.
  3. Curriculum missing rate — early EM iterations reveal extra observations.

Based on run_diffem_mmps_freq_gated.py with init swapped to
initialize_with_kalman.

References:
  Lin et al. "A Fourier Space Perspective on Diffusion Models" (2025)
  Rissanen et al. "Generative Modelling with Inverse Heat Dissipation" (2023)
"""
...
def lowpass_filter_image(x, cutoff_fraction):
    """
    Low-pass filter an image tensor via FFT truncation.

    Args:
        x: (B, C, H, W) tensor
        cutoff_fraction: fraction of frequencies to keep (0.0 = DC only, 1.0 = all)

    Returns:
        filtered: (B, C, H, W) low-passed tensor
    """
...
def compute_frequency_gate(sigma, sigma_max, gate_schedule='linear'):
    """
    Compute the fraction of frequency content to include in guidance.

    At sigma = sigma_max: only DC / very low freq (cutoff ~ 0.1)
    At sigma = 0:         all frequencies (cutoff = 1.0)

    Args:
        sigma: current noise level
        sigma_max: maximum noise level
        gate_schedule: 'linear' or 'cosine'

    Returns:
        cutoff_fraction in [0.1, 1.0]
    """
...
# =============================================================================
# Frequency-Gated MMPSDiffusionProcess
# =============================================================================

class FreqGatedMMPSDiffusionProcess:
    """
    MMPS sampler with frequency-gated observation guidance.

    At each reverse step, the residual r = x_obs - mask * denoised is
    low-pass filtered according to the current sigma level.  High-frequency
    observation details are only injected at low noise levels where the
    model has already settled the coarse structure.
    """
    ...
    def posterior_denoise(self, x_t, sigma, x_obs, mask):
        """Frequency-gated posterior denoiser."""
        sigma_sq = sigma ** 2
        ...
        # Frequency gating: low-pass filter the residual based on sigma
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    print(f"\n=== E-Step (EM iter {em_iter}) — Frequency-Gated MMPS ===")
    uncond_model.eval()
    ...
    with uncond_model.ema_scope():
        process = FreqGatedMMPSDiffusionProcess(
            args, uncond_model.net, target_shape,
            sigma_y=sigma_y, cg_iters=cg_iters,
            gate_schedule='linear'
        )

        for start_idx in tqdm(range(0, N, batch_size), desc="E-step Freq-Gated"):
...
def main(args):
    ...
    name = f"diffem_mmps_kalman_freq_gated_{name}"
    logging.info(args)
    ...
    with CompositeLogger([WandbLogger()]) if args.wandb else PrintLogger() as logger:
        ...
        # Phase 2: EM loop with frequency-gated MMPS E-step + curriculum
        print(f"\n{'='*60}")
        print(f"Starting Kalman + Frequency-Gated DiffEM-MMPS with {args.em_iters} EM iterations")
        print(f"{'='*60}")
        ...
        print("\n" + "="*60)
        print("Kalman + Frequency-Gated DiffEM-MMPS Training Complete!")
        print("="*60)
        if best_metrics:
            print("Best metrics:")
