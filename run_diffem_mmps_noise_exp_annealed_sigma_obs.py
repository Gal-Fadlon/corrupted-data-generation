"""
Experiment 2: Annealed sigma_obs. Early EM iterations use inflated sigma_obs (2x)
so the model denoises more freely. Late iterations use the true sigma_obs, trusting
observations more as the model improves. For missing-only (noise_level=0), no change.
"""
...
def e_step(args, uncond_model, corrupted_data, obs_masks, em_iter, device, logger=None):
    ...
    if sigma_obs_base > 0 and em_iter >= 0:
        # Anneal: start at 2x, decay to 1x over EM iterations
        progress = min(1.0, (em_iter + 1) / max(args.em_iters * 0.7, 1))
        anneal_factor = 2.0 - progress  # 2.0 -> 1.0
        sigma_obs = sigma_obs_base * anneal_factor
        print(f"  Annealed sigma_obs: {sigma_obs:.4f} (base={sigma_obs_base}, factor={anneal_factor:.2f})")
    else:
...
def main(args):
    ...
    name = f"noise_exp_annealed_sigma_obs_{name}"
    logging.info(args)
